...
## aarch64-pc-windows-msvc (`//:natives-win32-arm64`)

Second instantiation of the same rules: `@xwin_sysroot_arm64` (`arch = "aarch64"`,
`sdk_version = "10.0.22621"` — the latest SDK's arm64 MSIs reference no cab
files, which xwin 0.6.5 rejects) and `@msvc_cc_arm64` (`arch = "aarch64"`), with
`msvc-arm64-cc-from-*` toolchain()s gated on `[windows, aarch64]`. Arm64-only
quirks, all generated by `cc.bzl` for `arch = "aarch64"`:

- `bin/clang`: ring 0.17 forces GNU-driver `clang` from PATH for
  windows-aarch64; the shim translates the toolchain's clang-cl-style CFLAGS
  (`/D` → `-D`, other `/flags` dropped), selects the static CRT and adds the
  xwin includes. The `ring` crate.annotation prepends `@msvc_cc_arm64`'s bin/.
- `toolchain.cmake` sets `OPUS_PRESUME_NEON=ON` / `OPUS_MAY_HAVE_NEON=OFF`:
  opus' ARM RTCD (`celt/arm/armcpu.c`) uses MSVC's `__emit`, absent in clang-cl.
