...
- A ChatGPT account that keeps serving Codex requests off its credit balance is no longer treated as exhausted once its plan window is spent. `/wham/usage` reports only the plan verdict (`allowed:false`, `limit_reached:true`, `used_percent:100`), so credential selection blocked the account until the weekly reset — and could never heal that block — while the `codex` CLI on the same login kept working. `credits`/`spend_control` are now parsed, and credits that can fund plan overage keep the account selectable and let a stale usage-limit block clear. The override is scoped to the plan meter: a separate metered feature (Spark, reserve) keeps its own verdict, and a refusal that is not plan exhaustion still blocks ([#12171](https://github.com/can1357/oh-my-pi/pull/12171)).
