...
- Cursor passthrough ignores server-hosted `web_fetch` interaction announcements (while retaining deferrable `fetchArgs`) and rejects unsupported `toolChoice: "required"` instead of silently weakening to auto.
