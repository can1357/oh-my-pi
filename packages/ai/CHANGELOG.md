...
- Fixed forced `tool_choice` requests failing turns when models reject forced selectors: Anthropic, OpenAI Completions, and OpenAI Responses transports now self-heal by catching HTTP 400 forced-tool-choice rejections before stream output begins, recording the model in a process-lifetime rejection memo, logging a warning, and immediately retrying once with automatic tool selection while keeping tools advertised ([#13248](https://github.com/can1357/oh-my-pi/pull/13248) by [@jwaldrip](https://github.com/jwaldrip)).
