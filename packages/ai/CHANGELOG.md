...
- Fixed a Codex ChatGPT-account (or Cursor plan) model denial that no sibling credential could rotate away from surfacing as the provider's bare sentence: the credential resolver now throws `ModelEntitlementError`, which the auth-retry loop puts on the terminal error in place of the denial (the original text stays as `errorClassificationMessage`), naming the accounts that were denied or parked, any account signed out recently, and the `/login` step that fixes it. Any other resolver failure is still swallowed in favour of the provider's error.
