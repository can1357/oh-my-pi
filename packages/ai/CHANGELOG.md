...
- An OAuth credential the auth layer disables automatically (definitive refresh failure, upstream token invalidation, Copilot hard-401 retirement, broker disable) is now logged with its provider, account, and cause and reported through `credential_disabled`, so a session can no longer degrade to a sibling account with no record of what happened. Diagnostic causes are redacted for display while the protected forensic cause is kept intact ([#11910](https://github.com/can1357/oh-my-pi/pull/11910) by [@alphastorm](https://github.com/alphastorm)).
- Automatic OAuth credential disables (definitive refresh failure, upstream token invalidation, matched Copilot hard-401 exhaustion, broker disable) now log the provider, credential id, account, and cause, and `credential_disabled` events carry the credential id and account identity. Actionable summaries retain authoritative broker identities even when refresh claims are withheld, including successful normal, forced, and stored refreshes, changed or removed identity scopes, and untouched sibling entries, use fresh recovery state, respect account pools, and redact diagnostic causes while protected forensic data remains intact. Refresh mirrors retain the pre-refresh row and adopt newer authoritative broker data even if another caller already hydrated it. Usage prechecks are reused only for the same normalized routing identity. Snapshot pulls and stream bootstrap preserve intervening updates while allowing quiescent broker-restart resynchronization. Deferred API-key notices retire when a current same-provider API key proves recovery. Credential targeting validates supplied id/bearer pairs and retains its matched row across asynchronous resolution; cancellation also bounds noncooperative keyed lookups. Disabled-history project identities are capability-negotiated for strict older clients; supported empty history remains distinguishable from unsupported lookups. Catalog-defined Copilot retirement preserves missing/stale request identities, overrides, policy/quota exclusions, and peer replacements. Already-connected broker sessions receive actionable automatic-disable notices through snapshot updates, suppressing accounts recovered in the latest snapshot, without duplicating overlapping client-initiated notices or announcing logout and replacement; an all-failed local batch still announces a winning peer disable. Usage-facing history is re-read after credential revalidation; identity-less OAuth tombstones remain until an explicit later login or replacement establishes ordering. Failed broker removal classification retains token-free pending work with capped, idle-safe retry until recovery or authoritative classification. Credential-disable callbacks fan out from a listener snapshot and isolate callback failures, so one throwing or unsubscribing listener cannot starve peers. ([#11910](https://github.com/can1357/oh-my-pi/pull/11910) by [@alphastorm](https://github.com/alphastorm)).
- Automatic OAuth failure tombstones survive re-login for 30 days from their original disable; repeated disables of a retained row cannot overwrite its original cause or restart its retention; display warnings coalesce repeated account generations without modifying those forensic rows. Recovery reminders require a matching known identity, so an unidentified account remains visible until its history expires or is deliberately cleared. Single-account removal clears matching history and any pre-existing tombstone for the explicitly selected row, preserving unrelated unidentified tombstones; whole-provider logout clears that provider's prior history locally and through the broker, even with no active credentials. The public asynchronous logout hook retains its deliberate-removal cause argument for existing stores. Remote provider logout requires the broker's new provider-logout endpoint and rejects providers restricted by the client's account pool, preserving hidden accounts and history. Provider-wide and single-account removal propagate persistence failures without hiding durable credentials ([#11911](https://github.com/can1357/oh-my-pi/pull/11911) by [@alphastorm](https://github.com/alphastorm)).
