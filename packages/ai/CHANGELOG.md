...
- Harmony leak detection now catches marker-free prior collapse in the visible answer, the shape gpt-5.6-sol produces most often. The detector previously required a surviving `to=functions.NAME` marker before it would evaluate any co-signal, so a turn that collapsed committed silently and then re-seeded itself on every later turn — remote compaction preserves provider-native history, so `/compact` does not clear it. Four visible-channel signals now abort and retry the turn: staccato terminator runs (`D`), fabricated harness notices (`N`), stranded non-Latin script residue (`S`), and fabricated harness envelopes (`E`) — the model writing omp's own `<system-notice>` / `<job-progress>` wrapper tags into its answer, observed once as a 357,273-character block containing 824 self-authored wakes with an `elapsed` counter it advanced to `33d23h`. A bare marker in a rendered answer (`V`) also trips on its own now. Thresholds are measured against 6,006 assistant text blocks from 261 persisted sessions: 104 hits across 6 sessions, none on the other 255 and none on 52,033 thinking blocks. See `docs/ERRATA-GPT5-HARMONY.md` §2.9.
- Fenced-code exemption in harmony leak detection follows CommonMark fence semantics: a closing fence must repeat the opening character at least as many times and carry no info string. Naive toggling let a ` ```xml ` block nested inside a ` ```text ` block close the outer fence early, exposing its contents to detection — the only false positive measured across the corpus.
