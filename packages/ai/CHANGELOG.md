...
- Cursor tool passthrough synthesizes `writeShellStdinArgs` and `redactedReadArgs` for the caller, and excludes server-only `connect_scm` from the allowlist / interaction path.
