...
- OpenRouter requests now opt in to routing metadata (`X-OpenRouter-Metadata: enabled`) and the Responses API surface parses the selected endpoint from `openrouter_metadata.endpoints.available[]`, populating `upstreamProvider` on assistant messages for both the chat-completions and Responses paths ([#10258](https://github.com/can1357/oh-my-pi/pull/10258) by [@forest922](https://github.com/forest922)).
