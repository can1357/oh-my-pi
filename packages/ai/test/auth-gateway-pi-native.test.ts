...
/** Valid 1x1 BMP: a well-formed image in a format this build cannot verify or serialize. */
...
describe("pi-native gateway image reference validation", () => {
	...
	it("keeps assistant image artifacts out of input validation", async () => {
		const fixture = await createPiNativeImageGatewayFixture();
		fixture.mock.push({ content: ["ok"] });
			...
			const response = await fetch(`${fixture.handle.url}/v1/pi/stream`, {
				...
				body: JSON.stringify({
					...
					context: {
						messages: [
							baseAssistant({
								api: fixture.mock.api,
								provider: fixture.mock.provider,
								model: fixture.mock.id,
								content: [
									{
										type: "image",
										data: "",
										mimeType: "image/png",
										providerFile: { provider: "openai", id: "file_display_only" },
									},
								],
							}),
						],
	...
	it("rejects a computer screenshot whose runtime shape is not a computer_screenshot ref", async () => {
		const upstreamRequests: Array<Record<string, unknown>> = [];
		const upstream = Bun.serve({
			hostname: "127.0.0.1",
			port: 0,
				...
				const events = [
					{
						type: "response.output_item.added",
						output_index: 0,
						item: { type: "message", id: "msg_computer_shape", role: "assistant", content: [] },
					},
					{ type: "response.output_text.delta", delta: "ok" },
					{
						type: "response.output_item.done",
		...
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-pi-native-computer-shape-"));
		const storage = await AuthStorage.create(path.join(dir, "auth.db"));
		storage.setRuntimeApiKey("openai", "test-key");
		const model = buildModel({
			id: "gpt-5.4",
			name: "Custom Responses Computer Shape",
			api: "openai-responses",
			provider: "openai",
			baseUrl: `${upstream.url.origin}/v1`,
			reasoning: false,
		...
		const screenshotUrl = "https://images.example.invalid/shape.png";
		...
		try {
						...
						messages: [
							{
								...
								content: [
									{
										type: "toolCall",
										id: "call_computer_shape|item_computer_shape",
										name: "computer",
										arguments: {},
										providerMetadata: {
											type: "computer",
											providerItemId: "item_computer_shape",
											actions: [{ type: "screenshot" }],
										},
									},
								],
							...
							{
								role: "toolResult",
								toolCallId: "call_computer_shape|item_computer_shape",
								toolName: "computer",
								content: [],
								providerMetadata: {
									type: "computer",
									screenshot: { type: "bogus", image_url: screenshotUrl },
									acknowledgedSafetyChecks: [],
								},
								isError: false,
								timestamp: 0,
			...
			const input = upstreamRequests[0]?.input;
			if (!Array.isArray(input)) throw new Error("expected Responses input");
			expect(JSON.stringify(input)).not.toContain("bogus");
			expect(input).toContainEqual({
				type: "computer_call_output",
				call_id: "call_computer_shape",
				output: { type: "computer_screenshot", image_url: screenshotUrl },
			});
		} finally {
			await handle.close();
			storage.close();
	...
	it("degrades a well-formed unverifiable image instead of rejecting the request", async () => {
		const upstreamRequests: CapturedOpenAICompletionRequest[] = [];
		const upstream = startOpenAICompletionsUpstream(upstreamRequests);
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-pi-native-placeholder-image-"));
		const storage = await AuthStorage.create(path.join(dir, "auth.db"));
		storage.setRuntimeApiKey("openai", "test-key");
		const model = buildModel({
			id: "pi-native-placeholder-image",
			name: "Pi Native Placeholder Image",
			api: "openai-completions",
			provider: "openai",
			baseUrl: `${upstream.url.origin}/v1`,
			reasoning: false,
	...
	it("preserves a supported canonical image source beside stale computer metadata", async () => {
		const upstreamRequests: CapturedOpenAICompletionRequest[] = [];
		const upstream = startOpenAICompletionsUpstream(upstreamRequests);
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-pi-native-computer-alternate-"));
		const storage = await AuthStorage.create(path.join(dir, "auth.db"));
		storage.setRuntimeApiKey("openai", "test-key");
		const model = buildModel({
			id: "pi-native-computer-alternate",
			name: "Pi Native Computer Alternate",
			api: "openai-completions",
			provider: "openai",
			baseUrl: `${upstream.url.origin}/v1`,
			reasoning: false,
		...
		const imageUrl = "https://images.example.invalid/computer.png";
		...
		try {
				...
				body: JSON.stringify({
					...
					context: {
						messages: [
							{
								role: "assistant",
								content: [{ type: "toolCall", id: "call_computer_alt", name: "computer", arguments: {} }],
								api: model.api,
								provider: model.provider,
								model: model.id,
								usage: ZERO_USAGE,
							...
							{
								role: "toolResult",
								toolCallId: "call_computer_alt",
								toolName: "computer",
								content: [{ type: "image", data: "", mimeType: "image/png", url: imageUrl }],
								providerMetadata: {
									type: "computer",
									screenshot: { type: "computer_screenshot", file_id: "file_stale_screen" },
									acknowledgedSafetyChecks: [],
			...
			expect(response.status).toBe(200);
			await response.json();
			const imageParts = (upstreamRequests[0]?.messages ?? []).flatMap(message =>
				Array.isArray(message.content) ? message.content : [],
			);
