...
describe("credential disable bearer CAS", () => {
	...
	test("a disable under an active stream writes the credential out of the cache at once", async () => {
		const token = "cache-persist-token";
		handle?.close();
		handle = startAuthBroker({ storage, bind: "127.0.0.1:0", bearerTokens: [token], disableRefresher: true });
		const client = new AuthBrokerClient({ url: handle.url, token });
		const initial = await client.fetchSnapshot();
		...
		const streamed = new RemoteAuthCredentialStore({
			...
			streamSnapshots: true,
			onSnapshot: snapshot => {
				persisted.push(snapshot.credentials.map(entry => entry.id));
			},
		});
		remote = streamed;
		try {
			...
			// An active stream suppresses the follow-up refresh, so without an
			// immediate write the encrypted cache keeps the dead bearer for its
			// whole lifetime and a restart in that window would use it.
			expect(persisted.length).toBeGreaterThan(0);
			expect(persisted.at(-1)).not.toContain(id);
		} finally {
			streamed.close();
			remote = undefined;
		}
	...
	test("an unsupported-history latch expires so a same-generation replacement recovers", async () => {
		// A replacement broker can resume at the client's current generation, which
		// no restart detector sees. The latch expires instead of relying on one.
		let historySupported = false;
		const token = "latch-ttl-token";
		handle?.close();
		handle = startAuthBroker({ storage, bind: "127.0.0.1:0", bearerTokens: [token], disableRefresher: true });
		const upstreamUrl = handle.url;
		const proxy = Bun.serve({
		...
		try {
			...
			// Same generation, new capability: only the expiry recovers this.
			historySupported = true;
