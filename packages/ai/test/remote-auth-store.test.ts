...
describe("RemoteAuthCredentialStore + AuthStorage integration", () => {
	...
	test("conditional generated-key upload preserves a credential written concurrently on the broker", async () => {
		...
		await serverStorage!.credentials.set("anysearch", { type: "api_key", key: "broker-login-key", source: "login" });
		const inserted = await clientStorage.credentials.addGeneratedApiKeyIfAbsent("anysearch", "generated-key");
		...
		expect(clientStorage.credentials.list("anysearch").map(entry => entry.credential)).toEqual([
			{ type: "api_key", key: "broker-login-key", source: "login" },
		]);
		clientStorage.close();
	});
	...
	test("aborted conditional upload does not persist a generated key on the broker", async () => {
		const originalAddGeneratedApiKeyIfAbsent = serverStorage!.credentials.addGeneratedApiKeyIfAbsent.bind(
			serverStorage!.credentials,
		);
		const writeStarted = Promise.withResolvers<void>();
		const serverAttemptSettled = Promise.withResolvers<void>();
		vi.spyOn(serverStorage!.credentials, "addGeneratedApiKeyIfAbsent").mockImplementation(
			async (provider, apiKey, signal) => {
	...
	test("conditional generated-key upload stops without retrying when the caller aborts", async () => {
		...
		try {
			const insertion = clientStorage.credentials.addGeneratedApiKeyIfAbsent(
				"anysearch",
				"generated-key",
				controller.signal,
			);
