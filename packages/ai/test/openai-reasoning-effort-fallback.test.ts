...
describe("OpenAI reasoning effort fallback retry", () => {
	...
	it("remembers a rejected chat_template_kwargs.reasoning_effort for the rest of the session", async () => {
		const bodies: Record<string, unknown>[] = [];
		const fetchMock: FetchImpl = Object.assign(
			async (_input: string | URL | Request, init?: RequestInit): Promise<Response> => {
				const body = parseJsonBody(init);
		...
		expect(first.stopReason).toBe("stop");
		// One 400, one retry: the rejected kwarg is dropped and the effort
		// hoists onto the top-level field (that shape is pinned by the
		// kwargs-only rejection case below).
	...
	it("remaps a rejected kwargs effort value when the error lists allowed levels", async () => {
		...
		expect(result.stopReason).toBe("stop");
		expect(bodies).toHaveLength(2);
		// The field itself is accepted — only the value was rejected, so the
		// remap stays inside the kwargs object and nothing hoists top-level.
		expect(bodies[0]!.chat_template_kwargs).toEqual({
			preserve_thinking: true,
			enable_thinking: true,
