...
describe("disabled credential tombstone retention", () => {
	...
	it("preserves the deliberate cause for existing asynchronous provider-logout hooks", async () => {
		if (!store) throw new Error("test setup failed");
		const backing = store;
		const oldId = backing.upsertAuthCredentialForProvider("openai-codex", credential("old"))[0].id;
		backing.deleteAuthCredential(oldId, AUTOMATIC_CAUSE);
		const activeId = backing.upsertAuthCredentialForProvider("openai-codex", credential("active"))[0].id;
		const existingRemoteStore = Object.assign(backing, {
			async deleteAuthCredentialsRemote(provider: string, disabledCause: string): Promise<void> {
				backing.deleteAuthCredentialsForProvider(provider, disabledCause);
			},
		});
		const auth = new AuthStorage(existingRemoteStore);
		try {
			await auth.reload();
			await auth.remove("openai-codex");
			expect(readRows(dbPath)).toEqual([{ id: activeId, disabled_cause: "deleted by user" }]);
			expect(auth.listStoredCredentials("openai-codex")).toEqual([]);
		} finally {
			auth.close();
		}
	});
	...
	it.each([false, true])("keeps removal atomic when peer-disabled is %s", async peerDisabled => {
		if (!store) throw new Error("test setup failed");
		...
		if (peerDisabled) {
			...
			try {
				peer.deleteAuthCredential(newId, AUTOMATIC_CAUSE);
			} finally {
				peer.close();
			}
		}
		...
		try {
			...
			// Reject history cleanup after the target row has been disabled or deleted.
			...
			expect(readRows(dbPath)).toEqual(
				causesBefore
					.filter(row => row.id !== oldId && !(peerDisabled && row.id === newId))
	...
	it("preserves an unrelated identity-less tombstone on named-account removal until provider logout", async () => {
		...
		expect((await authStorage.listActionableDisabledCredentials("openai-codex")).map(row => row.id)).toEqual([oldId]);
	...
	it("removes a peer-disabled unidentified target without claiming other unidentified history", async () => {
		...
		const oldId = store.upsertAuthCredentialForProvider("openai-codex", bare("old"))[0].id;
		store.deleteAuthCredential(oldId, AUTOMATIC_CAUSE);
		const targetId = store.upsertAuthCredentialForProvider("openai-codex", bare("target"))[0].id;
		const stale = await SqliteAuthCredentialStore.open(dbPath);
		try {
			const storage = new AuthStorage(stale);
			await storage.reload();
			store.deleteAuthCredential(targetId, AUTOMATIC_CAUSE);
			const before = await store.listDisabledCredentials("openai-codex");

			expect(await storage.removeCredential("openai-codex", targetId)).toBe(true);
			expect(storage.exportSnapshot().credentials).toEqual([]);
			expect(await storage.listActionableDisabledCredentials("openai-codex")).toEqual(
				before.filter(row => row.id === oldId),
			);
			expect(readRows(dbPath)).toEqual([{ id: oldId, disabled_cause: AUTOMATIC_CAUSE }]);
		} finally {
			stale.close();
		}
	});

	it("preserves the original cause and disable time when a stale store client disables the same row", async () => {
		if (!store) throw new Error("test setup failed");
		const id = store.upsertAuthCredentialForProvider("openai-codex", credential("stale"))[0].id;
		const stale = await SqliteAuthCredentialStore.open(dbPath);
		const staleId = stale.listAuthCredentials("openai-codex")[0]!.id;
		try {
			store.deleteAuthCredential(id, AUTOMATIC_CAUSE);
			const db = new Database(dbPath);
			try {
				db.prepare("UPDATE auth_credentials SET updated_at = updated_at - 60 WHERE id = ?").run(id);
			} finally {
				db.close();
			}
			const [before] = await store.listDisabledCredentials("openai-codex");
			stale.deleteAuthCredential(staleId, "replaced by newer credential");
			const [after] = await store.listDisabledCredentials("openai-codex");
			expect(after?.cause).toBe(AUTOMATIC_CAUSE);
			expect(after?.disabledAtMs).toBe(before?.disabledAtMs);
		} finally {
			stale.close();
		}
	});
