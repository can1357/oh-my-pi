...
it("does not bypass a live cooldown probe in the blocked-candidate pass", async () => {
	const id = storage.listStoredCredentials(provider)[0]!.id;
	await storage.markUsageLimitReached(provider, undefined, { credentialId: id });
	expect(await storage.getApiKey(provider, "one", { requestId: "one" })).toBe("fixture-access");
	expect(await storage.getApiKey(provider, "two", { requestId: "two" })).toBeUndefined();
	storage.settleQuotaProbeSuccess("one");
	storage.releaseTurnReservation("one");
	expect(await storage.getApiKey(provider, "three", { requestId: "three" })).toBe("fixture-access");
});
