...
} from "@oh-my-pi/pi-ai/oauth/configured";
...
function config(fetchImpl: FetchImpl, client: Partial<ConfiguredOAuthClient> = {}): ConfiguredOAuthProvider {
	return {
...
describe("configured provider OAuth", () => {
	...
	it("keeps protocol parameters authoritative over custom params", async () => {
		...
		const provider = createConfiguredOAuthProvider(
			"custom",
			config(fetchMock, {
	...
	it("sends client credentials with HTTP Basic when configured", async () => {
		let authorization: string | null = null;
		let body = new URLSearchParams();
		const fetchMock = vi.fn(async (_input: string | URL | Request, init?: RequestInit) => {
			authorization = new Headers(init?.headers).get("authorization");
			body = new URLSearchParams(String(init?.body));
			return Response.json({ access_token: "access-new", token_type: "Bearer" });
		});
		...
		await provider.refreshToken({ access: "old", refresh: "refresh-old", expires: 1 });
		...
		expect(atob(encoded).split(":").map(decodeURIComponent)).toEqual(["client-id", "client-secret"]);
		expect(body.has("client_secret")).toBe(false);
	});

	it("fails before any token request when the client ID resolves empty", async () => {
		const fetchMock = vi.fn(async () => Response.json({ access_token: "never", token_type: "Bearer" }));
		const provider = createConfiguredOAuthProvider("custom", config(fetchMock, { clientId: "" }));
		await expect(provider.refreshToken({ access: "old", refresh: "refresh-old", expires: 1 })).rejects.toThrow(
			/oauth\.clientId could not be resolved/,
		);
		expect(fetchMock).not.toHaveBeenCalled();
	});

	it("rejects a missing secret for client_secret_basic before opening the browser", async () => {
		const fetchMock = vi.fn(async () => Response.json({ access_token: "never", token_type: "Bearer" }));
		const provider = createConfiguredOAuthProvider("custom", {
			...config(fetchMock, { clientSecret: undefined }),
			tokenEndpointAuthMethod: "client_secret_basic",
		});
		...
		).rejects.toThrow(/oauth\.clientSecret could not be resolved/);
		expect(authorizationOpened).toBe(false);
		expect(fetchMock).not.toHaveBeenCalled();
	});
});
