...
describe("auth-broker GET /metrics route", () => {
	...
	test("metrics are OFF unless the embedder opts in", async () => {
		// An embedder upgrading `@oh-my-pi/pi-ai/auth-broker` must not gain a
		// usage endpoint it never asked for. Defaulting on would expose the
		// exposition — which carries `email` labels — the moment the dependency
		// bumps, so the default is off and `/metrics` is genuinely absent.
		if (!storage) throw new Error("expected storage from beforeEach");
		const readyStorage = storage;
		const off = startAuthBroker({
			storage: readyStorage,
			bind: "127.0.0.1:0",
			bearerTokens: [MASTER],
			// No `metricsEnabled`, no `metricsTokens`: an untouched upgrade.
			disableRefresher: true,
		});
		try {
			const res = await fetch(`http://127.0.0.1:${off.port}/metrics`, {
	...
	test("refuses to boot when metrics are enabled with no token that can authorize them", () => {
		// `isAuthorized()` returns true for an EMPTY token set, so enabling
		// metrics with neither a scrape token nor a master bearer serves the
		// exposition — `email` labels included — to any unauthenticated caller.
		// A fail-open credential surface is a provisioning fault, not a default
		// to serve, so the broker refuses to start.
		if (!storage) throw new Error("expected storage from beforeEach");
		const readyStorage = storage;
		expect(() =>
			startAuthBroker({
