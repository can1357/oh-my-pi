...
describe("OpenAI compat policy", () => {
	...
	it("routes local Qwen3.8 effort selections onto the chat template (llama.cpp)", () => {
		...
		// llama-server reads per-request template controls out of
		// `chat_template_kwargs`, so the effort, the thinking toggle and the
		// history knob all ride that object and nothing goes top-level.
