...
export class AuthStorage {
	...
	async #adoptPoolAfterDisable(
		...
		blockedBeforeDisable?: ReadonlySet<number>,
	): Promise<boolean> {
		...
		const blockedBeforeReset = new Set([
			// The caller may have reset assignments already — a local conditional
			// disable does — so its own pre-mutation snapshot is the only record a
			// store without persisted blocks still has.
			...(blockedBeforeDisable ?? []),
	...
	#tryDisableCredentialAtIfMatches(
		...
		this.#emitCredentialDisabled(
			credentialDisabledEvent(provider, target.id, expectedCredential, disabledCause, this.#liveApiKeyIds(provider)),
		);
		return true;
	}
			...
			try {
				try {
					...
					if (options.isDefinitiveFailure?.(error)) {
						...
						if (disabled) {
							// Filter the snapshot in place rather than re-reading: candidate
							// preflights run concurrently on positional indices, so this path
							// must not await a store round-trip that could reorder the pool
							// under a sibling's in-flight index.
	...
	async rotateSessionCredential(
		...
		// suspect hook can't reindex the answer out from under us. The blocked set
		// is captured here for the same reason: the disable below resets this
		// provider's assignments, and a store that does not persist blocks keeps
		// them only in memory, so reading them afterwards finds an empty map.
		const blockedBeforeDisable = new Set(
			this.#getStoredCredentials(provider)
				.filter((_row, index) => this.#isCredentialBlocked(provider, providerKey, index))
				.map(row => row.id),
		);
		...
		if (retireOnHard401 || AIError.isInvalidatedOAuthTokenError(error)) {
			...
			if (deleted && this.#store.deleteAuthCredentialRemote) {
				...
				// observed the invalidation and announces it to its own subscribers.
				this.#loadStoredCredentials(provider);
				this.#emitCredentialDisabled(
					credentialDisabledEvent(
						provider,
