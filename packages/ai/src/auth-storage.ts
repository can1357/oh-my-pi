...
export class AuthStorage {
	...
	async login(
		...
		if (typeof result === "string") {
			...
			// Preserve rotation keys only for providers that opt into multi-key
			// login. Single-key services retain authoritative replacement semantics.
			const append = "apiKeyLoginMode" in def && def.apiKeyLoginMode === "append";
			const stored = append
