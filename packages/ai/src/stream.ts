...
function streamDispatch<TApi extends Api>(
	...
	// Providers whose transport resolves its own credentials for this API (e.g.
	// amazon-bedrock Chat via SigV4) prepare the request without a stored key;
	// other APIs keep the generic MissingApiKeyError policy.
...
function streamSimpleRequest<TApi extends Api>(
		...
		void (async () => {
			...
			if (lastKey === undefined) {
				...
				if (providerDefinition?.allowsMissingApiKey || providerDefinition?.nativeAuthApis?.includes(model.api)) {
					// No stored key, but this API's transport resolves its own
					// credentials in streamDispatch's prepareRequest (e.g.
					// amazon-bedrock Chat via SigV4).
	...
	if (model.api === "google-vertex") {
		...
		// The API's transport resolves credentials in prepareRequest (e.g.
		// amazon-bedrock Chat via SigV4); a resolved string key or env value is
		// optional and, when present, still flows through the preparer.
		const staticApiKey =
