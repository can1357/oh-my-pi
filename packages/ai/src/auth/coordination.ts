...
export class CredentialCoordination {
	...
	/** Actual quota block scopes live for this credential, excluding unrelated in-flight reservations. */
	activeQuotaBlockScopes(providerKey: string, credentialId: number, scopes: readonly string[] = []): string[] {
		return this.#deps.blocks.activeScopes(providerKey, credentialId, scopes);
	}
	...
	tryProbeApiKeySelection(
		...
		const active = this.activeQuotaBlockScopes(providerKey, credentialId, requestedScopes);
		if (active.length === 0 || active.some(scope => this.isRetryAfterSourced(credentialId, scope))) return false;
		if (!this.acquireOrReuseQuotaProbeLease(requestId, credentialId, active[0]!)) return false;
