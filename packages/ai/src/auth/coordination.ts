...
export class CredentialCoordination {
	...
	/** Record an already-acquired probe lease under `key` (requestId or anonymous key). */
	recordQuotaProbe(key: string, credentialId: number, blockScope: string, leaseId: string): void {
		this.#inflightProbes.set(key, { credentialId, blockScope, leaseId });
	}

	/**
	 * Drop the inflight probe under `key` only when it still holds `leaseId`. A
	 * concurrent re-acquire under the same key must not be clobbered.
	 */
	clearQuotaProbeIfLease(key: string, leaseId: string): void {
		const probe = this.#inflightProbes.get(key);
		if (!probe || probe.leaseId !== leaseId) return;
