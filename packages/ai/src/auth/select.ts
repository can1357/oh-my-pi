...
export class CredentialSelector {
	...
	async selectApiKey(
		...
		if (credentials.length === 1) {
			...
			return this.#deps.coordination.tryReserveApiKeySelection(provider, only, options?.requestId)
				? only
				: undefined;
		}
		...
		if (!strategy) {
			for (const idx of order) {
				...
				if (
					...
					this.#deps.coordination.tryReserveApiKeySelection(provider, candidate, options?.requestId)
				) {
					return candidate;
				}
			}
	...
	async tryOAuth(
		...
		if (
			...
			const lease = coordination.tryAcquireQuotaProbeLease(blockedId, probeScope);
			if (!lease) return undefined;
			const probeRequestKey = options?.requestId ?? anonymousProbeRequestKey(blockedId, probeScope);
			if (!options?.requestId)
				anonymousProbe = { credentialId: blockedId, blockScope: probeScope, leaseId: lease };
			coordination.recordQuotaProbe(probeRequestKey, blockedId, probeScope, lease);
		}
		if (options?.requestId) {
		...
		try {
			...
			// Prepare/broker refresh may bump incarnation and purge the reservation;
			// re-acquire against the post-prepare incarnation before vending the bearer.
			...
			if (!keepReservation) {
				if (options?.requestId) {
					...
					coordination.clearQuotaProbeIfLease(key, anonymousProbe.leaseId);
				}
			}
		}
	}
