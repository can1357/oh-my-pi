...
export interface AuthBrokerServerOptions {
	...
	/**
	 * Whether to register `GET /metrics` at all. Defaults to `false`: this is a
	 * library, and a dependency bump must not hand an embedder a usage endpoint
	 * it never asked for — the exposition carries `email` labels. When absent the
	 * route 404s like any unknown path.
	 *
	 * Gating on `metricsTokens` alone would not close it: master `bearerTokens`
	 * are folded into the accepted set below, so an operator who never asked for
	 * the endpoint would still expose usage data to any master-bearer holder.
	 *
	 * Enabling it requires a token that can authorize it — `metricsTokens` or
	 * `bearerTokens`. An empty token set is treated as open by `isAuthorized()`,
	 * so that combination refuses to boot rather than serving unauthenticated.
	 */
...
export function startAuthBroker(opts: AuthBrokerServerOptions): AuthBrokerServerHandle {
	...
	// OFF unless the embedder opts in. `@oh-my-pi/pi-ai/auth-broker` is a
	// library: defaulting on would hand every embedder a usage endpoint — one
	// exporting `email` labels — the moment they bump the dependency, which is
	// not a capability a version bump should confer.
	...
	if (metricsEnabled) {
		...
		// `isAuthorized()` treats an EMPTY token set as open, so enabling metrics
		// with neither a scrape token nor a master bearer would serve the
		// exposition — `email` labels included — to any unauthenticated caller.
		// The other routes fail open the same way, but they carry no data until a
		// credential is stored, whereas this one renders usage immediately. A
		// fail-open credential surface is a provisioning fault, so refuse to boot
		// rather than serve it.
		if (scrapeTokens.length === 0 && bearerTokens.length === 0) {
			throw new Error(
				"auth-broker: /metrics is enabled with no token that can authorize it (set metricsTokens or bearerTokens; an empty token set would serve the exposition unauthenticated)",
			);
		}
	}
