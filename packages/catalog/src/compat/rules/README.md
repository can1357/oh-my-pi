...
Only `grokbot-sand` seed rows may declare `sand-parameter-ids` with zero or more distinct non-empty string ids and `sand-max-mode` with one boolean. Both fields compile verbatim onto the `ModelSpec`; a bare `sand-parameter-ids` node explicitly declares that the model accepts no requested-model parameters.
