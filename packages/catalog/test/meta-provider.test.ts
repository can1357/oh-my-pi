...
describe("Meta Model API provider", () => {
	...
	test("unions model rosters discovered with every Meta account key", async () => {
		...
		const options = metaModelManagerOptions({
			...
			fetch: (_input, init) => {
				...
				return Promise.resolve(
					modelListResponse(
						authorization === "Bearer LLM|catalog-a"
							? ["muse-spark-1.3", "muse-image-1.0"]
							: ["muse-spark-1.3-contributor"],
					),
				);
			},
		...
		expect(authorizations.sort()).toEqual(["Bearer LLM|catalog-a", "Bearer LLM|catalog-b"]);
