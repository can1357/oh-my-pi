...
import type { RepositoryProfile } from "./repository-intelligence";
...
export function buildVerificationPlan(input: VerificationPolicyInput): VerificationPlan {
	...
	for (const packagePath of packages) {
		const scripts = input.packageScripts?.[packagePath] ?? input.repositoryProfile?.workspacePackages.find(item => item.root === packagePath)?.scripts ?? {};
		if (codeChanged || testsChanged) {
			const typecheck = check("check:types", scripts["check:types"], "Affected workspace has code/test changes", 10, "cheap", "typecheck", packagePath);
			const tests = check("test", scripts.test, "Affected workspace has source or test changes", 20, "moderate", "test", packagePath);
			const lint = check("lint", scripts.lint, "Affected workspace exposes deterministic linting", 30, "moderate", "lint", packagePath);
	...
	if (!checks.length && codeChanged && root["check:ts"]) checks.push({ name: "check:ts", command: "bun", args: ["run", "check:ts"], reason: "No affected package check discovered", priority: 10, cost: "cheap", dependencies: [], kind: "typecheck", broad: true });
...
export function buildRepairMessage(task: string, failure: VerificationFailure, attempts: readonly string[], nextCheck: VerificationCheck | undefined, complexity: TaskComplexity, repositoryProfile?: RepositoryProfile): string {
	const owner = failure.relatedFiles[0] ? repositoryProfile?.workspacePackages.find(workspace => failure.relatedFiles[0].startsWith(`${workspace.root}/`)) : undefined;
	return [
		...
		`Workspace: ${owner?.name ?? owner?.root ?? "unknown"}`,
...
export class VerificationRecoveryController {
	...
	decide(failure: VerificationFailure, workspaceSignature: string, tracker: TaskRouteTracker): VerificationRecoveryDecision {
		...
		const same = (this.counts.get(signature) ?? 0) + 1;
		this.counts.set(signature, same);
		const changedWorkspace = this.lastWorkspace !== undefined && this.lastWorkspace !== workspaceSignature;
		this.lastWorkspace = workspaceSignature;
...
const profileProviders = new Map<string, RepositoryProfile>();
export function setRepositoryProfileForVerification(cwd: string, profile: RepositoryProfile | undefined): () => void {
	const key = path.resolve(cwd);
	if (!profile) return () => {};
	profileProviders.set(key, profile);
	return () => profileProviders.delete(key);
}
...
export async function readWorkspacePackageScripts(cwd: string): Promise<{ rootScripts: Record<string, string>; packageScripts: Record<string, Record<string, string>> }> {
	const cached = profileProviders.get(path.resolve(cwd));
	if (cached) {
		const rootWorkspace = cached.workspacePackages.find(item => item.root === "." || item.manifest === "package.json");
		const packageScripts: Record<string, Record<string, string>> = {};
		for (const workspace of cached.workspacePackages) if (workspace.root !== ".") packageScripts[workspace.root] = { ...workspace.scripts };
		return { rootScripts: rootWorkspace?.scripts ?? {}, packageScripts };
	}
	...
	try {
		...
		rootScripts = Object.fromEntries(Object.entries(root.scripts ?? {}).filter(([, v]) => typeof v === "string")) as Record<string, string>;
	} catch {}
	try {
		for (const entry of await fs.readdir(path.join(cwd, "packages"), { withFileTypes: true })) {
			...
			try {
				...
				packageScripts[`packages/${entry.name}`] = Object.fromEntries(Object.entries(pkg.scripts ?? {}).filter(([, v]) => typeof v === "string")) as Record<string, string>;
			} catch {}
		}
	} catch {}
