...
- Tool results carrying a typed `outcome` now project that verdict onto the legacy `isError` flag at the result-normalization boundary, so `afterToolCall` hooks, `tool_execution_end` consumers, the execute-tool span, and replayed history can no longer report success for a call whose outcome is failed or interrupted. Producers without an `outcome` are unaffected.
