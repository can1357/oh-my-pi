...
- Added an optional `statusLine.showUnreportedSubagentCost` breakdown for child-agent spend omitted from session cost; the additive `(agents)` amount never overlaps session cost.
