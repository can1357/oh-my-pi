...
- Directory scans no longer conflate unreadable directories with empty ones: advisor transcript costs, memory consolidation, Claude/Codex session imports, and agent discovery now record a diagnostic log warning on real `readdir` failures (`EACCES`/`ENOTDIR`/…) while a missing directory stays silently absent (log output only; no user-visible change). Memory directory pruning additionally skips deletion when a child scan fails instead of treating the failure as empty ([#11554](https://github.com/can1357/oh-my-pi/pull/11554) by [@nikkoxgonzales](https://github.com/nikkoxgonzales)).
