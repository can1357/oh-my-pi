...
- A server stop that could not pin the server's helper subtree now fails instead of sweeping whatever part of it was visible and reporting success — whether the walk came back short, no reference to the server root could be opened at all, or the root was pinned alive and then exited before its subtree could be walked. In all three the helpers it may have left are past reach rather than absent: a walk rooted at an exited pid enumerates nothing and cannot tell that apart from a server that had no helpers. The graceful handshake still runs either way, so the server keeps its chance to exit cleanly.
