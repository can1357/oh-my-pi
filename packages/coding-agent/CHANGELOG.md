...
- Added `omp config schema --json`, printing every setting's key, type, default, tab/group, label, description, options, and a declarative visibility condition.
- Added `omp config unset <key>`, removing a setting's explicit value from the global config so it tracks the schema default.
