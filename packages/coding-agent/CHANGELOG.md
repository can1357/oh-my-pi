...
- Closing a terminal whose draft was resumed and turned into a real conversation by another terminal no longer deletes that conversation: the close-time draft GC re-reads the session file, re-checks the draft sidecar, and deletes only under the session-file cross-process lock, which the first durable append, the first durable atomic batch publish, a draft save, and a rename now share. A draft save that cannot take the lock fails instead of writing unlocked, a late draft veto preserves the cleanup marker, a rename appends its title change instead of republishing a stale body, a surviving `.draft-only-session` marker keeps a resumed terminal's first durable write on that lock even after another terminal consumed its draft sidecar, and the GC decides from a bounded prefix instead of loading a transcript it is only going to keep ([#11497](https://github.com/can1357/oh-my-pi/issues/11497)).
