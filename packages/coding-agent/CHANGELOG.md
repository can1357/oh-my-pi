...
- `/refresh settings` now reconciles the live subsystems it reloads instead of reporting success while the running session keeps its launch-time behavior: the memory backend transitions (disposing the old backend's state and replacing its tools), advisors start or stop, the `think` scratchpad follows `externalThinking`, the agent's tool dialect follows `tools.format`, `providers.openaiWebsockets`/`tools.abortOnFabricatedResult` reach the request loop, a reloaded `tier.openai`/`tier.anthropic`/`tier.google` reaches the per-request service-tier resolver, a `generate_image.enabled`/`speechgen.enabled` edit installs or removes those tools on the live registry rather than only at startup, and an `async.enabled` edit reaches the existing `bash` tool so enabling it accepts background execution and disabling it stops advertising and refuses `async`. A per-family tier or tool set the session itself selected at runtime is left alone when its configured value did not move. A `browser.enabled`/`computer.enabled` edit is now awaited as part of the refresh rather than left racing in the background, and a reloaded `providers.kimiApiFormat` reaches auto-learn captures created afterward instead of pinning them to the launch-time wire format. Automatic web search also stops skipping Exa for a session holding its own MCP-discovered key once the process-global `EXA_API_KEY` goes away: the key now counts toward provider availability, not just the request itself. A reloaded `workspace.additionalDirectories` and `async.maxJobs` now reach the objects that own them — the workspace roots the session grants tool access to and advertises in the prompt, and the running background-job limit — instead of updating only the merged value while the live session keeps its launch-time roots and cap; a root removed from `workspace.additionalDirectories` is now actually revoked rather than kept granted forever (the reconcile tracks which roots the setting owns, so session-header and `/add-dir` roots still survive), a reloaded `snapcompact.systemPrompt`/`snapcompact.toolResults`/`snapcompact.shape` reaches the transformer the request path holds — enabling starts rendering, disabling stops it, and a shape change re-rasterizes instead of serving frames in the retired variant — and an automatic retry-fallback thinking swap no longer records itself as a user pin, so a session following `defaultThinkingLevel` still follows it after a fallback. Refreshed workspace roots also resolve relative entries against the session's current directory rather than its launch-time one, so a configured root still lands in the right project after a `/move`, and the refresh now waits for that reconciliation to persist instead of reporting success while it is still in flight; `tools.intentTracing` reaches the live agent too, instead of being treated as frozen until restart. An explicitly pinned thinking level also survives a settings-driven model swap onto a model whose own `thinking.defaultLevel` differs, instead of being overwritten by the swap's re-apply before the follows-settings guard could protect it; and a subagent's settings refresh no longer republishes its child-scoped rule set as the process-global snapshot. A reloaded `mcp.enableProjectConfig` now also disconnects the project servers this session already started, instead of leaving their tools callable until a restart, and the documented `onBeforeRefresh` pre-refresh hook is reachable from `createAgentSession` rather than silently ignored. A session with no Exa key of its own no longer counts a peer-injected `EXA_API_KEY` as its own credential when choosing a search provider or authenticating, and an MCP server hidden by a suppressed project entry is disconnected. A workspace root removed from config while the session was stopped is revoked on resume, an explicit service-tier selection stays pinned even when it equals the configured tier, and the auto-learn guidance moves with the tool it describes, the image URL service is rebuilt when its settings move, the Ctrl+P model scope follows `enabledModels`, auto-learn captures use the tools the session currently has, and a descendant spawned with an explicit `skills` policy is no longer widened by its parent's refresh, skill discovery warnings are refreshed with the roster they describe, and the depth-dependent tool gates are re-evaluated at a subagent's real spawn depth. Every settings-gated core built-in is reconciled — `todo`, `task`/`hub`, the checkpoint pair, `lsp`, and the auto-learn tools plus their controller — so a refreshed session matches a freshly started one. The `inlineToolDescriptors`/`task.eager` prompt inputs are reconciled too, and a turn starting while a parent's refresh is still rebuilding this session's prompt now waits for it. A memory backend's injection token limit is treated as prompt text, so editing it resizes the memory block. The `lsp.enabled` tool gate is reconciled too, and disabling a built-in's gate no longer deactivates a custom tool registered over its name. A session that discovered no Exa key of its own no longer authenticates with a key another session injected into the environment. Re-enabling a core built-in such as `bash` now builds it, scoped to the tools the session was started with, and a default-model edit made while the session was stopped is reconciled on resume. A `service_tier_change` receipt now records which families still follow `tier.*`, so a `/fast` pin on one family no longer freezes the others — a `tier.*` edit made while the session was stopped survives the resume. The `mcp.notifications` and `async.maxJobs` reconciles are gated on owning the manager they mutate, so a subagent cannot rewrite its parent's subscriptions or background-job limit. A reloaded `lsp.shared` is reapplied, so language servers cold-started after the refresh follow the refreshed shared/private choice, and Exa credentials a project-config flip reveals or hides are applied instead of discarded. Core built-ins gated on a plain boolean setting (`bash.enabled`, `glob`, `grep`, `debug`, `web_search`, and the rest) are now reconciled too, so disabling one stops advertising and stops accepting calls without a restart. A `/refresh` error now shortens a Windows or UNC configuration path as well as a POSIX one, and an auto-learn capture created after the refresh follows the live `tools.intentTracing` policy instead of the launch-time one. The project opt-out reconcile is gated on owning the MCP manager, so a subagent that inherited its parent's cannot disconnect the parent's servers from its own settings scope, and it now converges the running set in both directions — connecting servers the opt-out reveals and replacing one whose winning source changed — a project entry shadowing a same-named user entry previously left the session with neither. A `bash.autoBackground.enabled` edit now also rebuilds the system prompt, so under `inlineToolDescriptors` the model stops being told long calls never auto-background (or still offered it) after the policy flips; and an ephemeral model handoff such as prewalk or plan-yolo carries the session's pre-handoff thinking provenance, so a handoff whose configured level differs from the active one is no longer recorded as a user pin that makes later `defaultThinkingLevel` edits ineffective; the image-URL fallback resolves its blob broker per request, so enabling image serving at runtime takes effect and a reloaded session recovers through the live broker rather than a retired one; and the checkpoint/rewind pair is kept whole regardless of the startup gate, so enabling `checkpoint.enabled` later no longer leaves a session able to checkpoint but not rewind, and the `read`/`write` tools read their own settings live so `read.defaultLimit`, `images.autoResize`, `lsp.formatOnWrite`, `lsp.diagnosticsOnWrite`, and `lsp.diagnosticsDeduplicate` take effect without a restart ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `/refresh settings` reporting `settings unchanged` after editing a project settings layer such as `.claude/settings.json`, `.cursor/settings.json`, or `.omp/config.yml`; those layers load through the process-lifetime capability cache, which is now invalidated for a settings-only refresh ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `/refresh skills` printing a rules result and `/refresh rules` printing a skills result; the coupled discovery still rescans both surfaces, but only the requested scope's count is reported ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed one top-level SDK/ACP session replacing or deleting another session's injected `EXA_API_KEY`; the process-global injection now records which session owns it, so a peer's rotated or absent config key cannot overwrite it and an operator-supplied value still always wins. Each session also resolves its OWN discovered key, so the later session no longer authenticates with the earlier one's credential — but only ahead of an `EXA_API_KEY` the harness itself injected, so an operator's exported value keeps its documented precedence over a key discovered from MCP config ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- `/refresh settings` now moves the reloaded `providers.kimiApiFormat` onto the live agent, so switching between `openai`, `anthropic`, and `auto` takes effect instead of leaving later Kimi calls on the launch-time wire protocol ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `providers.openaiWebsockets` reaching only the primary agent on `/refresh settings`, leaving side-channel requests (`/btw`), compaction, and advisors on the construction-time transport; the session-level preference and the auxiliary hosts now follow the reloaded value ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `providers.webSearchOrder`, `providers.webSearchExclude`, and `providers.imageOrder` updating only the `Settings` view on `/refresh settings`; the search and image implementations read module-level state, so the centralized provider-globals helper is reapplied when one of these moves and searches no longer keep the old order or exclusions ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed enabling `secrets.enabled` on disk leaving the construction-time obfuscator in place, so arbitrary secrets configured in `secrets.yml` kept being sent to providers while the refresh reported the privacy setting updated; the obfuscator is now rebuilt on refresh and the consumers reading it (final LLM conversion, provider-context transform, tool-argument deobfuscation) observe the rebuild. The rebuild reads the session's current directory like the rest of refresh, so after `/move` or a cross-project resume it loads the destination project's `secrets.yml` instead of leaving those secrets unobfuscated while still applying the original project's substitutions. The system prompt follows the rebuild too, so a session that starts minting `$$HASH$$` placeholders no longer omits the instruction that they are intentional opaque values ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- `/refresh settings` now joins the Code Mode and `extendedContext` reconciliations instead of letting them run fire-and-forget, so a refresh cannot report completion — nor the next turn send a request or compute compaction limits — before the Code Mode tool partition and prompt are repartitioned and the live model carries its new context window ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `/refresh settings` skipping the `secrets.yml` re-read whenever no merged setting changed, so a secret added to the file alone kept being sent to providers unobfuscated until an unrelated setting moved or the session restarted; the obfuscator now rebuilds whenever secrets are enabled ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a subagent's own `/refresh rules` rolling its rules back to the launch-time contents, undoing a newer roster a parent refresh had already installed; a child that inherited its parent's roster now re-buckets the live one, while an explicit SDK `rules`/`--no-rules` restriction still wins ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a settings reload on a read-only or in-memory instance updating the merged view without signalling Code Mode, so a `codeMode`, `codeModeDirectTools`, `eval.js`, or `edit.mode` change in a reloaded project or overlay file left the live tool partition stale ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
...
- Fixed `/refresh settings` overwriting an explicit thinking-level selection that happened to match the level already active; because the selection changed no effective effort it recorded no pin, so a later configured-default move replaced the user's choice. Selecting only a MODEL no longer pins thinking either: when the newly selected model carried its own default thinking level, applying it was recorded as an explicit thinking choice, so a later `defaultThinkingLevel` edit plus `/refresh settings` could no longer move the level even though the user had chosen only a model. A per-family service tier that `/refresh settings` updates now also survives switching sessions away and back, or restarting and resuming, instead of reverting to its pre-refresh value once another family had been set with `/fast` or the settings selector. A resumed session whose last model pick came from an older build's model cycling — which recorded no role on the transition — likewise no longer has that pick replaced by the configured default: a role-less transition written after the session's first message is read as the user's, while one preceding every message is the startup receipt and still follows settings. `/refresh` also sanitizes an invalid scope argument before echoing it, so a pasted or ACP-supplied value carrying ANSI escapes, control bytes, or an oversized payload can no longer corrupt the terminal ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed enabling a memory backend (`memory.backend` moving `off` → `hindsight`/`mnemopi`) forcing its tools active even when the user had already narrowed the active set through `/tools`; a backend's tools now become available on enable without overriding an explicit session selection ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a session that enabled image URLs mid-run through `/refresh settings` leaking its blob broker on disposal: the broker's callback server and local exposure/tunnel outlived the session and accumulated across sessions, because the reload disposed only a retired broker and disposal never stopped the active one. The active broker is now disposed with the session ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed resuming a session with no prior service-tier receipt and only `--openai-service-tier` set freezing the Anthropic and Google tiers alongside the intentional OpenAI pin, so a later `tier.anthropic`/`tier.google` edit plus `/refresh settings` could not move them; the unrecorded families now keep following settings, exactly as a fresh session does ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a resumed session whose recorded project directory is gone (running workspace-directory edits as runtime-only) keeping a settings-owned root marked as owned after it was removed, so a settings refresh that dropped the configured root would later revoke a directory the user had re-added manually with `/add-dir`; the in-memory ownership is now cleared on removal even on that fallback path ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `/refresh` applying an external `enabledProviders`/`disabledProviders` edit to the settings view only, so capability discovery kept loading a newly disabled provider's skills and rules (and omitting a newly enabled user source) while the refresh reported the settings updated; the module-level provider gates are now re-synced from the reloaded settings before rediscovery ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a settings-owned workspace root that was removed with `/remove-dir` and re-added with `/add-dir` still being treated as settings-owned, so a later `/refresh settings` that dropped the configured root revoked the manually re-added directory; the live reconcile now reads the session's current root ownership instead of a stale startup snapshot ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a transient MCP server or tool outage silently re-enabling an MCP tool the user had turned off through `/tools`: once an intermediate `/refresh` dropped the tool, its later reconnect was treated as newly connected and reactivated; the deselection is now remembered across the outage, while a genuinely new tool and a re-selected one still activate ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `/refresh settings` reloading `compaction.idleEnabled`/`idleThresholdTokens`/`idleTimeoutSeconds` on an idle interactive session without re-arming the idle-compaction timer, so it could compact after being disabled or keep the old threshold/timeout (and enabling from off armed nothing) until another turn ended; the timer is now re-armed on the reload exactly as an in-app settings change does ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a `/refresh` that is disposed while it awaits (in an async host pre-refresh hook, or during the skill/rule disk re-scan itself) still overwriting the process-wide active skill/rule snapshot, so a torn-down session could clobber the roster another running session had already published; the reload now rechecks disposal at the publication point and abandons the global swap ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed `/refresh settings` turning `externalThinking` off leaving the built-in `think` scratchpad's registry entry behind, so a Pi extension that later registered its own `think` replacement stayed inactive — where a session started with `externalThinking` off activates it; the disabled entry is now removed so late registration behaves like a fresh session ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
