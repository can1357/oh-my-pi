...
- Fixed vision requests failing with a request-size (413) error after a long session: outgoing image payloads are now bounded by a per-provider image-byte budget in addition to the existing image-count cap, dropping the oldest images so the combined base64 fits. Both budgets charge only the images a request actually carries: an assistant-generated display image is stripped before the request is built, so it no longer evicts live user or tool images to make room for artifacts that were never going to be sent ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a vision request losing every image when a corrupt one was newest: the per-provider image-byte budget is now applied after undecodable images are dropped, so a readable image is no longer evicted to make room for bytes that are discarded. The budget is also charged after Anthropic's own many-image downscale rather than before it, so a request whose resized payload fits no longer drops its oldest images on their full-size bytes — and clamping can no longer cut the image count below the threshold that triggers that downscale ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed the image-byte budget being bypassed on the blob broker's URL-to-inline recovery: materializing lazy frames back to inline data turns bytes the budget could not charge while they were URLs into a payload that skips the transform pipeline entirely, so a recovery request built from many frames could still hit the request-size (413) error it was retrying to escape. The budget is now re-applied after materialization, and charged after the provider's own image downscale rather than before it. Replayed native image-generation results are charged too: those live on the assistant's provider payload rather than in its content, so a few generated images could exceed the budget while the content-only tally read zero; they are evicted oldest-first by clearing the replayed result, which drops that item from replay while the rest of the payload survives ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Long vision sessions no longer fully decode images the per-request image-count cap immediately discards: the count cap now runs ahead of the normalize and undecodable-image passes, so a history past the cap (or past the 512-entry decode cache) stops re-decoding its whole archive on every request. That early pass keeps a bounded allowance above the cap rather than clamping straight to it, so a corrupt newer image no longer costs an older readable image its slot before the undecodable pass has replaced the corrupt one with text — a full cap's worth of readable images survives, while decode work stays bounded by a constant multiple of the cap instead of by the length of the history ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed an oversized vision image being restored to its over-cap original when a quality-ladder re-encode failed: a failed rung now only discards that candidate, so the request keeps the already-completed within-cap resize instead of one Anthropic rejects outright ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed the first request after resuming an OpenAI Responses session evicting a live image to make room for replayed generation results it was not going to send: the image-byte budget now charges a native payload only when the request will actually replay it ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed a computer tool result losing its text when its screenshot was dropped for the image-byte budget: on an OpenAI Responses model the eviction replaced the whole result with an image-omitted placeholder, so the tool's textual output vanished from the request; the text is now kept and only the image is removed ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
