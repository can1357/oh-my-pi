...
- A collab guest that rejoins repeatedly can no longer grow the host's memory without limit: a welcome snapshot waiting on a slow connection is now charged against the host's send-queue budget instead of counting as nothing, so the limit covers what a queued welcome is holding and not only what has already been serialized. It counts the size the sender declares — the snapshot's serialized bytes after image stripping — which tracks that memory rather than measuring it.
- A single collab guest can no longer end sharing for the room: the host's send queue now bounds each guest's share, discards the backlog of a guest that outruns it and tells that guest to rejoin, and reserves ending the session for a backlog the host itself generated. Sessions larger than that queue's whole budget are shareable again: the welcome ships with the snapshot built for it, the notice the same join emits is droppable rather than fatal, and live updates arriving during the join queue behind that snapshot instead of evicting it — so a guest joining a large session mid-turn is no longer dropped on the first update and told to rejoin into the same failure. A join that cannot be admitted even with the queue cleared no longer discards another guest's snapshot on its way to failing.
- Collab guests joining a busy session are no longer locked out while others hold the send queue, and a guest repeating `hello` no longer queues a snapshot per attempt or receives one paired with another snapshot's metadata.
- A collab guest can no longer end sharing by renaming itself on every `hello`: footer and roster updates are re-sent on the next change, so a saturated queue discards one instead of terminating the session. Nor can a guest evict well-behaved participants' backlogs by spamming requests — the room the host keeps for a new guest's welcome no longer applies to ordinary replies.
- A host prompt that asks collab guests a question no longer hangs when nobody can answer it: an ask the send queue could not deliver to anybody, and an ask whose last recipient was dropped for falling behind or gave up write permission, both resolve as unavailable; an ask whose last recipient left waits for the next writer instead of waiting for an answer that cannot come. A transcript reply is also no longer sent to a guest id whose owner left, or was reissued to someone else, while the transcript was being read, and a prompt or agent command that fails long after its guest is gone no longer costs whoever holds that id now their own replication.
