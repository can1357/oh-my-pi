...
- Not-found errors from `read`, `grep`, `glob`, `bash`, `ast_grep`, and `ast_edit` now append a hint listing the nearest existing directory's contents, so a mistyped path can be corrected in one step ([#13134](https://github.com/can1357/oh-my-pi/pull/13134) by [@schickling-assistant](https://github.com/schickling-assistant)).
