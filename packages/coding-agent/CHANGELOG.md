...
- Fixed `/shake` surfacing a failed history rewrite as an unhandled rejection instead of reporting it on the command; neither slash-command dispatcher wraps the handler, so the error escaped the invocation that caused it.
- `restart` now refuses as busy while `compact()`, `handoff()`, or a manual history rewrite (`shake()`/`dropImages()`) is still running, so a recycle cannot abort a context-maintenance pass an SDK caller is awaiting. A history rewrite persists only through an awaited `rewriteEntries()`, which the recycle's seal turned into a silent no-op — the caller still read a successful reduction while the replacement reopened the unchanged transcript. In the reverse order a rewrite requested after the restart has latched, or after a host teardown has begun, is now rejected rather than admitted and dropped ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- `restart` now refuses as busy while a session transition (`newSession`/`fork`/`moveSession`/`switchSession`/`branch`) that started first is still parked in an awaited `session_before_switch`/`session_before_branch` hook. Previously the recycle's coherence check compared against a session file the transition had not renamed yet, so the host could be handed a path the resuming transition swapped out from under the replacement ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- `restart` now verifies the captured session file still exists on disk before handing it to `onRestartRequested`, refusing with `no-session-file` instead of giving the host a reattachment handle that `SessionManager.open()` cannot reopen ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- `restart` now waits for an in-flight extension or custom slash-command handler instead of disposing under it, so a handler still awaiting I/O no longer keeps using a torn-down extension/session runtime past the durability barrier. A handler that requests the restart and awaits it is excluded from the wait it owns, which previously latched the session permanently — including when it awaits a restart another caller already started, where the coalesced promise was returned without releasing the handler's window and the two waited on each other forever. Every other in-flight prompt still blocks the recycle, and that handler's window is retired once, so a detached timer or floating promise it left behind can no longer release the window a second time and let the recycle dispose beneath an unrelated prompt. A queue-only custom message dispatched once disposal has begun is now refused rather than accepted into a queue the teardown clears, which returned a promise of a turn that could never run ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` recycle now parks the parent's adopted subagents rather than running the process-level agent-lifecycle shutdown, so idle subagents stay addressable and revivable after the replacement session attaches. A parked subagent is now rebuilt through the replacement parent's persisted-subagent reviver rather than the one captured at spawn time, which closed over the recycled parent's MCP manager and other spawn-time dependencies that its own teardown had disconnected; with no reviver installed the revival is refused, leaving the transcript readable and the agent parked for a later attempt, rather than returning a session on those disconnected resources. Idle Vibe worker sessions, whose process-local records the recycle's teardown detaches, are likewise restored from the reopened transcript once the replacement has attached, so `vibe_list` no longer comes back empty with existing workers unreachable ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Documented that a restart reconstruction must omit `model` and `modelPattern` (or re-pass the session's *active* model), since any supplied model or nonempty pattern is treated as explicit and suppresses restoring the user's current selection from the transcript ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A model change (`setModel`/`setModelTemporary`/the cycle methods) invoked after a `restart` has latched, or after a host teardown has begun, is now refused in that method's own no-op shape instead of being admitted. Previously such a call parked in its awaited metadata probe, resumed against the sealed `SessionManager`, and reported a successful switch whose `model_change` append was dropped — leaving the replacement session on the previous model ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` after `/fresh` no longer resumes the provider session the user rotated away from. The restart handoff now carries the session's active provider-facing identity alongside the durable id, so a reconstruction that passes it through as `providerSessionId` keeps the rotated identity while still reattaching the transcript off the durable id. Previously the handoff exposed only the durable id, so the replacement fell back to it and silently undid the `/fresh` rotation ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` recycle's all-agent subagent parking is now exclusive with revival: a concurrent hub `send` waits the barrier out instead of cancelling the park it would otherwise win, and a revival already in flight is settled — under the same deadline as the parking itself — rather than skipped. Previously the parking could report completion while a subagent stayed live or was still being rebuilt, and the parent's teardown then disposed the shared resources under it; a revival wedged in its persisted factory or session creation wedged the recycle permanently, since the parent had already stopped accepting work and the parking phase that triggers the replacement was never reached ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` recycle now keeps subagent revival blocked for the parent's whole teardown, not just its subagent parking. Previously a hub `send` waiting to revive was released as soon as parking finished, while the shared kernels, MCP, and LSP resources that child borrows were disposed on the following step — so the revived subagent came back on top of resources that were already going away. The barrier is now held until the host's reattachment callback returns, and a revival already in flight that outlives the parking deadline is failed rather than allowed to attach a live child behind the recycle — the deadline still bounds the parking itself, so a wedged reviver cannot stall the restart. Overlapping recycles compose rather than replacing one another: when two callback-enabled sessions recycle at once, revival stays blocked until every handoff has released, so a second one finishing first no longer reopens revival while the first parent is still tearing down. A handoff that ends without a replacement parent is now recorded on the manager instead of on the barrier it raised, so its refusal survives the barrier's own release: a waiter that never held that barrier — because it was blocked on a sibling one, or on a park settling while the whole failed recycle came and went — is refused instead of reviving a child onto resources the failed recycle had already disposed. That refusal is retained rather than scoped to the calls the failed handoff overlapped, since the disposed resources stay gone after its barrier lifts: a hub `send` arriving once the failure has settled is refused on the same grounds, and a later recycle that reattached cleanly does not lift it, because parking children rebuilds nothing the failure destroyed. It lifts when a replacement session installs its persisted-subagent reviver factory — the point at which a live parent has rebound the dependencies a revival needs ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- The synchronous model controls (`setThinkingLevel`, `cycleThinkingLevel`, `setServiceTierFamily`, `setFastMode`, `toggleFastMode`) are now refused once a `restart` has latched or a host teardown has begun, each in its own no-op shape. Previously they still changed the session's observable thinking level and service tier while the sealed `SessionManager` dropped the matching `thinking_level_change`/`service_tier_change` entry, so the replacement session restored the previous selection and the change was silently undone ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Documented in the `onRestartRequested` SDK contract that `cwd` must be re-derived from the reopened manager rather than preserved. The primary API docs still listed it among the options to re-pass, so a host following them rebuilt a session that combined the original project's prompt and extensions with the moved-to project's filesystem ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
...
- A `restart` recycle now scopes its subagent parking to the restarting parent's own ownership chain. With two SDK top-level sessions sharing the process-global lifecycle, recycling one previously drained, parked, and stale-marked the *other* session's still-live children, so a later follow-up to them either could not revive or was rebuilt through the wrong replacement factory; the other session's subagents are now left untouched. Persisted-subagent reviver factories are likewise kept per top-level owner and selected by the parked child's own ownership chain, so a child is never rebuilt through another session's factory — which would bind it to that session's auth, models, settings, buses, cwd, and artifact manager. And a reattachment that fails after parking its children now fences only that owner's children: another live session's parked child, whose dependencies the failed recycle never disposed, still revives rather than being rejected indefinitely ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- An SDK host that follows the documented `createAgentSession` reconstruction now gets a persisted-subagent reviver factory installed automatically on the replacement session, so a subagent parked across the restart revives on demand. Previously only the CLI bootstrap installed one — and the CLI does not enable restart — so after an SDK restart every adopted child stayed parked and `ensureLive()` rejected it permanently unless the embedder wired undocumented internals ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` requested by a local extension/custom slash-command handler now holds that handler's in-flight window until the handler exits, so a handler that fires the restart fire-and-forget and keeps doing async work is never disposed out from under while it still uses the old runtime. `requestRestart()` is fire-and-forget from such a handler: awaiting your own restart would wait on the very in-flight window the recycle waits on, so the handler must fire it and return ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Extended the `onRestartRequested` SDK contract to list the remaining launch-time options a reconstruction must omit or re-derive, matching `docs/tools/restart.md`: `thinkingLevel`/`openAIServiceTier`/`resolveServiceTierByFamily` (re-passing the launch value rolls back a later `/thinking` or `/fast`), the one-shot `prewalk`/`planYolo` startup modes (re-passing a consumed one re-arms it), and a `rules` array snapshotted at first launch (which skips the reload that picks up changed rule files) ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` recycle no longer stalls other live SDK sessions while it rebuilds. Its all-agent parking barrier is now scoped to the restarting session's own subagents, so a slow or hung reconstruction no longer freezes IRC, collaboration, and hub traffic for an unrelated session's children — none of which is parked by the recycle ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` is no longer refused forever after an advisor hits a provider usage limit. The advisor pauses with its unreviewed batch still queued for replay after the quota window resets, and the recycle previously read that frozen backlog as an active review and refused as busy indefinitely — deadlocking the very restart that would rebuild or reset the advisor. A genuinely in-flight review still blocks the recycle ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- A `restart` requested while advisors are enabled at the default `advisor.syncBacklog: "off"` no longer refuses as busy forever. The turn that invokes `restart` queues its own advisor review as it ends — after the recycle has latched but before the agent goes idle — and the recycle read that review as active work and refused, while every retry queued another. The recycle now drains that review to completion before disposing, preserving its note as a persisted card the replacement reopens with. A review a caller started before requesting the restart still blocks it ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
