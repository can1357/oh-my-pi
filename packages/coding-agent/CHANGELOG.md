...
- A server stop that could not pin the server's helper subtree now fails instead of sweeping whatever part of it was visible and reporting success — whether the walk came back short, no reference to the server root could be opened at all, the root had already gone by the time it was pinned, or it went during the walk. In all four the helpers it may have left are past reach rather than absent: a walk rooted at an exited pid enumerates nothing and cannot tell that apart from a server that had no helpers. The root's liveness is now demanded on both sides of the walk rather than compared across it, so a server that was already a zombie when the stop reached it fails the stop like any other unwalkable subtree. The graceful handshake still runs either way, so the server keeps its chance to exit cleanly.
...
**A helper the language server spawns while it is answering `shutdown` is not covered on every host.** The mux pins the server's subtree before the handshake, and no instant is the right one: a helper born after that pin is in neither the pinned set nor anything the mux can walk afterwards, since the root is gone by then. What covers it instead is the server's process group, which names the subtree at termination time rather than at capture time. Deferred to the follow-up that carries the platform-enumeration work, where the uncovered hosts can actually be tested:
