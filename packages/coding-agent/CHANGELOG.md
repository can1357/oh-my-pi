...
- Added the `composer.pinToBottom` setting (Appearance → Composer → Pin to Bottom) to keep the input editor and status line on the terminal's bottom rows while the transcript is shorter than the screen.
