...
- `omp collab list` and `omp collab link` import chalk from the in-repo `@oh-my-pi/pi-utils/chalk` instead of the undeclared `chalk` package, so installs that only link declared dependencies (such as the Nix build) compile the CLI again.
