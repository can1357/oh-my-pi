...
- A `restart` requested by a local extension/custom slash-command handler now holds that handler's in-flight window until the handler actually awaits the restart, not the instant it calls `requestRestart()`. A handler that fired the restart fire-and-forget and kept doing async work — or awaited it and then cleaned up — could have the recycle observe zero in-flight prompts and dispose the session while the handler was still using the old runtime ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
