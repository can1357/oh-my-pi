...
- Fixed subagent `yield` retries for exact legacy `result` envelopes and batches of array-valued incremental sections; schema errors now identify invalid JSON, field paths, and the expected section shape, while thinking-only last turns explicitly request `data` ([#13170](https://github.com/can1357/oh-my-pi/pull/13170) by [@schickling-assistant](https://github.com/schickling-assistant)).
