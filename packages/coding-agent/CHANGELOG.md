...
- Added `retry.classifierRefusalCooldownMs` to time-box classifier-refusal model fallbacks: a positive value lets `retry.fallbackRevertPolicy` restore the primary model after the cooldown instead of pinning the fallback for the rest of the session (default `0` preserves the pin behavior). The cooldown is tracked per session so it never sidelines a model for a sibling session sharing the model registry. ([#10206](https://github.com/can1357/oh-my-pi/issues/10206))
