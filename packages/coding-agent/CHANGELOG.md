...
- Added mode skills: skills may declare `mode: true` and `reminder: "..."` in frontmatter, and `/mode` pins/unpins a mode skill for the session; a pinned skill's reminder rides the system prompt every turn and its name shows in the status line.
