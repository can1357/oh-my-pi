...
- A collab guest that rejoins repeatedly can no longer grow the host's memory without limit: a welcome snapshot waiting on a slow connection is now charged against the host's send-queue budget instead of counting as nothing, so the advertised limit is the real ceiling. Sharing a session larger than that limit still works.
- A single collab guest can no longer end sharing for the room: the host's send queue now bounds each guest's share, discards the backlog of a guest that outruns it and tells that guest to rejoin, and reserves ending the session for a backlog the host itself generated.
...
- A host prompt that asks collab guests a question no longer hangs when the send queue could not deliver the question to anybody: it resolves as unavailable instead of waiting for an answer nobody was asked for. A transcript reply is also no longer sent to a guest id whose owner left, or was reissued to someone else, while the transcript was being read.
