...
- A server stop that could not pin the server's helper subtree now fails instead of sweeping whatever part of it was visible and reporting success — whether the walk came back short or no reference to the server root could be opened at all, since in both cases the helpers it may have left are past reach rather than absent. The graceful handshake still runs either way, so the server keeps its chance to exit cleanly.
