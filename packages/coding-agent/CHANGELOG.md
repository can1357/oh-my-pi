...
- Fixed short generic model selectors like the smol role's bare `mini` matching mid-word inside `gemini-*` ids and silently routing background smol calls to a top-tier pro model; substring matches now require a token boundary ([#13318](https://github.com/can1357/oh-my-pi/pull/13318) by [@xiaodu55](https://github.com/xiaodu55))
