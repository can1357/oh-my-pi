...
- Added `omp gc --prune [archive|delete]` for whole dead conversations — the session-level counterpart to `/prune`. Two independent things make a session dead, and the pass reports which one fired: nobody answered (a prompt whose run was abandoned before the model said anything or ended a turn) and nobody asked (a file with no user message at all, which is what test harnesses and one-token probes leave behind — the model's canned reply makes such a file look answered, so a rule that only checks for a response keeps it forever). The mode is optional and defaults to archiving; deletion has to be asked for. The live session is never touched.
- Added `omp gc --prune [archive|delete]` for whole dead conversations — the session-level counterpart to `/prune`. Two independent things make a session dead, and the pass reports which one fired: nobody answered (a prompt whose run was abandoned before the model said anything or ended a turn) and nobody asked (a file with no user message at all, which is what test harnesses and one-token probes leave behind — the model's canned reply makes such a file look answered, so a rule that only checks for a response keeps it forever). The mode is optional and defaults to archiving; deletion has to be asked for. The live session is never touched. Pruning also removes per-cwd session directories that hold nothing, including ones earlier runs left behind: the directory is created when a session starts, so one accumulates for every throwaway working directory a session ever ran in, whether or not it produced a transcript, and archiving a transcript left its directory sitting there.
- The test runner now runs each chunk in a mount namespace where `~/.omp` is a tmpfs, so a test can no longer write into the developer's real agent directory. `bun test` sandboxes nothing, and session tests that pass a temp cwd but inherit the real agent dir had left 42 directories behind on one machine — 13 of them holding a one-line `"ok"` transcript that the session picker listed as a real conversation. Containment is at the OS level rather than by redirecting home-derived paths, because `os.homedir()` is fixed at Bun startup and repointing `XDG_CONFIG_HOME` breaks tests that set `HOME` for a child process. Everything else stays real and readable: gitconfig, the module cache, the natives addon. Where bubblewrap is unavailable, a test preload redirects `PI_CODING_AGENT_DIR` instead and a tripwire fails the run if anything still lands in the real directory.
