...
- Added `hindsight.bankIdTemplate` for exact `per-project` Hindsight bank IDs using the `{gitProject}` placeholder, while preserving existing bank derivation when unset.
