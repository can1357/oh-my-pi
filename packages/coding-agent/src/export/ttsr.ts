...
export class TtsrManager {
	 ...
	 *
	 * No-op while TTSR is globally disabled. `keep` is produced by `bucketRules`
	 * from `addOrUpdateRule`'s return, and that gates on `#settings.enabled` —
	 * so with TTSR off EVERY rule is rejected and `keep` comes back empty
	 * regardless of what is on disk. Reconciling against it would delete every
	 * registration AND every injection record, breaking {@link reconfigure}'s
	 * stated contract that both survive an enabled flip: re-enabling would
	 * register the rules from scratch, letting a `repeatMode: "once"` rule that
	 * was already injected fire a second time. An empty `keep` from a disabled
	 * manager carries no information, so the correct reconcile is none; the next
	 * refresh once TTSR is back on re-buckets against a real discovery and drops
	 * anything genuinely gone.
	 */
