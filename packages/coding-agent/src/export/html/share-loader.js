(function () {
	...
	async function fetchGistBlob(id) {
		...
		var file =
			files.find(function (f) {
				return /\.ompshare\.txt$/.test(f.filename);
			}) || files[0];
	...
	async function load() {
		...
		var plain;
		try {
			plain = await crypto.subtle.decrypt({ name: "AES-GCM", iv: sealed.subarray(0, 12) }, key, sealed.subarray(12));
		} catch (_err) {
			throw new Error("Decryption failed: wrong or corrupted #key.");
		}

		var data = await new Response(new Blob([plain]).stream().pipeThrough(new DecompressionStream("gzip"))).json();
