...
- **Persist selectors** to `.omp/mapreduce/<task-name>.selectors.md` (name, tool, exact pattern, rationale) so re-runs and reviewers can inspect, test, and tune them.
...
3. Group signals into **bounded batches**: target 10–20 signals or ≤ ~8 files per batch; keep same-file signals in one batch. Write batches to `local://mapreduce-batches.json`.
...
Format each assignment with the subagent handoff structure:
...
# Inputs
<the batch: file, line, selector, evidence — inline or local:// path>
# Non-goals
