...
export const getModelsConfigSchemaBundle = once(() => {
	...
	const ProviderOAuthSchema = type({
		...
		if (value.tokenEndpointAuthMethod && value.tokenEndpointAuthMethod !== "none" && !value.clientSecret) {
			return ctx.mustBe(`clientSecret set when tokenEndpointAuthMethod is ${value.tokenEndpointAuthMethod}`);
		}
		return true;
	});
