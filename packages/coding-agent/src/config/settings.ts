...
export class Settings {
	...
	/**
	 * Physical write target for a YAML config path. A quarantined file keeps
	 * writing to the moved-aside target until the next successful save clears
	 * the mapping; everything else resolves through the shared symlink-aware
	 * walk (see {@link resolveSymlinkWriteTarget}).
	 */
	async #resolveYamlWritePath(filePath: string): Promise<string> {
		...
		return resolveSymlinkWriteTarget(filePath);
	}
