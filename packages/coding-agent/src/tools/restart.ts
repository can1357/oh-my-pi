 ...
 *
 * The three variants live in one static asset selected by `reason` rather than
 * three files: they are one message with one shape, and a single template keeps
 * the set visible to a prompt reviewer in one place.
 */
function refusalNotice(reason: "unavailable" | "no-session-file" | "busy"): string {
	return prompt.render(restartRefusedNotice, { reason });
}
...
export class RestartTool implements AgentTool<typeof restartSchema, RestartToolDetails> {
	...
	async execute(
		...
		if (!requestRestart) {
			return {
				content: [{ type: "text", text: prompt.render(restartUnavailableNotice) }],
				isError: true,
		...
		void requestRestart()
			...
			.catch((err: unknown) => {
				...
				// rejection can carry terminal control sequences, tabs, many lines, or
				...
				if (this.session.isDisposed?.() === false) {
					this.session.queueDeferredMessage?.(
						createCustomMessage(
							"restart-refused",
							prompt.render(restartFailedNotice, { message }),
							true,
							undefined,
							new Date().toISOString(),
						),
		...
		return toolResult<RestartToolDetails>({ scheduled: true }).text(prompt.render(restartScheduledAck)).done();
	}
}
