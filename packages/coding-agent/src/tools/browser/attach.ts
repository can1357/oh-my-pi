...
export async function findReusableCdp(
	...
	const normalizedRequestedUserDataDir =
		requestedUserDataDir !== null && path.isAbsolute(requestedUserDataDir)
			...
			: null;
	// Process paths use the executable real path, not its launcher symlink. A distro wrapper
	// script defeats realpath, so resolve through the wrapper exec target for Chromium on Linux.
