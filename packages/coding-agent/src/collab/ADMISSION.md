...
| Advisory dropped                        | One missing transcript line. There are two rosters and neither rides on the notice: the participant list travels in `state` (`CollabHost#buildState`) and the agent list in `agents` (`CollabGuest#applyAgentSnapshots`), both re-sent on the next change. Both of those frames are themselves advisory, so a guest can hold either roster stale until then                                                                                                  |
...
| An ask whose last recipient was shed    | A `ui-request-end` for anyone still in the room; `requestGuestUi` resolves `unavailable` rather than waiting on the peer that was shed or gave up write permission                                                                                                                                                                                                                                                                                           |
| An ask whose last recipient left        | Nothing yet. The ask is retained, as one raised before any writer joined is, and handed to the next writer in its welcome batch                                                                                                                                                                                                                                                                                                                              |
...
The set only shrinks from there. A peer that leaves, is shed, or gives up write
permission on a later `hello` stops being able to answer anything it was handed,
and admission is not a promise that outlives the peer. A shed or a demotion that
empties the set settles the ask: the shed peer's rejoin would replay the backlog it
just outran, and the demoted peer is still in the room without the right to
answer. A departure that empties it does not. The ask returns to the retained
state a dialog raised before any writer joined is in, and `#handleHello` re-poses
every retained ask to the next writable guest inside its welcome batch, so up to
`MAX_PENDING_UI_REQUESTS` of them cost one entry of that peer's share rather than
one each, and a joiner can inherit a question nobody left in the room could
...
eligibility — so a read-only viewer's `hello` is enough to put the whole session
into the shared queue, and repeating `hello` re-arms it. It is appended, not
inserted: it delays nothing already queued and reaches the head only once the work
ahead of it drains, which is why this is a delay on what comes after rather than a
way to jump the queue. The guest does not
...
- **A batch's charge is declared, not measured.** The queue trusts the figure
  `sendBatch` was given, so a caller that under-declares under-charges, and there
  ...
  stripped. It does not trust the _domain_: `NaN` would make the queue read as
  never full and a negative figure would subtract from every other entry's charge,
  so both are charged as oversized instead — one entry, admitted by the floor,
  excluded from the sum. Not zero, which is the under-charge the charge exists to
  prevent, and not a refusal, because a caller bug should not silently cost a
  guest its only route to a replica. The figure is also only a proxy for the object graph, loose in both
...
- **A superseded batch is discarded before the replacement is admitted.** Step 3
  ...
  timer, which fires and prompts another rejoin.
   ...
   supersede _would_ leave, before discarding anything. That changes what step 3
