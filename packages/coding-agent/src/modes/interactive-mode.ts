...
export class InteractiveMode implements InteractiveModeContext {
	...
	/**
	 * One tick of an interval loop. A turn already streaming gets the prompt delivered as a
	 * non-interrupting *aside*: it rides the run's next step-boundary aside poll, so a long turn
	 * actually receives the instruction while it works (a two-hour turn on a 30-minute interval
	 * gets the reminder roughly four times) without disturbing anything in flight. Steering would
	 * be wrong here — under the default `interruptMode: "immediate"`, a queued steer aborts any
	 * interruptible tool the turn is currently running (a pure wait such as `hub wait` or `vibe`),
	 * so a periodic reminder could cancel a long-running wait it was never meant to touch. Follow-up
	 * delivery would also be wrong — it only drains once the run ends, so every tick during that
	 * turn would collapse into a single late delivery.
	 *
	 * The message is tagged with {@link LOOP_REMINDER_CUSTOM_TYPE} so a tick can detect one is
	 * already queued (a tick landing before the previous aside has been delivered must not pile up
	 * duplicates) and so `disableLoopMode` can retract it.
	 *
	 * An idle agent gets the same prompt through the normal submit path — when the agent has
	 * stopped, submission *is* just a prompt, which is what keeps the loop going after a turn ends.
	 * Compacting/post-prompt work (busy but not `isStreaming`) is skipped rather than risking
	 * `session.promptCustomMessage()` falling through to a fresh-turn start and throwing
	 * `AgentBusyError`; the next tick retries. A tick landing in the narrow gap between turns (busy
	 * flags clear but the next getUserInput() hasn't re-armed onInputCallback yet) is likewise
	 * skipped.
	 */
	#fireLoopIntervalTick(): void {
		...
		if (this.session.isStreaming) {
			...
			this.session
				...
				.catch(err => logger.warn("loop interval aside failed", { error: String(err) }));
			return;
		}
	...
	/** A mid-turn reminder is delivered as a non-interrupting aside, so the aside queue is the only
	 *  place one can be pending: a tick landing before the previous reminder has been polled must
	 *  not pile up a duplicate. The optional chain defends against a session facade that doesn't
	 *  implement the aside accessors (some test doubles and pooled-session stand-ins don't). */
	#hasQueuedLoopReminder(): boolean {
		return this.session.hasQueuedAside?.(LOOP_REMINDER_CUSTOM_TYPE) ?? false;
	}

	/** Drop any interval-loop reminder still pending on the aside queue so disabling or pausing the
	 *  loop doesn't leave a stray reminder to be polled into the run after the fact. `customType`-
	 *  scoped, so an unrelated aside (peer IRC record, extension send) riding the same queue is
	 *  never discarded. */
	#clearQueuedLoopReminders(): void {
		this.session.dropQueuedAsides?.(LOOP_REMINDER_CUSTOM_TYPE);
	}

	/**
	 * Drop a pending interval reminder so a user interrupt is honoured.
	 *
	 * An aside that outlives the run it was queued for is stranded, and
	 * `AgentSession.#resumeStrandedIrcAsides` folds a stranded non-IRC record into context once the
	 * user interrupt has suppressed auto-resume. Without this, Esc would silently append the
	 * reminder the user just cancelled to the transcript. Called *before* `session.abort()`, since
	 * the abort path is what strands the queue. The loop itself stays armed and reminds again on
	 * its next interval.
	 */
