...
import { setColorBlindMode, setMarkdownMermaidRendering, setSymbolPreset, setTheme } from "@oh-my-pi/pi-tui/theme";
import type { AgentSession } from "../../session/agent-session";
import type { ConfiguredThinkingLevel } from "@oh-my-pi/pi-tui/thinking";
import { applyHyperlinkSetting } from "@oh-my-pi/pi-tui/render/hyperlink";
import { setTerminalTitleSpinnerStyle, setTerminalTitleStateEnabled } from "../../utils/title-generator";
...
] as const satisfies readonly AnySetting["id"][];
...
export function snapshotReplaySettings(snapshotSettings: Settings): Map<string, unknown> {
	return new Map(REPLAYED_SETTING_IDS.map(id => [id, lookup(id)?.get(snapshotSettings)]));
}
...
export async function replaySessionSettingSideEffects(
	...
	for (const id of REPLAYED_SETTING_IDS) {
		const value = lookup(id)?.get(session.settings);
