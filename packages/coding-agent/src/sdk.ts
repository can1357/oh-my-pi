...
async function createAgentSessionScoped(
	...
	const unsubscribeCredentialDisabled: (() => void) | undefined = authStorage.onCredentialDisabled(event => {
		// A replayed sign-out that a previous session already handed to its
		// extension runner must not reach handlers twice: the replay exists for
		// the operator surface below, which never saw it.
		if (!authStorage.credentialDisabledReachedExtensions(event)) {
	...
	startupCleanup.releaseCredentialDisabled = () => {
		...
		// Anything not still waiting in the extension array already reached the
		// runner, so the next session must replay it to the operator only.
		const pendingForExtensions = new Set(startupCredentialDisabledEvents);
		// Only once the runner has initialized has anything it was handed actually
		// reached a handler; before that `emitCredentialDisabled` merely buffers,
		// and that buffer dies with the failed startup.
		const handlersRan = credentialDisabledTarget?.credentialDisabledHandlersReady === true;
		if (undelivered.length > 0) {
			authStorage.retainUndeliveredCredentialDisabled(undelivered, {
				extensionsAlreadyNotified: handlersRan ? undelivered.filter(event => !pendingForExtensions.has(event)) : [],
			});
		}
	};
...

		{
			...
			session.dispose = async () => {
				try {
					...
					const unseen = session.retainedCredentialDisabledEvents();
					if (unseen.length > 0) {
						authStorage.retainUndeliveredCredentialDisabled(unseen, {
							// Same rule: handed to the runner is not the same as delivered.
							extensionsAlreadyNotified:
								credentialDisabledTarget?.credentialDisabledHandlersReady === true ? unseen : [],
						});
					}
