...
function parseSuccessfulResponse(raw: string, numResults: number, fallbackRequestId?: string): SearchResponse {
	...
	for (const item of payload.data.results) {
		...
		if (!url) continue;
		sources.push({
			title: normalizeSearchText(item.title, 300) ?? url,
			url,
			snippet: normalizeSearchText(item.content, 8_000) ?? normalizeSearchText(item.snippet, 8_000),
		});
	}
...
async function callAnySearch(
	...
	if (response.ok) {
		const raw = await readLimitedText(response, "anysearch", MAX_RESPONSE_BYTES, false, 502);
		return {
			response: {
	...
	const raw = await readLimitedText(response, "anysearch", MAX_ERROR_BYTES, true);
...
async function persistGeneratedCredential(
	...
	try {
		await params.authStorage.credentials.addGeneratedApiKeyIfAbsent("anysearch", credential.apiKey, params.signal);
		const selectedKey = await params.authStorage.keys.get("anysearch", params.sessionId, { signal: params.signal });
...
export async function searchAnySearch(params: AnySearchParams): Promise<SearchResponse> {
	...
	const initialKey = await operationParams.authStorage.keys.get("anysearch", operationParams.sessionId, {
		signal: operationParams.signal,
	});
	if (initialKey) {
		const keyOrResolver: ApiKey = operationParams.authStorage.keys.resolver("anysearch", {
...
export class AnySearchProvider extends SearchProvider {
	...
	isAvailable(_authStorage: AuthStorage): boolean {
		// Anonymous quota is available without credentials, including in the default chain.
		// Credential provisioning remains restricted to an explicit primary selection.
		return true;
	}
