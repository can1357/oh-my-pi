...
- `agent(prompt, *, agent=None, label=None, schema=None, isolated=None, apply=None, merge=None{{#if evalTools}}, tools=None{{/if}}, model=None)`: immediate `AgentHandle`; use for a small fixed dependency graph or when the parent needs validated `schema` data. `model` accepts one selector or an ordered candidate list; when provided, selection is closed to that list and never falls back outside it. `.wait()` returns text/data; `.handle` is `agent://<id>`. Unwaited results auto-deliver.
