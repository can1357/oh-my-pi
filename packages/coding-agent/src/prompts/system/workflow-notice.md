...
- `agent(prompt, *, agent=None, label=None, schema=None{{#if isolationEnabled}}, isolated=None, apply=None, merge=None{{/if}}{{#if evalTools}}, tools=None{{/if}})`: immediate `AgentHandle`; use for a small fixed dependency graph or when the parent needs validated `schema` data. `.wait()` returns text/data; `.handle` is `agent://<id>`. Unwaited results auto-deliver.{{#if isolationEnabled}} `isolated` requests a worktree; `apply`/`merge` control its changes.{{/if}}
