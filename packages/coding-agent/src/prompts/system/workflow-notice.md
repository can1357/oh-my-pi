...
- `agent(prompt, *, agent=None, label=None, schema=None, isolated=None, apply=None, merge=None{{#if evalTools}}, tools=None{{/if}}, model=None)`: immediate `AgentHandle`; use for a small fixed dependency graph or when the parent needs validated `schema` data. Per-agent settings and custom definitions take precedence over `model`, followed by bundled defaults and parent model. If per-spawn `model` wins, it selects one selector or an ordered candidate list without outside fallback. `.wait()` returns text/data; `.handle` is `agent://<id>`. Unwaited results auto-deliver.
