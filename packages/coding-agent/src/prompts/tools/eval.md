...
{{#if spawns}}agent(prompt, agent?="{{spawnDefaultAgent}}", label?=None, schema?=None, schema{{#if js}}Mode{{else}}_mode{{/if}}?="permissive"{{#if isolationEnabled}}, isolated?=None, apply?=None, merge?=None{{/if}}{{#if evalTools}}, tools?=None{{/if}}) → AgentHandle
    Spawns a background subagent and returns immediately. `agent` selects a discovered agent; omit it to use `{{spawnDefaultAgent}}`.{{#if spawnAllowedAgentsText}} Allowed agents: {{spawnAllowedAgentsText}}.{{/if}} Handle: `.id`, `.handle` ("agent://<id>"), `.status`, `.done()`, `.wait(timeout?)` → final text (parsed with `schema`), `.send(message)`, `.cancel()`, `.output()`. Unwaited results auto-deliver like async jobs. `schema` overrides agent/session schemas;{{#if isolationEnabled}} `isolated` requests a worktree; `apply`/`merge` control its changes.{{/if}}{{#if evalTools}} `tools`: names of your @tool-defined tools the child may call.{{/if}}
{{#if js}}    JS: ONE trailing object — agent(prompt, { agent, label, schema, schema{{#if js}}Mode{{else}}_mode{{/if}}{{#if isolationEnabled}}, isolated, apply, merge{{/if}}{{#if evalTools}}, tools{{/if}} }).{{/if}}
...
{{#if evalTools}}{{#if py}}@tool / tool(fn, name=None, description=None){{/if}}{{#if js}}tool(fn, { name?, description?, parameters? }){{/if}}
    Define a tool that runs in this kernel{{#if py}} (schema inferred from type hints){{/if}}; reference by name in `task` items' `tools`{{#if spawns}}, `agent(tools=…)`, `workpool(tools=…)`{{#if isolationEnabled}}, `isolated`/`apply`/`merge`{{/if}}{{/if}}. `tool.defined()`, `tool.undefine(name)`.
{{/if}}
