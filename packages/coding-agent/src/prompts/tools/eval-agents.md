```
agent(prompt, agent?="{{spawnDefaultAgent}}", label?=None, schema?=None, schema{{#if js}}Mode{{else}}_mode{{/if}}?="permissive"{{#if isolationEnabled}}, isolated?=None, apply?=None, merge?=None{{/if}}{{#if evalTools}}, tools?=None{{/if}}) → AgentHandle
...
{{#if js}}    JS: ONE trailing object — agent(prompt, { agent, label, schema, schemaMode{{#if isolationEnabled}}, isolated, apply, merge{{/if}}{{#if evalTools}}, tools{{/if}} }).{{/if}}
