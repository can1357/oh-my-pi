...
export async function reachableModelMayRequireStructuredOutputHardening(args: {
	...
	const fallbackEnabled = cfgRetryModelFallback.get(settings);
	const promotionEnabled = cfgContextPromotionEnabled.get(settings) === true;
	...
	const runSubagent = async (): Promise<{
		...
		try {
			...
			const buildSubagentSessionOptions = (
				...
				preloadedPreparedExtensions: lockedInit ? [] : options.preloadedPreparedExtensions,
				...
				customTools: !lockedInit && sessionCustomTools.length > 0 ? sessionCustomTools : undefined,
