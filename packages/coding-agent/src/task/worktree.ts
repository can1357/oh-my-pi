...
export interface SpeculativeTargetEvidence {
	exists: boolean;
	mode?: number;
	size?: number;
	digest?: string;
...
export class SpeculativePalCommitConflictError extends Error {
	override readonly name = "SpeculativePalCommitConflictError";
}

export const SPECULATIVE_PAL_MAX_TARGET_BYTES = 4 * 1024 * 1024;

export class SpeculativePalTargetTooLargeError extends Error {
	override readonly name = "SpeculativePalTargetTooLargeError";
}

export async function speculativePalTargetsFitSnapshotLimit(targetPaths: readonly string[]): Promise<boolean> {
	for (const targetPath of targetPaths) {
		try {
			const state = await fs.lstat(targetPath);
			if (state.isFile() && state.size > SPECULATIVE_PAL_MAX_TARGET_BYTES) return false;
		} catch (error) {
...
export interface SpeculativePalTransaction {
...
function evidenceMatches(left: SpeculativeTargetEvidence, right: SpeculativeTargetEvidence): boolean {
	return (
...
type SpeculativeTargetSnapshot = {
	evidence: SpeculativeTargetEvidence;
	content?: Uint8Array;
};
...
async function captureSpeculativeTargetEvidence(target: string): Promise<SpeculativeTargetEvidence> {
	try {
		...
		if (state.isSymbolicLink()) throw new Error(`Speculative target is a symbolic link: ${target}`);
		if (!state.isFile()) return { exists: true, mode: state.mode, size: state.size };
		if (state.size > SPECULATIVE_PAL_MAX_TARGET_BYTES) {
			throw new SpeculativePalTargetTooLargeError(
				`Speculative PAL target exceeds ${formatBytes(SPECULATIVE_PAL_MAX_TARGET_BYTES)}: ${target}`,
			);
		}
		...
		const hash = new Bun.CryptoHasher("sha256");
		hash.update(content);
		return { exists: true, mode: state.mode, size: state.size, digest: hash.digest("hex") };
	} catch (error) {
...
async function assertNoSymlinkComponents(repositoryRoot: string, target: string): Promise<void> {
	const relative = path.relative(repositoryRoot, target);
	if (relative === "" || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
		throw new Error(`Speculative target is outside the repository: ${target}`);
	}
	...
	for (const segment of relative.split(path.sep)) {
		...
		try {
			if ((await fs.lstat(current)).isSymbolicLink())
				throw new Error(`Speculative target traverses a symbolic link: ${target}`);
		} catch (error) {
...
/**
 * Creates a target-scoped PAL transaction. It never writes the source checkout:
 * callers perform speculative I/O only through the returned mapped targets.
 */
export async function openSpeculativePalTransaction(
	baseCwd: string,
	id: string,
	targetPaths: readonly string[],
): Promise<SpeculativePalTransaction> {
	const repositoryRoot = await getRepoRoot(baseCwd);
	const sourceTargets = new Map<string, string>();
	const baseline = new Map<string, SpeculativeTargetSnapshot>();
	for (const targetPath of targetPaths) {
		const target = path.resolve(baseCwd, targetPath);
		await assertNoSymlinkComponents(repositoryRoot, target);
		if (sourceTargets.has(target)) continue;
		sourceTargets.set(target, path.relative(repositoryRoot, target));
		baseline.set(target, await captureSpeculativeTargetSnapshot(target));
	}
	const isolation = await ensureIsolation(repositoryRoot, `speculation-${id}`);
	try {
		for (const [sourceTarget, relative] of sourceTargets) {
			const sandboxTarget = path.join(isolation.mergedDir, relative);
			const [sourceEvidence, sandboxEvidence] = await Promise.all([
				captureSpeculativeTargetEvidence(sourceTarget),
				captureSpeculativeTargetEvidence(sandboxTarget),
			]);
			if (!evidenceMatches(sourceEvidence, sandboxEvidence)) {
				throw new Error(`PAL materialization did not preserve speculative target: ${sourceTarget}`);
			}
		}
		return {
			repositoryRoot,
			isolation,
			targets: new Map(
				[...sourceTargets].map(([sourceTarget, relative]) => [
					sourceTarget,
					path.join(isolation.mergedDir, relative),
				]),
			),
			async verify(): Promise<boolean> {
				for (const [sourceTarget, expected] of baseline) {
					if (!evidenceMatches(await captureSpeculativeTargetEvidence(sourceTarget), expected.evidence))
						return false;
				}
				return true;
			},
			async restore(): Promise<void> {
				for (const [sourceTarget, snapshot] of baseline) {
					if (!snapshot.evidence.exists) {
						await fs.rm(sourceTarget, { force: true });
						continue;
					}
					if (!snapshot.content) throw new Error(`Cannot restore non-file speculative target: ${sourceTarget}`);
					await Bun.write(sourceTarget, snapshot.content);
					if (snapshot.evidence.mode !== undefined) await fs.chmod(sourceTarget, snapshot.evidence.mode);
				}
			},
