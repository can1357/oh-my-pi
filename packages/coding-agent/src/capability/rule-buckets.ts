...
export function bucketRules(
	...
	for (const rule of includedRules) {
		...
		// buckets below, exactly as at init. When `replaceTtsrRules` is set the
		// manager has already atomically re-registered every rule via
		// `replaceRules`, so membership comes from that pass instead.
		const hasTtsrCondition =
			(rule.condition && rule.condition.length > 0) || (rule.astCondition && rule.astCondition.length > 0);
		if (hasTtsrCondition) {
			const consumed = replacedTtsrNames ? replacedTtsrNames.has(rule.name) : ttsrManager.addOrUpdateRule(rule);
			if (consumed) {
