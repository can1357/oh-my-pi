		...
		while (true) {
			...
			if (requested) {
				...
				if (options?.producesTurn) {
					...
					// at most ONCE per caller: `prompt()` re-runs this settle after image
					// preprocessing, so a caller that already holds a claim (or parked in
					// this same call) passes `alreadyClaimed` to avoid a second token its
					// single release would never balance.
