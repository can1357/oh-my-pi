...
export class AgentSession {
	...
	async #dispatchPrompt(
		...
		if (this.isStreaming) {
			...
			return await this.#queueUserMessage(expandedText, options?.images, streamingBehavior, {
	...
	async #dispatchCustomMessage<T = unknown>(
		...
		if (options?.deliverAs === "nextTurn") {
			if (options?.triggerTurn) {
				...
				// disposed session parks inside #promptOrParkAgentInitiatedMessage, and
				// returning a false `true` would leave a protocol host awaiting an
				// `agent_end` that never comes.
				outcome.sessionClaimed = await this.#promptOrParkAgentInitiatedMessage(normalizedAppMessage, {
