 ...
 * guidance advertises a `proc://<id>/progress` retune, so a batch built for a
 * session without the `write` tool must not offer it — the batch's own
 * contents cannot answer that question.
 */
		...
		chattyJobs.length === 0
			...
			: prompt
					.render(chattyProgressGuidanceTemplate, {
						...
						service: chattyJobs.some(job => job.type === "process"),
						procWrite: capabilities?.procWrite === true,
					})
					.trim();
	return {
		role: "custom",
