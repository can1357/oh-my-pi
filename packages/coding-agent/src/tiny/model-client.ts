 ...
 * die on its own once idle. Callers provide message text and optional
 * generation controls; the worker only ever sees message-level `chat`
 * requests.
 */
...
import { $env, getTinyWorkerRuntimeDir, logger } from "@oh-my-pi/pi-utils";
...
type PendingRequest =
	...
	| { kind: "load"; modelKey: TinyLocalModelKey; resolve: (result: TinyModelDownloadResult) => void };
...
export interface TinyModelCompletionOptions {
	maxTokens?: number;
	/** Text appended after the formatted prompt before generation starts. */
	prefill?: string;
	signal?: AbortSignal;
	/** Text sequence that stops generation when it appears in the generated output. */
	stop?: string;
	systemPrompt?: string;
}
 ...
 * used. Only resolved keys are returned — never the default sentinel — so
 * the worker's built-in defaults apply for anything absent. Pure for testability;
...
/**
 * Handle that resolves its backing worker asynchronously. `TinyModelClient`
 * needs a handle synchronously (it subscribes and sends in the same tick),
 * but reaching a worker means probing and possibly spawning; sends queue until
...
async function probeTinyWorker(endpoint: string, tag: string): Promise<ProbeResult> {
	...
	try {
		...
		if (reply?.type === "pong") {
			logger.debug("tiny-model: worker launch tag mismatch; replacing", {
				endpoint,
				running: reply.tag,
				expected: tag,
			});
...
export class TinyModelClient {
	...
	async #connectDefault(modelKey: TinyLocalModelKey): Promise<WorkerHandle> {
		if (tinyWorkerUsesMlx()) {
			try {
				...
				mlxUnavailable = true;
				logger.warn("tiny-model: MLX worker unavailable; falling back to ONNX CPU", {
					modelKey,
	 ...
	 * the first completion finds a live connection instead of paying the
	...
	prewarm(modelKey: string): void {
		...
		try {
			...
			logger.debug("tiny-model: prewarm failed", {
				modelKey,
				error: error instanceof Error ? error.message : String(error),
			});
		}
	...
	async complete(
		...
		const request: Extract<TinyWorkerRequest, { type: "chat" }> = {
			type: "chat",
		...
		if (options.prefill !== undefined) request.prefill = options.prefill;
	...
	#run<T>(
		...
		try {
			...
			logger.debug("tiny-model: local request failed", {
	...
	#handleMessage(modelKey: TinyLocalModelKey, message: TinyWorkerResponse): void {
		...
		if (message.type === "error") {
			logger.debug("tiny-model: worker returned error", { modelKey, error: message.error });
		...
		if (message.type === "text") {
			if (pending.kind === "completion") pending.resolve(message.text.trim() || null);
			return;
		}
	...
	#markFailedModel(pending: PendingRequest): void {
		if (pending.kind === "completion") this.#failedModels.add(pending.modelKey);
	}
	...
	#handleWorkerError(modelKey: TinyLocalModelKey, error: Error): void {
		...
		if (failed === 0 && error.message.startsWith(TINY_WORKER_CLOSED)) {
			logger.debug("tiny-model: worker went away while idle", { modelKey });
		} else {
			logger.warn("tiny-model: worker error", { modelKey, error: error.message, failedRequests: failed });
		}
	}
}
