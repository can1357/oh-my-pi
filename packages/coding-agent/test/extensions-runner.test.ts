...
describe("ExtensionRunner", () => {
	...
	describe("tool_call input", () => {
		...
		it("defers passive context until the wrapper approval gate succeeds (P2)", async () => {
			// Wrapper-dispatched calls (nested xd://, direct Cursor execution)
			// collect additionalContext before the approval gate. Forwarding
			// must wait for approval: an approved call delivers, a denied call
			// (or fail-closed safety refusal) throws before anything is injected.
			const extCode = `
				export default function(pi) {
					pi.on("tool_call", async (event) => {
			...
			await expect(
				(wrapped as ExtensionToolWrapper<any>).execute(
					"call-deny",
