...
describe("ExtensionRunner", () => {
	...
	describe("tool approval lifecycle", () => {
		...
		describe("session approval options", () => {
			...
			it("uses ACP approval before similar grants without approving later arguments", async () => {
				addSimilarApproval(
				...
				const classify = vi
					...
					.mockResolvedValue({ covered: false, writeTargets: [] });
				const select = vi.fn(async (_title: string, _options: ExtensionUISelectItem[]) => "Deny");
				const runner = await makeRunner(select);
				const wrapper = new ExtensionToolWrapper(
					{ ...approvalTool, parameters: Type.Object({ command: Type.String() }) },
					runner,
				);
				const context = makeContext({ acpApprovedArgs: { command: "git diff" } });

				const approved = await wrapper.execute(
					"call-acp-approved",
					{ command: "git diff" },
					undefined,
					undefined,
				...
				expect(resultText(approved)).toBe("ok");
				expect(select).not.toHaveBeenCalled();
				expect(classify).not.toHaveBeenCalled();

				await expect(
					wrapper.execute("call-acp-different", { command: "git clean -fd" }, undefined, undefined, context),
