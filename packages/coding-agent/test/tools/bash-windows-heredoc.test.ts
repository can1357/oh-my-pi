...
import { admitWindowsHeredocCommand, shouldEnforceWindowsHeredocDiscipline } from "../../src/tools/bash-command-fixup";
...
describe("windows heredoc admission", () => {
	test("unrestricted profiles are not rejected", () => {
		const decision = admitWindowsHeredocCommand("cat <<'EOF'\nhello\nEOF", undefined, "win32");
