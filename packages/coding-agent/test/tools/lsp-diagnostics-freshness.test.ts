...
import {
	...
	cfgLspFormatOnWrite,
} from "@oh-my-pi/pi-coding-agent/lsp/settings";
...
describe("LSP diagnostics freshness", () => {
	...
	it("WriteTool honors a live lsp.formatOnWrite flip so a reloaded config formats without restart", async () => {
		...
		cfgLspFormatOnWrite.set(settings, false);
		cfgLspDiagnosticsOnWrite.set(settings, false);
		cfgLspDiagnosticsDeduplicate.set(settings, true);
		const session: ToolSession = {
			cwd: tempDir.path(),
			hasUI: false,
			getSessionFile: () => null,
		...
		cfgLspFormatOnWrite.override(settings, true);

		await tool.execute("format-after-enable", { path: filePath, content: raw });
		expect(await Bun.file(filePath).text()).toBe("export const value = 1;\n");
	});

	it("WriteTool honors a live lsp.diagnosticsOnWrite flip so reloaded settings defer diagnostics to the agent", async () => {
		...
		cfgLspDiagnosticsOnWrite.override(settings, true);
