...
import { powershellToolRenderer } from "@oh-my-pi/pi-tui/tools/powershell";
...
suite("PowerShellTool (persistent host)", () => {
	...
	test("a session host that dies mid-run is dropped and respawned", async () => {
		...
		await expect(
			tool.execute("d2", {
				command: "Write-Host 'before-crash-marker'; Start-Sleep -Milliseconds 200; [Environment]::Exit(5)",
			}),
