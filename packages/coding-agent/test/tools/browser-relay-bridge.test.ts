...
async function acknowledgeImmediatePreloadRegistration(
	bridge: RelayBridge,
	ext: FakeExtSocket,
	identifier: string,
	loaderId?: string,
): Promise<void> {
	ack(bridge, ext, "send", { identifier });
	await waitFor(
		() => ext.pending("send").some(rpc => rpc.method === "Page.getFrameTree"),
		"post-registration loader snapshot",
	);
	ack(bridge, ext, "send", { frameTree: { frame: loaderId !== undefined ? { loaderId } : {} } });
	await flush();
}
...
async function attachTab(
	...
	bridge.cdpMessage(
		...
		JSON.stringify({
			id: attachId,
			method: "Target.attachToTarget",
			params: { targetId: `TAB${instanceCode(instanceId)}.${tabId}`, flatten: true },
		}),
	);
	await waitFor(
		() => ext.pending("attach").length > 0 || cdp.sessionFor(attachId) !== undefined,
...
describe("RelayBridge tab grouping", () => {
	...
	it.each([
		{
			...
			source: '#!/usr/bin/env node\n"use strict";\nthis.__preloadRan = true;',
		},
		{
			name: "after a leading BOM",
			source: '\uFEFF"use strict";\nthis.__preloadRan = true;',
		},
	])("does not rerun immediate preload scripts when guard recovery preserves contexts $name", async ({ source }) => {
		const bridge = new RelayBridge({});
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
	...
	it("records the loader after an initial immediate preload registration", async () => {
		...
		ack(bridge, ext, "send", { identifier: "root-script-before-recovery" });
		await waitFor(() => ext.pending("send").some(rpc => rpc.method === "Page.getFrameTree"));
		ack(bridge, ext, "send", { frameTree: { frame: { loaderId: "loader-after" } } });
		await flush();
		...
		ack(bridge, ext2, "attach");
		await waitFor(() => ext2.pending("send").some(rpc => rpc.method === "Page.getFrameTree"));
		ack(bridge, ext2, "send", { frameTree: { frame: { loaderId: "loader-after" } } });
	...
	it("keeps preserved sessions when a changed frame disappears before isolated-world replay", async () => {
		const bridge = new RelayBridge({});
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
		...
		nack(bridge, ext2, "send", "No frame with given id found");
		await waitFor(() => ext2.pending("send").some(rpc => rpc.method === "Runtime.disable"));
		ack(bridge, ext2, "send");
		await waitFor(() => ext2.pending("send").some(rpc => rpc.method === "Page.addScriptToEvaluateOnNewDocument"));
		ack(bridge, ext2, "send", { identifier: "root-script-marker-only" });
		await flush();
		ack(bridge, ext2, "send", { frameTree: { frame: { id: "main", loaderId: "main-loader" } } });
		await flush();
		...
		await waitFor(() => ext2.pending("send").some(rpc => rpc.method === "Runtime.evaluate"));
		ack(bridge, ext2, "send", { result: { value: 2 } });
		await waitFor(() => cdp.messages.some(message => message.id === commandId));
	...
	it("runs a preload once and clears its marker in every existing frame during handoff", async () => {
		...
		expect(cleanup?.params).toMatchObject({ runImmediately: true });
		ack(bridge, ext2, "send", { identifier: "root-script-marker-only" });
		await waitFor(() =>
			ext2
				.pending("send")
	...
	it("does not rerun a preload for navigation after registration was acknowledged", async () => {
		...
		await waitFor(() => ext.pending("send").some(rpc => rpc.method === "Page.addScriptToEvaluateOnNewDocument"));
		await acknowledgeImmediatePreloadRegistration(bridge, ext, "root-script-replayed", "loader-before");
	...
	it("reruns an immediate preload when recovery has no prior loader baseline", async () => {
		...
		await waitFor(() => ext.pending("send").some(rpc => rpc.method === "Page.addScriptToEvaluateOnNewDocument"));
		await acknowledgeImmediatePreloadRegistration(bridge, ext, "root-script-before-recovery");
	...
	it("retires the application marker after an interrupted preload replay registration", async () => {
		const bridge = new RelayBridge({});
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
		...
		// Chrome may have accepted the marker-bearing registration, but its reply
		// never reaches the relay. The fresh-root recovery must also retire this
		// unjournaled marker from exception suppression.
		bridge.extClosed(ext2);
		await flush();
		const ext3 = new FakeExtSocket();
		connect(bridge, ext3, [tab({ tabId: 1, groupId: -1 })], {
...
describe("RelayBridge multiple extension instances", () => {
	...
	it("replaces the socket, restores runtime state, and keeps the registry when the same instance reconnects", async () => {
		const bridge = new RelayBridge({});
		...
		// The preserved holder is reattached and its enabled Runtime state is
		// restored on the replacement socket, rather than being short-circuited by
		// stale rootRuntimeEnabled state from the retired socket.
		await waitFor(() => second.pending("attach").length === 1, "replacement recovery attach");
		ack(bridge, second, "attach");
		await waitFor(() => second.pending("send").some(rpc => rpc.method === "Runtime.disable"));
		expect(second.pending("send").map(rpc => rpc.method)).toEqual(["Runtime.disable"]);
		ack(bridge, second, "send");
		await waitFor(() => second.pending("send").some(rpc => rpc.method === "Runtime.enable"));
		expect(second.pending("send").map(rpc => rpc.method)).toEqual(["Runtime.enable"]);
		ack(bridge, second, "send");
		await flush();

		const evaluateId = ++msgSeq;
		bridge.cdpMessage(connId, JSON.stringify({ id: evaluateId, sessionId: session, method: "Runtime.evaluate" }));
		await waitFor(() => second.pending("send").some(rpc => rpc.method === "Runtime.evaluate"));
		ack(bridge, second, "send", { value: true });
		await waitFor(() => cdp.messages.some(message => message.id === evaluateId));
		expect(cdp.messages.find(message => message.id === evaluateId)?.error).toBeUndefined();
	});
