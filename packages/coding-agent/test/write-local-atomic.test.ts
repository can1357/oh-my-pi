...
describe("write tool atomic local routing", () => {
	...
	it("keeps generated local files protected before atomic routing", async () => {
		const localUrl = "local://generated.ts";
		const localOptions: SessionLocalOptions = {
			getArtifactsDir: () => path.join(tmpDir, "artifacts"),
			getSessionId: () => "generated-local",
		};
		const absolutePath = localProtocol.resolveLocalUrlToPath(localUrl, localOptions);
		const generatedContent = "// Code generated by sqlc. DO NOT EDIT.\nexport const value = 1;\n";
		await fs.mkdir(path.dirname(absolutePath), { recursive: true });
		await fs.writeFile(absolutePath, generatedContent);
		const atomicWriteSpy = vi.spyOn(localProtocol, "writeLocalUrlAtomically");

		await expect(
			new WriteTool(createSession(tmpDir, localOptions)).execute("generated-local", {
				path: localUrl,
				content: "export const value = 2;\n",
			}),
		).rejects.toThrow("auto-generated");

		expect(atomicWriteSpy).not.toHaveBeenCalled();
		expect(await fs.readFile(absolutePath, "utf8")).toBe(generatedContent);
	});
});
