...
export default function nativeInputExtension(pi: ExtensionAPI): void {
	...
	pi.registerCommand("native-gated", {
		description: "Hold a locally consumed command until the probe releases it",
		handler: async args => {
			await fetch(`${url}/gates`, { method: "POST", body: JSON.stringify({ name: "command" }) });
			await record({ event: "command", args });
		},
	});
