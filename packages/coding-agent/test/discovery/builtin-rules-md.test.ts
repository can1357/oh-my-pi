 ...
 * from both `~/.ompk/agent/RULES.md` (user) and the nearest `.ompk/RULES.md`
...
import { type Rule, ruleCapability } from "@pk-nerdsaver-ai/pi-coding-agent/capability/rule";
...
test("user ~/.ompk/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".ompk", "agent", "RULES.md"),
...
test("project .ompk/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".ompk", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".ompk", "RULES.md"), "# Repo-wide sticky rule\n");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".ompk", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .ompk/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".ompk", "agent", "rules", "other.md"), "# Unrelated rule\n");
