...
import { cfgMcpNotifications } from "@oh-my-pi/pi-coding-agent/mcp/settings";
import {
...
describe("applySettingSideEffects replay coverage", () => {
	...
	it("replays statusLine segment keys through the shared status line apply", () => {
		...
		cfgStatusLineRightSegments.set(settings, ["token_total"]);
		cfgStatusLineSegmentOptions.set(settings, { model: { showSpeed: true } });
		...
		for (const [id, handle] of [
			...
			applySettingSideEffects(ctx, id, handle.get(settings), { persist: false });
		}
...
describe("replaySessionSettingSideEffects", () => {
	...
	it("replays mcp.notifications through the host-supplied MCP manager", async () => {
		...
		const session = stubSession();
		const beforeReplay = snapshotReplaySettings(settings);
		cfgMcpNotifications.set(settings, true);
