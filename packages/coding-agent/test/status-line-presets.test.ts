...
import { cfgGitEnabled, cfgStatusLinePreset } from "@oh-my-pi/pi-coding-agent/modes/settings";
...
beforeEach(async () => {
	...
	cfgGitEnabled.override(settings, false);
	await initTheme();
});
...
function makeComponent(): StatusLineComponent {
	...
	const session = {
		...
		modelRegistry: {
			...
			authStorage: {
				oauth: {
					identity: (provider: string) => (provider === "openai-codex" ? { accountId: ACCOUNT_ID } : undefined),
				},
			},
		},
		sessionManager: {
	...
	return new StatusLineComponent(session, statusLineHost);
}
...
async function renderPreset(preset: TestedPreset): Promise<string> {
	cfgStatusLinePreset.override(settings, preset);
