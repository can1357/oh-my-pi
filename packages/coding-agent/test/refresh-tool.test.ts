...
// The INVALID-scope branch interpolates the raw argument, and that argument is
// as attacker-influenced as any reload error: ACP/RPC clients pass it through
// verbatim and a TUI paste can carry anything. It must get the same treatment
// the failure branch gets, or a hostile `/refresh <arg>` corrupts the renderer
// from a path that never even reaches the session.
describe("/refresh invalid-scope sanitization", () => {
	it("strips control bytes and tabs from an invalid scope before rendering it", async () => {
		const refresh = vi.fn(async (_scope: RefreshScope) => ({}) as RefreshResult);
		const h = commandRuntime(refresh);

		const hostileScope = "\x1b[31mbogus\x1b[0m\tscope\x07";
		const result = await executeAcpBuiltinSlashCommand(`/refresh ${hostileScope}`, h.runtime);
		...
		expect(message).not.toContain("\x1b");
		expect(message).not.toMatch(/[\x00-\x08\x0b-\x1f\x7f-\x9f]/);
		// The readable payload still identifies what the user typed, and the
		// valid-scope list is still advertised.
		expect(message).toContain("bogus");
		expect(message).toContain("skills, rules, settings, mcp, all");
	});

	it("truncates an oversized invalid scope to the standard line width", async () => {
		const refresh = vi.fn(async (_scope: RefreshScope) => ({}) as RefreshResult);
		const h = commandRuntime(refresh);

		const longScope = "z".repeat(600);
		await executeAcpBuiltinSlashCommand(`/refresh ${longScope}`, h.runtime);

		expect(refresh).not.toHaveBeenCalled();
		const message = (h.output.mock.calls[0]?.[0] as string) ?? "";
		// The interpolated argument is what can overflow; the surrounding usage
		// text is the command's own constant framing.
		const rendered = message.match(/^Unknown refresh scope "(.*)"\. Use: /)?.[1] ?? "";
		expect(rendered.length).toBeGreaterThan(0);
		expect(rendered.length).toBeLessThanOrEqual(TRUNCATE_LENGTHS.LINE);
		expect(message).not.toContain(longScope);
	});
});
