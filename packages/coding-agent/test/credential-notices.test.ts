...
describe("credential sign-out notices", () => {
	it("keeps an API-key sign-out when revalidation could not prove the pool", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "credential-notices-unknown-pool-"));
		const store = await SqliteAuthCredentialStore.open(path.join(dir, "agent.db"));
		const authStorage = new AuthStorage(store);
		try {
			// A sibling key was re-saved, which purges every API-key tombstone, so the
			// history read below legitimately returns nothing for this row.
			const retained = new Map<number, RetainedCredentialDisable>([
				[
					7,
					{
						event: {
							provider: "openai",
							disabledCause: "rejected by the provider",
							credentialId: 7,
							credentialType: "api_key",
						} satisfies CredentialDisabledEvent,
						disabledAtMs: Date.now() - 1_000,
						siblingApiKeyIds: undefined,
					},
				],
			]);
			// Revalidation fails: the live-key pool is unknown, not empty.
			vi.spyOn(authStorage, "revalidateCredentials").mockRejectedValue(new Error("broker unreachable"));
			store.listDisabledCredentials = async () => [];

			const notices = await collectDisabledCredentialNotices(authStorage, Date.now(), () => false, retained);
			expect(notices).toHaveLength(1);
			expect(retained.has(7)).toBe(true);
		} finally {
			authStorage.close();
