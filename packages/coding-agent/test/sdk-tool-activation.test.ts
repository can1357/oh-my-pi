...
describe("createAgentSession defaultInactive tool activation", () => {
	...
	it.each([
		{ params: { data: { token: "x" } }, error: "Output does not match schema" },
		{ params: { error: 42 }, error: "error must be a string" },
	])("locks a schema-correction session to yield after invalid arguments: %j", async ({ params, error }) => {
		const tempDir = makeTempDir();
		const outputSchema = {
			type: "object",
		...
		try {
			...
			await expect(yieldTool!.execute("bad-yield", params)).rejects.toThrow(error);
	...
	it("aborts a running side-effect sibling after a malformed yield error locks the active batch", async () => {
		const tempDir = makeTempDir();
	...
	it("lets a live resolver clear an initial hardening policy before correction lock", async () => {
		...
		await withProviderAuth(["merge-gateway", "openai"], async () => {
			...
			try {
				...
				await expect(yieldTool.execute("bad-yield", { data: { token: "x" } })).rejects.toThrow(
	...
	it("applies correction policy against the live model after a runtime switch", async () => {
		...
		await withProviderAuth(["merge-gateway", "openai"], async () => {
			...
			try {
				...
				const yieldTool = session.getToolByName("yield");
				if (!yieldTool) throw new Error("Expected yield tool");
				await expect(yieldTool.execute("bad-yield-after-lock", { data: { token: "x" } })).rejects.toThrow(
					"Output does not match schema",
				);
