...
class NativeInputProbe {
	...
	promptResult(id: string): Promise<Frame> {
		return this.wait(
	...
	describe(`native external input through ${mode}`, () => {
				...
				for (const type of ["prompt", "steer", "follow_up", "abort_and_prompt"] as const) {
					for (const message of ["/native-local blocked", "empty"]) {
						...
						if (type === "prompt" || type === "abort_and_prompt") {
							...
							expect(await probe.promptResult(String(response.id))).toMatchObject({
								agentInvoked: false,
								status: "completed",
								sessionSettled: true,
							});
		...
		test("transforms before builtin, native command, skill, and template dispatch", async () => {
			...
			try {
				...
				expect(await probe.promptResult(String(builtin.id))).toMatchObject({
					agentInvoked: false,
					status: "completed",
					sessionSettled: true,
				});
				...
				expect(await probe.promptResult(String(local.id))).toMatchObject({
					agentInvoked: false,
					status: "completed",
					sessionSettled: true,
				});
		...
		for (const laterType of ["steer", "follow_up"] as const) {
			test(`accepts later ${laterType} input while an extension command prompt is still running`, async () => {
				const probe = new NativeInputProbe();
				try {
					await probe.start(mode);
					// An assistant tail lets an idle follow-up drain like an idle steer.
					await probe.command({ type: "prompt", message: "BEFORE_COMMAND" });
					await probe.finish(await probe.request(0));
					const command = await probe.command({ type: "prompt", message: "/native-gated held" });
					const release = await probe.gate("command");
					await probe.command({ type: laterType, message: "LATER_INPUT" });
					expect(probe.events.some(event => event.event === "command")).toBe(false);
					release();
					// The later input's turn is still live, so the session is not settled yet.
					expect(await probe.promptResult(String(command.id))).toMatchObject({
						agentInvoked: false,
						status: "completed",
						sessionSettled: false,
					});
					expect(probe.events.filter(event => event.event === "command")).toEqual([
						{ event: "command", args: "held" },
					]);
					const later = await probe.request(1);
					expect(userContent(later)).toContain("LATER_INPUT");
					await probe.finish(later);
				} finally {
					await probe.close();
				}
			}, 60000);
		...
		test("aborts ordinary prompt admission during vision preprocessing without a ghost turn", async () => {
			...
			try {
				...
				expect(await probe.promptResult(String(pending.id))).toMatchObject({
					agentInvoked: false,
					status: "completed",
					sessionSettled: true,
				});
		...
		test("reports a deleted skill through a same-id failure after prompt acknowledgement", async () => {
			...
			try {
				...
				const result = await probe.promptResult(String(response.id));
				expect(result).toMatchObject({ agentInvoked: false, status: "error", sessionSettled: true });
				expect(String(isRecord(result.error) ? result.error.message : undefined)).toContain("SKILL.md");
				expect(probe.frames.indexOf(result)).toBeGreaterThan(probe.frames.indexOf(failure));
		...
		test("aborts the active turn even when replacement is handled and preserves generated replacement work", async () => {
			...
			try {
				...
				expect(await probe.promptResult(String(replaced.id))).toMatchObject({
					agentInvoked: false,
					status: "completed",
					sessionSettled: true,
				});
				...
				await probe.finish(request);
				expect(await probe.promptResult(String(generated.id))).toMatchObject({
					agentInvoked: true,
					status: "completed",
					sessionSettled: true,
				});
		...
		test("a later abort invalidates a replacement already waiting for abort cleanup", async () => {
			...
			try {
				...
				// Background bash is a reader handshake proving both abort commands already settled.
				// abort_and_prompt's own ack still waits on its abort, but a context hook now fails
				// closed on the turn's abort signal instead of blocking cleanup on it (docs/extensions.md),
				// so this turn's abort finishes fast and the replacement's response may already be here.
				await probe.command({ type: "bash", command: "true" });
				release();
				...
				expect(await probe.promptResult(replacement)).toMatchObject({
					agentInvoked: false,
					status: "aborted",
					sessionSettled: true,
				});
		...
		test("only the newest replacement survives overlapping abort-and-prompt cleanup", async () => {
			...
			try {
				...
				await probe.command({ type: "bash", command: "true" });
				release();
				expect(await probe.promptResult(stale)).toMatchObject({
					agentInvoked: false,
					status: "aborted",
					sessionSettled: true,
				});
		...
		for (const decision of ["commit", "cancel"] as const) {
			test(`suspends pending input during a session transition that will ${decision}`, async () => {
				...
				try {
					...
					expect(await probe.promptResult(String(armed.id))).toMatchObject({
						agentInvoked: false,
						status: "completed",
						sessionSettled: true,
					});
...
test("serializes real abort cleanup before queued steering or later native input can resume", async () => {
	...
	try {
		...
		// The retained steer still resumes the session, so it is not settled yet.
		expect(await probe.promptResult(stale)).toMatchObject({
			agentInvoked: false,
			status: "aborted",
			sessionSettled: false,
