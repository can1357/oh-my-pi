...
describe("models.yml provider OAuth", () => {
	...
	test("allows configured OAuth in the user models.yml without an explicit option", async () => {
		const authStorage = await AuthStorage.create(":memory:");
		const agentDir = TempDir.createSync("@models-config-oauth-agent-dir-");
		const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
		try {
			setAgentDir(agentDir.path());
			await writeModelsConfig(agentDir);
			const registry = new ModelRegistry(authStorage, path.join(agentDir.path(), "models.yml"));
			expect(registry.getError()).toBeUndefined();
			expect(getOAuthProvider(PROVIDER)?.name).toBe("Configured OAuth");
		} finally {
	...
	test("fails login when a command-backed client ID resolves empty", async () => {
		...
		try {
			...
			).rejects.toThrow(/oauth\.clientId could not be resolved/);
