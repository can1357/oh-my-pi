...
import { cfgTtsr, type TtsrSettings } from "@oh-my-pi/pi-coding-agent/export/ttsr-settings";
import { TtsrManager } from "@oh-my-pi/pi-coding-agent/export/ttsr";
import { cfgMemoriesEnabled } from "@oh-my-pi/pi-coding-agent/memories/settings";
...
describe("/reload-settings slash command", () => {
	...
	it("applies an on-disk edit and reports the changed setting", async () => {
		...
		expect(cfgAdvisorSyncBacklog.get(settings)).toBe("1");
	...
	it("reconciles session-owned advisor and queue-mode settings without promoting them into config", async () => {
		...
		// The handles write through Settings.writeValue; a "global" layer call is
		// exactly the promotion into config.yml this reconcile must avoid.
		const writeSpy = vi.spyOn(settings, "writeValue");
		...
		for (const [setting, , layer] of writeSpy.mock.calls) {
			if (layer !== "global") continue;
			expect(["steeringMode", "followUpMode", "interruptMode"]).not.toContain(setting.id);
		}
	...
	it("never installs layers older than a mutation that lands mid-reload", async () => {
		...
		cfgAdvisorSyncBacklog.set(settings, "3");
		await reload;
	...
	it("pushes a reloaded ttsr manager setting into the live manager", async () => {
		...
		expect(updateTtsrSettings).toHaveBeenCalledWith(cfgTtsr.get(settings));
	});
	...
	it("reconciles agent-owned request options without persisting them", async () => {
		...
		for (const [setting, , layer] of writeSpy.mock.calls) {
			if (layer !== "global") continue;
			expect(["temperature", "omitThinking"]).not.toContain(setting.id);
		}
	});
	...
	it("re-resolves the scope only after the reloaded settings take effect", async () => {
		...
		await runCommand(settings, {
			refreshScopedModels: vi.fn(async () => {
				seenDuringScopeRefresh = cfgEnabledModels.get(settings);
				return false;
			}),
		});
	...
	it("applies a persisted value that appears over a host-default override", async () => {
		...
		// gets a soft pin of the declared default (registry `pinDefault`), which
		// holds as a runtime override until an explicit persisted value appears.
		const hostDefault = cfgMemoriesEnabled.default;
		cfgMemoriesEnabled.pinDefault(settings);

		const persisted = !hostDefault;
		await writeSettings({ advisor: { syncBacklog: "2" }, memories: { enabled: persisted } });
		const { output } = await runCommand(settings);

		expect(cfgMemoriesEnabled.get(settings)).toBe(persisted);
	...
	it("re-buckets ttsr rules when ttsr.enabled flips on during reload", async () => {
		...
		const manager = new TtsrManager(cfgTtsr.get(settings));
...
describe("TtsrManager.updateSettings", () => {
	// cfgTtsr.get(...) always fills every key of the ttsr group, so the fixture
	// mirrors the complete shape the handler passes through.
	...
	it("re-merges defaults, adopts reloaded manager settings, and reports the change", () => {
		...
		// Manager-level keys the override does not touch keep the group's values.
