...
/** A saved role preset as consumed by the hub: role selectors plus an optional captured fallback-chain snapshot. */
export interface ModelRolePresetView {
...
export interface ModelHubSource extends ModelBrowserSource {
	...
	/** Global settings-layer `retry.fallbackChains` (unsanitized); the layer presets capture and restore. */
	readonly globalFallbackChains: unknown;
	readonly modelRoleStorage: "global" | "project";
	readonly cycleOrder: readonly string[];
	readonly presetsApplyOnSelect: boolean;
	readonly presetsAutoSave: boolean;
	readonly presetsKeepRolesWhenUnset: boolean;
	/** Global settings-layer default thinking level, used for captured-Auto dirty detection. */
	readonly globalDefaultThinkingLevel: string;
	...
	formatWithRouting(model: Model): string;
	/** Filter available models by the configured `enabledModels` patterns (identity when none). */
	filterEnabledModels(available: Model[]): Model[];
	/** Whether `name` is a legal saved-preset name (rejects the reserved `default`). */
	isValidPresetName(name: string): boolean;
	/** Roles a preset assigns: its built-in set plus custom keys, optionally folding in `extraRoleKeys`. */
	presetRoles(profile: ModelRolePresetView | undefined, extraRoleKeys?: Iterable<string>): string[];
	/** OMP's curated built-in Default preset for `model`. */
	buildBuiltInDefaultPreset(model: Model, available: readonly Model[]): ModelRolePresetView;
	presetNames(model: Model): string[];
	preset(model: Model, name: string): ModelRolePresetView | undefined;
	presetDefault(model: Model): ModelRolePresetView | undefined;
	presetDefaultName(model: Model): string | undefined;
	presetKeyExists(model: Model, name: string): boolean;
	/** Global-layer preset lookups, used only for chain/default dirty detection. */
	globalPreset(model: Model, name: string): ModelRolePresetView | undefined;
	globalPresetDefault(model: Model): ModelRolePresetView | undefined;
	globalPresetDefaultName(model: Model): string | undefined;
}
...
export class ModelHubComponent implements Component {
	...
	#syncFromRegistryState(): void {
		...
		if (this.#scopedModels.length > 0) {
			...
			try {
				availableModels = this.#settings.filterEnabledModels(this.#registry.getAvailable());
			} catch (error) {
				this.#configError = error instanceof Error ? error.message : String(error);
				availableModels = [];
			}
	...
	#globalFallbackChains(): Record<string, string[]> {
		try {
			const chains = this.#settings.globalFallbackChains;
			if (!chains || typeof chains !== "object" || Array.isArray(chains)) return {};
			const sanitized: Record<string, string[]> = {};
			for (const key in chains) {
				const chain = (chains as Record<string, unknown>)[key];
	...
	#buildRolesRows(availableModels: ReadonlyArray<Model>): void {
		...
		const presetDefaultIsEditable =
			this.#settings.modelRoleStorage !== "project" || this.#settings.getModelRoleProvenance("default") !== "global";
		if (
			this.#activePreset &&
			(!presetDefaultIsEditable ||
				!defaultModel ||
				this.#activePreset.model.provider !== defaultModel.provider ||
				this.#activePreset.model.id !== defaultModel.id ||
				this.#settings.formatWithRouting(this.#activePreset.model) !==
					this.#settings.formatWithRouting(defaultModel))
		) {
			this.#activePreset = undefined;
			this.#activePresetDirty = false;
			this.#activePresetManuallyDirty = false;
		}
		if (defaultModel && presetDefaultIsEditable) {
			const defaultName = this.#settings.presetDefaultName(defaultModel);
			const storedRoles =
				this.#settings.modelRoleStorage === "project"
					? this.#settings.getProjectModelRoles()
					: this.#settings.getGlobalModelRoles();
			if (!this.#activePreset) {
				const configuredDefault = this.#settings.presetDefault(defaultModel);
				const useBuiltInDefault = configuredDefault === undefined && this.#settings.presetsApplyOnSelect;
			...
			const activeProfile = active?.useBuiltInDefault
				? this.#settings.buildBuiltInDefaultPreset(defaultModel, availableModels)
				: active?.name === undefined
					? this.#settings.presetDefault(defaultModel)
					: this.#settings.preset(defaultModel, active.name);
			const presetScope = this.#settings.modelRoleStorage === "project" ? "project" : "global";
			const keepUnsetRoles = this.#settings.presetsKeepRolesWhenUnset;
			const selected = this.#settings.formatWithRouting(defaultModel);
			...
			const profileRoles = this.#settings.presetRoles(
				activeProfile,
				...
				activeProfile === undefined
					...
					: (Object.fromEntries(
							profileRoles.map(role => {
								...
								const candidate = this.#settings.resolveRoleValue(
									value,
			...
			const globalProfile = active?.useBuiltInDefault
				? undefined
				: active?.name === undefined
					? this.#settings.globalPresetDefaultName(defaultModel)
						? undefined
						: this.#settings.globalPresetDefault(defaultModel)
					: this.#settings.globalPreset(defaultModel, active.name);
			...
			for (const name of this.#settings.presetNames(defaultModel)) {
	...
	#unassignRole(role: string): void {
		...
		const scope = this.#settings.modelRoleStorage === "project" && source !== "default" ? source : undefined;
		this.#mutateRole(role, scope, () => this.#callbacks.onUnassign(role, scope));
		this.#refreshAfterMutation();
	}
	...
	#notePresetRoleEdit(role: string, scope?: ModelRoleSelectionScope): () => void {
		...
		const active = this.#activePreset;
		return () => {
			if (editsActivePresetScope && active && this.#activePreset === active) {
	...
	#saveActivePreset(automatic: boolean): void {
		if (automatic && !this.#settings.presetsAutoSave) return;
		const active = this.#activePreset;
		const defaultModel = this.#roles.default?.model;
		if (
			!active ||
	...
	#openRoleStrip(item: ModelBrowserItem): void {
		...
		for (const name of this.#settings.presetNames(item.model)) {
			chips.push({
				label: `preset: ${name}`,
	...
	#activateStripChip(): void {
		...
		switch (chip.action) {
			...
			case "unassign":
				if (chip.role) {
					const scope = this.#settings.modelRoleStorage === "project" ? chip.scope : undefined;
			...
			case "thinking":
				if (strip.role && chip.thinkingLevel !== undefined) {
					...
					if (changed) {
						const commitPresetEdit = this.#notePresetRoleEdit(role, strip.scope);
	...
	#setFallbackChain(role: string, chain: string[]): void {
		...
		if (active && !active.useBuiltInDefault) {
			const profile =
				active.name === undefined
					? this.#settings.presetDefault(active.model)
					: this.#settings.preset(active.model, active.name);
	...
	#submitPresetName(): void {
		...
		if (strip.renameFrom !== undefined) {
			...
			// no-op rename.
			if (name !== strip.renameFrom && this.#settings.presetKeyExists(strip.item.model, name)) {
				return;
			}
			...
			if (
				this.#settings.presetKeyExists(strip.item.model, name) &&
				(strip.renameFrom === name || !this.#settings.presetKeyExists(strip.item.model, strip.renameFrom))
			) {
	...
	#applyPreset(model: Model, name: string | undefined, builtInDefault?: boolean, replaceUnsetRoles = false): void {
		...
		const useBuiltInDefault =
			builtInDefault ??
			(name === undefined &&
				(this.#settings.presetDefaultName(model) !== undefined ||
					this.#settings.presetDefault(model) === undefined));
	...
	#discardActivePreset(target?: { model: Model; name: string | undefined }): boolean {
		...
		const saved =
			active.name === undefined
				? this.#settings.presetDefault(active.model)
				: this.#settings.preset(active.model, active.name);
		if (!saved && !active.useBuiltInDefault && !this.#settings.presetsApplyOnSelect) {
			this.#activePresetDirty = false;
			this.#activePresetManuallyDirty = false;
			return true;
		}
