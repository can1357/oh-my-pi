...
describe("ProcessTerminal OSC 11 appearance detection", () => {
	...
	it("kitty keyboard probe owns its own DA1 sentinel — does not consume OSC 11's", () => {
		...
		// the DECRQM probes for DEC 2026, 2048, 2031, 1010, and 1011 (each
...
describe("ProcessTerminal DECRQM + in-band resize (DEC 2026/2048)", () => {
	...
	function setup(options?: TerminalStartOptions) {
		const writes: string[] = [];
		const received: string[] = [];
		let resizeCount = 0;
	...
	it("routes an unbracketed multiline burst by the event-loop stall probe, even on a bracketing terminal (#12540, #13344)", () => {
		let loopBlocked = false;
		const { terminal, received } = setup({ isLoopBlocked: () => loopBlocked });
		// The terminal brackets pastes, but input-method commits still arrive as
		// typed text: on a responsive loop the commit must land as one insert.
		...
		// The same bytes drained by a stall are typed keys: every Enter submits.
		loopBlocked = true;
		received.length = 0;
