...
/**
 * Native addon loader for `@pk-nerdsaver-ai/pi-natives`.
 *
...
function getNativesDir() {
	...
	return path.join(os.homedir(), ".ompk", "natives");
}

function resolveLeafPackageDir(platformTag) {
	try {
		const require_ = createRequire(import.meta.url);
		return path.dirname(require_.resolve(`@pk-nerdsaver-ai/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * into the per-version cache directory (`~/.ompk/natives/<version>/`) before loading.
 *
 ...
 * `node_modules/@pk-nerdsaver-ai/pi-natives/native/`, leaving an old binary next to a
