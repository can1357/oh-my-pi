...
- Fixed a termination reporting success over children it never captured, when its root exited while the descendant walk was running. The root's liveness was demanded before the walk and not after, and a walk rooted at a pid that has since exited answers empty *and whole* — its children were reparented out of the subtree — so the plan kept `live_at_capture: true`, signalled a corpse, and ended with nothing to say the survivors were missed. Liveness is now demanded after the walk as well, in the graceful capture and in the hard-kill sweep alike; the sweep had no such check at all, and its own caller's check runs before the native walk begins, so it could not cover a root that died inside one. The sweep reports that as its own failure rather than as a walk that stopped short, because the enumeration did not stop short. It is demanded of a group capture too: the group answers for its own members, but a descendant that left it under `setsid(2)` is in neither the group nor a walk the root can no longer hold. Windows is exempt, and deliberately so — its walk is one Toolhelp snapshot keyed on the parent pid, the kernel does not reparent, and the retained root handle keeps the pid from being reused underneath the snapshot, so sweeping a dead root's tree there is the intended behaviour rather than a race. The exemption is not "snapshot walks are safe": the macOS walk is also a snapshot, but of current parentage, so it loses a reparented child exactly as the Linux walk does. The cost on the non-Windows hosts is a root that exits just after a walk that did see everything, which is now reported incomplete although nothing was missed. The hard wave's rescan deliberately keeps no such demand: the polite wave signals the root, so a dead root by then is the ordinary successful path and demanding liveness would mark nearly every graceful termination incomplete. That leaves one case open — a descendant spawned during the grace window and reparented by the root's own exit before the rescan is in neither the signal nor the wait. It is not closable by looking harder: a delivered `TERM` does not prove the exit that followed was its doing, and once the root is gone the kernel has discarded the ancestry. Reaching that descendant needs containment the walk does not have, a cgroup or a subreaper or the process group.
- Fixed the tree sweep dropping the members of a group it could not prove ownership of. Where a signal cannot be scoped to the leader's retained pidfd, the pgid number is the only handle and ownership has to be proved from the leader's pid. The sweep asked, and on a negative answer silently continued with the descendant walk alone: a same-group survivor reparented out of that walk was then neither signalled nor waited on, and `killTreeAndWait()` reported the tree gone over a process still running. It now makes the distinction its own group-only counterpart and the `Empty` answer beside it already make — unresolved ownership is not an absent group — refusing when a live member cannot be accounted for, and contributing the scanned members when none of them is still running. The condition is the scope being unavailable rather than any particular version, so it covers macOS and Linux before 6.9 but also a newer kernel where seccomp or an LSM rejects the call. The refusal is deliberately fail-closed: a pgid that has been reused by an unrelated group presents a live member that is not ours, and refusing there fails a termination that had in fact swept its own tree. That case is not separable from our own leader having been reaped while our survivors carry the number, which is the one this exists to catch. One gap is left open on purpose: a process that joins the group after the scan, when nothing the scan found is still running, is neither signalled nor waited on. Broadcasting to cover it makes an ordinary sweep of an emptied group fail instead, because the post-signal re-scan requires a member to find, so closing it means teaching that re-scan to tell an emptied group from an unreadable one.
...
- On Linux, `Process::from_pid` opens the pidfd and only then reads the start time out of `/proc/<pid>/stat` — by number, not out of the reference it just opened. A pid recycled between the two syscalls leaves the reference holding the exited process's pidfd and the replacement's start time, so it claims the replacement's identity in every `is_same_process` comparison while every signal and every status poll goes to the process that already exited. An identity-keyed dedup then keeps that reference and drops a genuine one for the replacement, which is the defect the dedup exists to prevent, arriving from the constructor instead of the walk. Pre-existing, unchanged from `daf07999c2`, and Linux-only: Windows reads the creation time from the handle it opened, and macOS holds no reference and re-checks the identity by number immediately before each signal.
