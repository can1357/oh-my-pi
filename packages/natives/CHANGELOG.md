...
- Fixed a termination plan reporting success over children it never captured, when its root exited while the descendant walk was running. The root's liveness was demanded before the walk and not after, and a walk rooted at a pid that has since exited answers empty *and whole* — its children were reparented out of the subtree — so the plan kept `live_at_capture: true`, signalled a corpse, and ended with nothing to say the survivors were missed. Liveness is now demanded on both sides, which is the rule the LSP mux already applies around its own subtree walk. It is demanded of a group capture too: the group answers for its own members, but a descendant that left it under `setsid(2)` is in neither the group nor a walk the root can no longer hold, so holding a group is not evidence the tree was seen whole. The cost is a capture whose root exits just after a walk that did see everything, which is now reported incomplete although nothing was missed.
