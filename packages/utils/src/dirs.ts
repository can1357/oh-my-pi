 ...
 *
 * Uses PI_CONFIG_DIR (default ".ompk") for the config root and
 ...
 *
 * Exported so consumers of `@pk-nerdsaver-ai/pi-utils/dirs` (CLI bootstrap, tests,
...
/** Get the config directory name relative to home (e.g. ".ompk" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".ompk/agent" or PI_CONFIG_DIR + "/agent"). */
...
class DirResolver {
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// profile could resolve to `~/.ompk/profiles/<name>` on first activation
		...
		// XDG flattens the agent/ prefix: ~/.ompk/agent/sessions → $XDG_DATA_HOME/omp/sessions
		this.#agentDirs = {
 ...
 * `~/.ompk/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.ompk). */
...
/** Get the agent config directory (~/.ompk/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.ompk). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.ompk/*)
// =============================================================================

/** Get the reports directory (~/.ompk/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.ompk/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get the path to a dated log file (~/.ompk/logs/omp.YYYY-MM-DD.log). */
...
/**
 * Get the plugins directory (~/.ompk/plugins or its XDG equivalent).
 *
 ...
 * fall back to `OMP_WORKTREE_DIR` or the `~/.ompk/wt` default.
 *
 ...
 * `worktree.base` setting), then the `~/.ompk/wt` default. The env var and the
...
/** Get the SSH control socket directory (~/.ompk/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.ompk/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.ompk/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.ompk/agent/python-gateway; XDG default: $XDG_STATE_HOME/omp/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.ompk/puppeteer). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.ompk/wt/`).
...
/** Get the path to a single worktree directory (~/.ompk/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.ompk/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.ompk/cache/github-cache.db).
...
/**
 * Get the encrypted auth-broker snapshot cache path (~/.ompk/cache/auth-broker-snapshot.enc).
...
/** Get the local FastEmbed model cache directory (~/.ompk/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.ompk/cache/fastembed-runtime). */
...
/** Get the stats database path (~/.ompk/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.ompk/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.ompk/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.ompk/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.ompk/autoresearch/<encoded-project>/runs/<runId>). */
...
// =============================================================================
// Agent subdirectories (~/.ompk/agent/*)
// =============================================================================
...
/** Get the last-seen-changelog-version marker file (~/.ompk/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.ompk/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.ompk/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/omp/cache/document-conversions). */
...
/**
 * Persistent per-install UUID stored at `~/.ompk/install-id`.
 *
 ...
 * `~/.ompk/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.ompk/install-id`) regardless of the
