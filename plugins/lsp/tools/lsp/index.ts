...
const LANGUAGE_MAP: Record<string, string> = {
   ...
   ".go": "go", ".rs": "rust", ".py": "python", ".c": "c", ".cpp": "cpp", ".h": "c", ".hpp": "cpp",
   ".java": "java", ".rb": "ruby", ".lua": "lua", ".sh": "shellscript", ".zig": "zig",
};
...
const factory: CustomToolFactory = (pi) => {
               ...
               textDocument: {
                  ...
                  codeAction: {
                     codeActionLiteralSupport: {
                        codeActionKind: { valueSet: [CodeActionKind.QuickFix, CodeActionKind.Refactor, CodeActionKind.Source] },
                     },
   ...
   const handleCodeActions = async (pos: FilePosition, endLine?: number, endChar?: number, kind?: string, apply?: number): Promise<string> => {
      const absPath = path.resolve(pi.cwd, pos.file);
      const client = await getClientForFile(absPath);
      await ensureFileOpen(client, absPath);
      await refreshFile(client, absPath);
      ...
      if (apply !== undefined) {
         ...
         if (!action.edit && client.capabilities.codeActionProvider && typeof client.capabilities.codeActionProvider === "object" && client.capabilities.codeActionProvider.resolveProvider) {
         ...
         if (action.edit) {
            ...
            await client.connection.sendRequest(ExecuteCommandRequest.type, { command: action.command.command, arguments: action.command.arguments });
   ...
   const handleHover = async (pos: FilePosition): Promise<string> => {
      ...
      if (Array.isArray(hover.contents)) return hover.contents.map((c: string | { value: string }) => typeof c === "string" ? c : c.value).join("\n\n");
   ...
   const handleSymbols = async (file: string): Promise<string> => {
      ...
      const symbols = await client.connection.sendRequest(DocumentSymbolRequest.type, { textDocument: { uri: fileToUri(absPath) } });
   ...
   return {
      ...
      async execute(toolCallId, args) {
         ...
         try {
            ...
            switch (p.action) {
               ...
               case "references":
                  if (!p.file || !p.line || !p.character) return { content: [{ type: "text", text: "Error: file, line, character required" }] };
               ...
               case "rename":
                  if (!p.file || !p.line || !p.character || !p.new_name) return { content: [{ type: "text", text: "Error: file, line, character, new_name required" }] };
                  result = await handleRename({ file: p.file, line: p.line, character: p.character }, p.new_name, p.apply === true);
                  break;

               case "actions":
                  if (!p.file || !p.line || !p.character) return { content: [{ type: "text", text: "Error: file, line, character required" }] };
                  result = await handleCodeActions({ file: p.file, line: p.line, character: p.character }, p.end_line, p.end_character, p.kind, typeof p.apply === "number" ? p.apply : undefined);
                  break;
      ...
      renderResult(result, { expanded }, theme) {
         ...
         let summary = hasError && !hasSuccess ? theme.fg("error", lines[0])
            : hasSuccess && !hasError ? theme.fg("success", lines[0])
