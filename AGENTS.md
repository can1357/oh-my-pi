...
- **Run the tests for what you touched before committing** — CI must never be the first run. Find the bucket that owns a suite with `bun scripts/ci-test-ts.ts <bucket> --dry-run` (buckets: `coding-agent-runtime`, `coding-agent-ui`, `coding-agent-singleton`, `coding-agent-native`, `coding-agent-heavy`, `workspace`, `native`), then run it: `bun run ci:test:coding-agent:runtime` etc. `bun run test` is the full local gate (every TS bucket plus Rust). A bare `bun test <files>` inside the package is the inner loop, not the gate: it skips the sibling suites that share the chunk's process and the env scrubbing/GC knobs `ci-test-ts.ts` applies.
...
- **Never assert a latency snapshot.** APIs that deliberately bound their startup wait return partial state by design — `MCPManager.connectServers` gives up after `STARTUP_TIMEOUT_MS` (250 ms) and finishes connecting in a background continuation. Asserting on whatever happened to be ready when such a call returns (`result.connectedServers`, or `getTools()`/`getConnection()`/`getServerResources()` read right after it) passes on an idle dev box and fails on a loaded CI runner. Await the real completion signal instead — `waitForConnection`, the `connected` status event, `refreshServerTools` (see `test/helpers/mcp-connect.ts`) — and prove it by setting the budget to 0 locally: the assertion must still hold.
