...
fn render_change(
	...
	if change
		...
		|| change
			...
			.is_some_and(|mode| mode.kind() == gix::objs::tree::EntryKind::Commit)
	{
		return render_gitlink(repo, change);
	}
	let old_kind = change
		.old_mode
		.or(change.new_mode)
...
fn render_gitlink(repo: &gix::Repository, change: &FileChange) -> Result<Rendered> {
	let commit = gix::objs::tree::EntryKind::Commit;
	if change.old_mode.is_some_and(|mode| mode.kind() != commit)
		|| change.new_mode.is_some_and(|mode| mode.kind() != commit)
	{
		return Err(Error::backend("git diff", "cannot render a gitlink changing to a non-gitlink"));
	}
	let mut text = String::new();
	text.push_str("diff --git a/");
	text.push_str(&change.old_path);
	...
	let null = repo.object_hash().null();
	match (change.old_id != null, change.new_id != null) {
		(false, true) => {
			text.push_str("--- /dev/null\n+++ b/");
			text.push_str(&change.new_path);
			text.push_str("\n@@ -0,0 +1 @@\n+Subproject commit ");
			text.push_str(&change.new_id.to_string());
			text.push('\n');
			Ok(Rendered { text, added: Some(1), removed: Some(0) })
		},
		(true, false) => {
			text.push_str("--- a/");
			text.push_str(&change.old_path);
			text.push_str("\n+++ /dev/null\n@@ -1 +0,0 @@\n-Subproject commit ");
			text.push_str(&change.old_id.to_string());
			text.push('\n');
			Ok(Rendered { text, added: Some(0), removed: Some(1) })
		},
		(true, true) => {
			text.push_str("--- a/");
			text.push_str(&change.old_path);
			...
			text.push_str(&change.new_id.to_string());
			text.push('\n');
			Ok(Rendered { text, added: Some(1), removed: Some(1) })
		},
		(false, false) => Err(Error::backend("git diff", "gitlink change has no object")),
	}
}
