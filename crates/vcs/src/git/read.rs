...
impl GitRepo {
	...
	pub fn submodule_paths(&self) -> Result<Vec<String>> {
		...
		while let Some((root, prefix)) = queue.pop_front() {
			let Some(repository) = Self::discover(&root)? else {
				continue;
			};
			let repo = repository.gix_fresh()?;
			let index = repo
				.index_or_empty()
				.map_err(|err| Error::backend("git submodules", err))?;
			for entry in index
				.entries()
				.iter()
...
mod tests {
	...
	fn read_objects_logs_and_refs() -> TestResult {
		...
		git(dir.path(), &["tag", "v1.9"])?;
		git(dir.path(), &["tag", "v1.10"])?;

		assert_eq!(repo.resolve_ref("main")?, Some(second.clone()));
		assert!(repo.ref_exists("refs/heads/main")?);
		assert_eq!(repo.tags_at("HEAD")?, vec!["v1.10", "v1.9", "v1.2.0"]);
