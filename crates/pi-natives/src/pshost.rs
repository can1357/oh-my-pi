...
impl HostCore {
	...
	async fn exec(
		...
		if env.as_ref().is_some_and(|entries| {
			entries
				.keys()
			...
			return Err(Error::from_reason(
				"PowerShell environment variable names must be non-empty and cannot contain '='",
			));
		}
...
fn encode_frame<T: Serialize>(msg: &T) -> Result<Vec<u8>> {
	...
	if json.len() > MAX_FRAME_BYTES {
		return Err(Error::from_reason("PowerShell request exceeds the 64 MiB protocol frame limit"));
	}
...
mod tests {
	...
	async fn persists_state_and_maps_cancellation() {
		...
		let (_, res) = run_cmd(&host, path_exit_cmd, task::CancelToken::default()).await;
		assert_eq!(res.exit_code, Some(7), "repeated identical path-invoked native exit is reported");
