...
function Write-Chunk([int] $Id, [string] $Stream, [string] $Text) {
    ...
    $sliceChars = 4194304
    for ($offset = 0; $offset -lt $Text.Length; $offset += $len) {
        $len = [Math]::Min($sliceChars, $Text.Length - $offset)
        if ($offset + $len -lt $Text.Length -and [char]::IsHighSurrogate($Text[$offset + $len - 1]) -and [char]::IsLowSurrogate($Text[$offset + $len])) {
            $len--
        }
