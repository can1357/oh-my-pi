...
mod format_modifiers {
	...
	//!
	//! This module implements GNU-compatible format modifiers for date
	//! formatting. These modifiers extend standard strftime format specifiers
	//! with optional width and flag modifiers.
	//!
	//! ## Syntax
	//!
	...
	/// returning `FieldWidthTooLarge` on arithmetic overflow or allocation
	/// failure.
	fn try_alloc_padded(
		current_len: usize,
		padding: usize,
...
#[allow(clippy::cognitive_complexity, reason = "date option parsing follows GNU date semantics")]
fn date_main(host: &mut Host, matches: &ArgMatches) -> Result<(), DateError> {
	...
	// StreamWriter is owned (no `host` borrow), so it may be created before
	// `Dates::Stdin` takes `&mut host.stdin`.
	let mut stdout = host.stdout_writer();
	let dates = match &settings.date_source {
		...
		DateSource::File(path) => {
			...
			Dates::File(parse_dates_from_reader(
				file,
				&now,
				DebugOptions::new(settings.debug, true),
				reader_stderr,
...
type DateResult = Result<Zoned, (String, parse_datetime::ParseDateTimeError)>;

struct DateReader<'a, R, W> {
	reader:   BufReader<R>,
	line:     String,
	now:      &'a Zoned,
...
impl<R: Read, W: Write> Iterator for DateReader<'_, R, W> {
	type Item = DateResult;
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		match self.reader.read_line(&mut self.line) {
			...
			Ok(_) => {
				...
				Some(parse_date(self.line.as_str(), self.now, self.dbg_opts, &mut self.error))
			},
		}
	}
}
...
/// inputs.
fn parse_dates_from_reader<R: Read, W: Write>(
	reader: R,
	now: &Zoned,
	dbg_opts: DebugOptions,
	error: W,
