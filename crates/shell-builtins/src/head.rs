...
mod parse {
	...
	/// Processes the numeric block of the input string to generate the
	/// appropriate options.
	fn process_num_block(
		...
		// Lowercase suffixes are obsolete byte multipliers; uppercase suffixes
		// scale the line count like the modern `-n NUM<suffix>` form.
	...
	mod tests {
		...
		fn test_parse_numbers_obsolete() {
			...
			assert_eq!(obsolete("-1vzqvq"), obsolete_result(&["-q", "-z", "-n", "1"]));
...
mod take {
	...
	///
	/// If `n` exceeds the number of bytes in the input file then nothing is
	/// copied. If no errors are encountered then the function returns the
	/// number of bytes copied.
	///
	/// Algorithm for this function is as follows...
	/// 1 - Chunks of the input file are read into a queue of [`TakeAllBuffer`]
	/// instances.     Chunks are read until at least we have enough data to
	/// write out the entire contents of the     first [`TakeAllBuffer`] in the
	/// queue whilst still retaining at least `n` bytes in the queue.     If we
	/// hit `EoF` at any point, stop reading. 2 - Assess whether we managed to
	/// queue up greater-than `n` bytes. If not, we must be done, in     which
	/// case break and return. 3 - Write either the full first buffer of data,
	/// or just enough bytes to get back down to having     the required `n`
	/// bytes of data queued. 4 - Go back to (1).
	...
	/// If `n` exceeds the number of lines in the input file then nothing is
	/// copied. The last line in the file is not required to end with a
	/// `separator` character. If no errors are encountered then they function
	/// returns the number of bytes copied.
	///
	/// Algorithm for this function is as follows...
	/// 1 - Chunks of the input file are read into a queue of
	/// [`TakeAllLinesBuffer`] instances.     Chunks are read until at least we
	/// have enough lines that we can write out the entire     contents of the
	/// first [`TakeAllLinesBuffer`] in the queue whilst still retaining at
	/// least     `n` lines in the queue.     If we hit `EoF` at any point, stop
	/// reading. 2 - Asses whether we managed to queue up greater-than `n`
	/// lines. If not, we must be done, in     which case break and return.
	/// 3 - Write either the full first buffer of data, or just enough lines to
	/// get back down to     having the required `n` lines of data queued.
	/// 4 - Go back to (1).
	///
	...
	/// Create an adaptor that will read at most `limit` lines from a given
	/// reader.
	///
...
/// token, so that value is not mistaken for an obsolete `-NUM` form.
...
/// Rewrites obsolete `-NUM[suffix]` tokens before the end-of-options marker.
...
impl Utility for Head {
	...
	fn run(self, host: &mut Host) -> i32 {
		...
		for file in &options.files {
			let result = if file == "-" {
				if print_headers {
					print_header(&mut stdout, b"standard input", &mut first);
				}
				let mut input = io::BufReader::with_capacity(BUF_SIZE, &mut host.stdin);
				match options.mode {
					Mode::FirstBytes(n) => read_n_bytes(&mut input, &mut stdout, n),
					Mode::AllButLastBytes(n) => read_but_last_n_bytes(&mut input, &mut stdout, n),
					Mode::FirstLines(n) => {
						read_n_lines(&mut input, &mut stdout, n, options.line_ending.into())
					},
					Mode::AllButLastLines(n) => {
						read_but_last_n_lines(&mut input, &mut stdout, n, options.line_ending.into())
					},
				}
			} else {
				...
				if resolved.is_dir() {
					if print_headers {
						print_header(&mut stdout, file.as_encoded_bytes(), &mut first);
					}
				...
				head_file(&mut input, &mut stdout, &options)
			};
