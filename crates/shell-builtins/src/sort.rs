...
/// the children are spawned from deep inside the temp-file abstraction, far
/// from the `Host`. Carrying an owned [`ChildEnv`] here is what keeps them
/// resolving the shell's `PATH`, running in the shell's working directory, and
/// reporting on the command's stderr instead of the host process's.
#[derive(Clone)]
...
mod chunks {
	...
	/// that there is still input left: If the input fits _exactly_ into a
	/// buffer, we will only notice that there's nothing more to read at the
	/// next invocation. In case there is no input left, nothing will be sent.
	///
	/// # Arguments
	///
	...
	/// * `carry_over`: The bytes that must be carried over in between
	///   invocations.
	/// * `file`: The current file.
	/// * `next_files`: What `file` should be updated to next.
	/// * `separator`: The line separator.
	/// * `settings`: The global settings.
	#[allow(clippy::too_many_arguments, reason = "chunk reads share parser and spill state")]
	pub fn read<T: Read>(
		sender: &Sender<Chunk>,
	...
	///
	/// This function makes sure that at least two lines are read (unless we
	/// reach EOF and there's no next file), growing the buffer if necessary.
	/// The last line is likely to not have been fully read into the buffer. Its
	/// bytes must be copied to the front of the buffer for the next invocation
	/// so that it can be continued to be read (see the return values and
	/// `start_offset`).
	///
	/// # Arguments
	///
	...
	///   reading.
	/// * `buffer`: The buffer that is filled with bytes. Its contents will
	///   mostly be overwritten (see `start_offset` as well). It will be grown up
	///   to `max_buffer_size` if necessary, but it will always grow to read at
	///   least two lines.
	...
	///   The remaining bytes must be copied to the start of the buffer for the
	///   next invocation, if another invocation is necessary, which is
	///   determined by the other return value.
	...
	fn read_to_buffer<T: Read>(
		...
		loop {
			match file.read(read_target) {
					...
					if read_target.is_empty() {
						...
						if let Some(last_line_end) = sep_iter.next() {
							if found_newline || sep_iter.next().is_some() {
								...
								// We want to include the separator here, because it shouldn't be carried
								// over.
				...
				Err(e) => {
					return Err(SortError::message(e.to_string()));
				},
			}
		}
	}
...
mod ext_sort {
	...
	mod threaded {
		...
		//! Threaded external sort: read input in chunks, sort them in a
		//! background thread, and spill to temporary files when memory is
		//! exceeded.
		...
		/// Sort files by using auxiliary files for storing intermediate chunks
		/// (if needed), and output the result.
		///
		/// Two threads cooperate: one reads input and writes temporary chunk
		/// files, while the other sorts each chunk in memory. Once all chunks
		/// are written, they are merged back together for final output.
		pub fn ext_sort(
		...
		enum ReadResult<I: WriteableTmpFile> {
			...
			/// The input was read into multiple chunks, which were written to
			/// auxiliary files.
		...
		/// `compress` optionally pipes file contents through
		/// `--compress-program`.
		...
		mod tests {
			...
			/// the sorter thread after `read_write_loop`. A tiny explicit
			/// buffer forces spilling to temporary files, so the join runs on
			/// the `WroteChunksToFile` path — it must surface sorted output
			/// rather than deadlock or truncate.
			#[test]
	...
	mod wasi {
		...
		//! WASI single-threaded sort: read all input into memory, sort, and
		//! output. Threads are not available on WASI, so we bypass the
		...
		/// Sort files by reading all input into memory, sorting in a single
		/// thread, and outputting directly.
		pub fn ext_sort(
			files: &mut impl Iterator<Item = SortResult<Box<dyn Read + Send>>>,
			settings: &GlobalSettings,
			output: Output,
...
mod merge {
	...
	//! but only one channel from the sorter back to the reader. The channels to
	//! the sorter are used to send the read chunks. The sorter reads the next
	//! chunk from the channel whenever it needs the next chunk after running
	//! out of lines from the previous read of the file. The channel back from
	//! the sorter to the reader has two purposes: To allow the reader to reuse
	//! memory allocations and to tell the reader which file to read from next.
	...
	/// If the output file occurs in the input files as well, copy the contents
	/// of the output file and replace its occurrences in the inputs with that
	/// copy.
	...
	/// Determine the effective merge batch size, enforcing a minimum and
	/// respecting the file-descriptor soft limit after reserving stdio/output
	/// and a safety margin.
	...
	pub struct WriteableCompressedTmpFile {
		...
		/// Drains the compressor's stderr onto the command's; joined once the
		/// child has exited so its diagnostics land before we report a result.
	...
	impl WriteableTmpFile for WriteableCompressedTmpFile {
		...
		fn create((file, path): (File, PathBuf), compress: Option<&Compressor>) -> SortResult<Self> {
			...
			Ok(Self { path, compress, child, child_stdin: BufWriter::new(child_stdin), forwarder })
		}
	...
	impl MergeInput for CompressedTmpMergeInput {
		...
		fn finished_reading(self) -> SortResult<()> {
			...
			#[allow(clippy::drop_non_drop, reason = "explicitly closes stdout before waiting")]
...
mod numeric_str_cmp {
	...
	//! Fast comparison for strings representing a base 10 number without
	//! precision loss.
	//!
	//! To be able to short-circuit when comparing, [`NumInfo`] must be passed
	//! along with each number to [`numeric_str_cmp`]. [`NumInfo`] is generally
	//! obtained by calling [`NumInfo::parse`] and should be cached. It is
	//! allowed to arbitrarily modify the exponent afterward, which is
	//! equivalent to shifting the decimal point.
	//!
	//! More specifically, exponent can be understood so that the original number
	//! is in `(1..10)*10^exponent`. From that follows the constraints of this
	...
	impl NumInfo {
		...
		/// idx is the char after the last zero. If the input is not a number
		/// (which has to be treated as zero), the returned empty range will be
		/// 0..0.
		#[allow(clippy::cognitive_complexity, reason = "numeric parsing preserves sort's grammar")]
					...
					return if let Some(start) = start {
						...
						(
							...
							if had_digit {
								...
								// For debug output to work properly, we have to match the character after
								// the last zero.
								idx..idx
							} else {
	...
	/// Compare two numbers as strings without parsing them as a number first.
	/// This should be more performant and can handle numbers more precisely.
...
#[expect(
	...
	reason = "thiserror requires references in format arguments"
)]
...
fn detect_numeric_locale() -> NumericLocaleSettings {
	NumericLocaleSettings { decimal_pt: Some(b'.'), thousands_sep: None }
}
...
pub(crate) fn current_open_fd_count(allow_proc_self: bool) -> Option<usize> {
	...
	for fd in 0..limit {
		...
		// SAFETY: `fcntl` only probes the descriptor and does not take ownership of it.
...
#[allow(
	...
	reason = "sort orchestration handles all option interactions"
)]
fn uu_sort(
	host: &mut Host,
	matches: &ArgMatches,
...
#[allow(
	clippy::cognitive_complexity,
	reason = "numeric prefix parsing handles locale-sensitive forms"
)]
