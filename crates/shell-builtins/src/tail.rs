...
mod args {
	...
	impl Settings {
		...
		/// Verify [`Settings`] and try to find unsolvable misconfigurations of
		/// tail originating from user provided command line arguments. In
		/// contrast to [`Settings::check_warnings`] these misconfigurations
		...
		Command::new("tail")
			...
			.about(
				"Print the last 10 lines of each FILE to standard output.\nWith more than one FILE, \
				 ...
				 read standard input.\nMandatory arguments to long flags are mandatory for short \
				 flags too.",
			)
			...
			.arg(
				Arg::new(options::MAX_UNCHANGED_STATS)
					...
					.help(
						"Reopen a FILE which has not changed size after N (default 5) iterations to see \
						 if it has been unlinked or renamed (this is the usual case of rotated log \
						 files); This option is meaningful only when polling (i.e., with --use-polling) \
...
mod chunks {
	...
	//! Iterating over a file by chunks, either starting at the end of the file
	//! with [`ReverseChunks`] or at the end of piped stdin with [`LinesChunk`]
	//! or [`BytesChunk`].
	//!
	...
	/// value is constantly set to 8192 on all platforms, where the value in
	/// stdio.h is determined on each platform differently. Since libc chose
	/// 8192 as a reasonable default the value here is set to this value, too.
	pub const BUFFER_SIZE: usize = 8192;

	/// An iterator over a file in non-overlapping chunks from the end of the
	/// file.
	///
	...
	pub struct BytesChunk {
		...
		/// what makes it necessary to track the number of stored bytes. The
		/// choice of usize is sufficient here, since the number of bytes max
		/// value is [`BUFFER_SIZE`], which is a usize.
		bytes: usize,
	}
	...
	impl BytesChunk {
		...
		/// Create a new chunk from an existing chunk. The new chunk's buffer will
		/// be copied from the old chunk's buffer, copying the slice
		/// `[offset..old_chunk.bytes]` into the new chunk's buffer but starting
		/// at 0 instead of offset. If the offset is larger or equal to
		/// `chunk.lines` then a new empty `BytesChunk` is returned.
		///
		/// # Arguments
		///
		/// * `chunk`: The chunk to create a new `BytesChunk` chunk from
		/// * `offset`: Start to copy the old chunk's buffer from this position.
		///   May not be larger than `chunk.bytes`.
		///
		/// # Examples
		///
		/// ```rust,ignore
		...
		/// Receive the internal buffer safely, so it returns a slice only
		/// containing as many bytes as large the `self.bytes` value is.
		///
	...
	pub struct BytesChunkBuffer {
		...
		/// [`Self::chunks`]. Use u64 here to support files > 4GB on 32-bit
		/// systems. Note, this differs from `BytesChunk::bytes` which is a
		/// usize. The choice of u64 is based on `tail::FilterMode::Bytes`.
		bytes:     u64,
	...
	impl BytesChunkBuffer {
		...
		/// method ensures that there are exactly as many chunks as needed to
		/// match `self.num_print` bytes, so there are in sum exactly
		/// `self.num_print` bytes stored in all chunks. The method returns an
		/// iterator over these chunks. If there are no chunks, for example
		/// because the piped stdin contained no bytes, or `num_print = 0` then
	...
	impl LinesChunk {
		...
		/// Count the number of lines delimited with [`Self::delimiter`] contained
		/// in the buffer. Currently [`memchr`] is used because performance is
		/// better than using an iterator or for loop.
		///
		/// # Examples
		///
		/// ```rust,ignore
		...
		/// lines. The new chunk contains exactly `chunk.lines - offset` lines.
		/// The offset in bytes is calculated and applied to the new chunk, so
		/// the new chunk contains only the bytes encountered after the offset
		/// in number of lines and the `delimiter`. If the offset is larger than
		...
		/// Return the number of lines the buffer contains. `self.lines` needs to
		/// be set before the call to this function returns the correct value.
		/// If the calculation of lines is needed then use `self.count_lines`.
		...
		/// the [`BytesChunk::fill`] function besides that this function also
		/// counts and stores the number of lines encountered while reading from
		/// the `filehandle`.
		pub fn fill(&mut self, filehandle: &mut impl BufRead) -> io::Result<Option<usize>> {
		...
		/// number of lines. The resulting offset is 0-based and points to the
		/// byte after the delimiter.
		///
		/// # Arguments
		///
		...
		/// Write the bytes contained in this buffer calculated with the given
		...
		/// Write the bytes contained in this buffer beginning from the given
	...
	impl LinesChunkBuffer {
		...
		/// Fills this buffer with chunks and consumes the reader completely. This
		/// method ensures that there are exactly as many chunks as needed to
		/// match `self.num_print` lines, so there are in sum exactly
		/// `self.num_print` lines stored in all chunks. The method returns an
		/// iterator over these chunks. If there are no chunks, for example
		/// because the piped stdin contained no lines, or `num_print = 0` then
...
mod follow {
	...
	mod files {
		...
		/// `last` always holds the path/key of the last file that was printed
		/// from. The keys of the [`HashMap`] can point to an existing file path
		/// (normal case), or stdin ("-"), or to a non-existing path (--retry).
		...
		impl FileHandling {
			...
			/// Return true if there is at least one "tailable" path (or stdin)
			/// remaining
			...
			/// Set `reader` to None to indicate that `path` is not an existing
			/// file anymore.
		...
		/// and the `display_name` (`header_name`) of files that are being
		/// followed.
		pub struct PathData {
	...
	mod watch {
		...
		impl WatcherRx {
			...
			/// Wrapper for `notify::Watcher::watch` to also add the parent
		...
		pub struct Observer {
			...
			/// Indicates whether to use the fallback `polling` method instead of
			/// the platform specific event driven method. Since `use_polling`
			/// is subject to change during runtime it is moved out of
			/// [`Settings`].
			pub use_polling: bool,
...
mod platform {
	...
	mod unix {
		...
		// this is never `true`, even with `<&-` because Rust's stdlib is
		// reopening fds as /dev/null see also: https://github.com/uutils/coreutils/issues/2873
		// (gnu/tests/tail-2/follow-stdin.sh fails because of this)
		// unsafe { libc::fcntl(fd, libc::F_GETFD) == -1 && get_errno() ==
		// libc::EBADF } false
		//}
	}
...
/// token, so that value is not mistaken for an obsolete `-N`/`+N` form.
...
/// Rewrites obsolete count forms wherever they occur before `--`.
...
/// BSD `tail -r`: print selected lines in reverse order.
...
fn uu_tail(settings: &Settings, host: &mut Host) -> TailResult<()> {
	let mut printer = HeaderPrinter::new(settings.verbose, true);
	let mut observer =
		Observer::from(settings, host.stdout_writer(), host.stderr_clone(), host.cancel_flag());
