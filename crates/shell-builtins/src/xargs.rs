...
impl MaxCharsCommandSizeLimiter {
	...
	fn new_system(env: &HashMap<OsString, OsString>) -> Self {
		...
		// SAFETY: `sysconf` only reads the process configuration value.
...
impl CommandBuilder<'_> {
	...
	fn execute(self, host: &mut Host) -> Result<CommandResult, CommandExecutionError> {
		...
		match &self.options.action {
			...
			ExecAction::Echo => {
				for (index, arg) in self.extra_args.iter().enumerate() {
					if index > 0 {
						let _ = host.stdout.write_all(b" ");
					}
					let _ = write!(host.stdout, "{}", arg.to_string_lossy());
				}
...
impl WhitespaceDelimitedArgumentReader {
	...
	fn stdin() -> Self {
		Self { rd: InputSource::Stdin, pending: Vec::with_capacity(4096), position: 0 }
	}
}
	...
	fn next(&mut self, host: &mut Host) -> io::Result<Option<Argument>> {
		...
		loop {
			if self.position == self.pending.len() {
				self.pending.clear();
				self.pending.resize(4096, 0);
				let bytes_read = loop {
					match self.rd.read(host, &mut self.pending) {
				...
				self.pending.truncate(bytes_read);
				self.position = 0;
			...
			let c = self.pending[self.position];
			self.position += 1;
			match (&escape, c) {
		...
		Ok(Some(Argument {
			arg:  os_string_from_utf8_lossy(result),
...
impl ByteDelimitedArgumentReader {
	...
	fn stdin(delimiter: u8) -> Self {
		Self { rd: InputSource::Stdin, delimiter, pending: Vec::with_capacity(4096), position: 0 }
	}
}
...
impl ArgumentReader for ByteDelimitedArgumentReader {
	fn next(&mut self, host: &mut Host) -> io::Result<Option<Argument>> {
		loop {
			let mut result = Vec::new();
			loop {
				let unread = &self.pending[self.position..];
				if let Some(offset) = unread.iter().position(|byte| *byte == self.delimiter) {
					result.extend_from_slice(&unread[..offset]);
					self.position += offset + 1;
					break;
				}
				result.extend_from_slice(unread);
...
fn os_string_from_utf8_lossy(bytes: Vec<u8>) -> OsString {
