...
impl DeclareCommand {
				...
				match &assignment.value {
					ast::AssignmentValue::Scalar(s) => {
						if let Some(index) = &assigned_index {
							...
							initial_value = Some(ShellValueLiteral::Scalar(s.value.to_string()));
							name_is_array = false;
						}
					},
					ast::AssignmentValue::Array(a) => {
						initial_value = Some(ShellValueLiteral::Array(ArrayLiteral(
							a.iter()
								.map(|(i, v)| {
									(i.as_ref().map(|w| w.value.to_string()), v.value.to_string())
								})
								.collect(),
						)));
						name_is_array = true;
	...
	fn display_matching_env_declarations(
		...
		#[expect(
			...
			reason = "runtime attribute filters require heterogeneous closures"
		)]
