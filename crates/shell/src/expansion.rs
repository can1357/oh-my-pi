...
impl Expansion {
	fn into_string(self, separator: &str) -> String {
		self.fields.into_iter().map(String::from).join(separator)
	}
...
fn encode_separator(separator: Option<char>, buffer: &mut [u8; 4]) -> &str {
	separator.map_or("", |separator| separator.encode_utf8(buffer))
}
...
impl<'a, SE: extensions::ShellExtensions> WordExpander<'a, SE> {
	...
	pub async fn basic_expand_to_str(&mut self, word: &str) -> Result<String, error::Error> {
		...
		Ok(expansion.into_string(separator))
	}
	...
	async fn basic_expand_pattern(&mut self, word: &str) -> Result<patterns::Pattern, error::Error> {
		...
		let pattern_pieces: Vec<_> = expansion
			...
			.flat_map(|(index, field)| {
				(index > 0)
					.then(|| patterns::PatternPiece::Literal(separator.clone()))
					.into_iter()
					.chain(field.0.into_iter().map(patterns::PatternPiece::from))
			})
			.collect();
	...
	async fn expand_word_piece(&mut self, word_piece: WordPiece) -> Result<Expansion, error::Error> {
		match word_piece {
			...
			WordPiece::AnsiCQuotedText(s) => {
				...
				Ok(Expansion::from(ExpansionPiece::Unsplittable(
			...
			WordPiece::EscapeSequence(s) => {
				if let Some(escaped) = s.strip_prefix('\\') {
					...
					Ok(Expansion::from(ExpansionPiece::Unsplittable(escaped.to_owned())))
				} else {
					Ok(Expansion::from(ExpansionPiece::Unsplittable(s)))
				}
			},
			complex => self.expand_complex_word_piece(complex).await,
		}
	}
	...
	async fn process_double_quoted_pieces(
		...
		for piece in pieces {
			...
			let fields_to_append = if concatenate {
				let separator = concatenation_joiner.map_or_else(String::new, String::from);
				let mut concatenated: Vec<ExpansionPiece> = this_fields
					.into_iter()
					.enumerate()
					.flat_map(|(index, WordField(pieces))| {
						(index > 0)
							.then(|| ExpansionPiece::Unsplittable(separator.clone()))
							.into_iter()
							.chain(pieces.into_iter().map(ExpansionPiece::make_unsplittable))
					})
					.collect();
			...
			for (i, WordField(next_pieces)) in fields_to_append.into_iter().enumerate() {
				...
				{
					last_pieces.extend(next_pieces);
					continue;
				}

				fields.push(WordField(next_pieces.collect()));
			}
		}
			...
			ParameterExpr::AssignDefaultValues { parameter, indirect, test_type, default_value } => {
				...
				match (test_type, expanded_parameter.classify()) {
					...
					_ => {
						...
						let mut separator_buffer = [0; 4];
						let separator =
							encode_separator(self.shell.get_ifs_first_char(), &mut separator_buffer);
						let expanded_default_value = expanded_default_value.into_string(separator);
						self
			...
			ParameterExpr::Substring { parameter, indirect, offset, length } => {
				...
				if matches!(
					...
					crate::parser::word::Parameter::Special(
						crate::parser::word::SpecialParameter::AllPositionalParameters { concatenate: _ },
					)
				) {
				...
				#[expect(clippy::cast_possible_wrap, reason = "expanded field counts fit i64")]
				...
				#[expect(clippy::cast_sign_loss, reason = "offset is clamped nonnegative")]
...
#[expect(clippy::panic_in_result_fn, reason = "tests use assertions to verify expansion behavior")]
#[cfg(test)]
mod tests {
	use super::*;
	...
	async fn test_tilde_expands_for_every_brace_element() -> Result<()> {
		...
		assert_eq!(full_expand_and_split_word(&mut shell, &params, "~/project/{a,b}").await?, vec![
			"/home/user/project/a",
			"/home/user/project/b"
		],);
		// A bare `~/{a,b}` (tilde immediately followed by the brace) must also
		// expand on both elements.
		assert_eq!(full_expand_and_split_word(&mut shell, &params, "~/{a,b}").await?, vec![
			"/home/user/a",
			"/home/user/b"
		],);
