...
pub struct SimpleCommand<'a, SE: extensions::ShellExtensions> {
	...
	/// process.
	#[allow(clippy::type_complexity, reason = "the callback signature is the public hook contract")]
	...
	match sys::process::spawn(cmd) {
		Ok(child) => {
			...
			#[expect(
				...
				reason = "child process ids fit the platform pid type"
			)]
...
/// Invokes a shell function behind one dyn-erased allocation.
///
/// Function bodies re-enter the interpreter, so an opaque future here would
/// close a second opaque-type cycle alongside the compound-list one. The
/// concrete boxed signature keeps the call graph acyclic; the allocation is
/// once per function invocation, dominated by the body's execution.
pub(crate) fn invoke_shell_function<'a, SE: extensions::ShellExtensions>(
	function: functions::Registration,
	mut context: ExecutionContext<'a, SE>,
	args: &'a [CommandArg],
) -> Pin<Box<dyn Future<Output = Result<ExecutionSpawnResult, error::Error>> + Send + 'a>> {
	Box::pin(async move {
...
/// non-interactive host (e.g. embedded shell execution), `new_pg` is false, the
