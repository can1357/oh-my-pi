...
impl EditObserver {
	...
	pub async fn inspect(
		...
		let pending = self.blackbox.path.as_ref().map(|_| {
			let model = self
				.auto_repair
				.as_ref()
				.and_then(EditRepairClient::model_identity)
				.unwrap_or_else(|| self.model.current());
			PendingBlackbox {
				record: bounded_record(&snapshot, mode, args, &self.blackbox, model),
			}
		});
...
mod tests {
	...
	async fn callback_repair_uses_dynamic_model_only_for_record_metadata() {
		...
		let observer = EditObserver::new(
			...
			EditBlackboxModel::new(sf!("registry/model")),
			Some(client),
		);
