...
/// Names that are conclusive on their own: every producer of these emits them,
/// and no hand-authored file plausibly carries one.
///
/// A bare `generated.<ext>` deliberately does **not** belong here — it is a
/// common hand-authored name, and the generators that do emit it (gqlgen and
/// friends) write a `Code generated by ... DO NOT EDIT.` header that
/// [`detect_generated_marker`] catches from the file's content instead.
