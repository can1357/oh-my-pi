...
#[tokio::test]
async fn plain_generated_filename_blocks_only_on_a_generated_header() {
	for header in ["", "// Code generated by github.com/99designs/gqlgen, DO NOT EDIT.\n"] {
		let ws = Workspace::new(EditMode::Patch);
		let rel = "internal/services/organizations/generated.go";
		let original = format!("{header}package organizations\n\nfunc Build() {{}}\n");
		ws.write(rel, &original);
		let writer = DiskWriter::default();
		let result = ws
			.apply_json(
				&serde_json::json!({ "path": rel, "edits": [
					{ "op": "update", "diff": "@@\n-func Build() {}\n+func Build() { run() }" }
				] }),
				&writer,
			)
			.await;
		if header.is_empty() {
			result.expect("handwritten generated.go is editable");
			assert_eq!(
				ws.read(rel).unwrap(),
				original.replace("func Build() {}", "func Build() { run() }")
			);
		} else {
			let err = result.expect_err("generated header still blocks the edit");
			assert!(err.to_string().contains("auto-generated"), "{err}");
			assert_eq!(ws.read(rel).unwrap(), original);
		}
	}
}
