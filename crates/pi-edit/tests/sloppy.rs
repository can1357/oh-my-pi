...
async fn no_op_recovery_respects_utf8_boundaries_in_suffix_overlap() {
	...
	// the normalized text after the match. With normalized `AAAAAAAAAA文…`
	// and a 10-byte rewrite the first window end (byte 20) lands inside the
	// fourth `文` (bytes 19..22): the pre-fix slice panicked at
	// apply.rs:2111 instead of reporting the no-op.
	let workspace = Workspace::new(EditMode::Sloppy);
	let before = "AAAAAAAAAA\n文文文文文文文文\n";
	workspace.write("a.txt", before);
	let writer = DiskWriter::default();
	let input = "*** SM:EDIT a.txt\n*** SM:FIND\nAAAAAAAAAA\n*** SM:PUT\nAAAAAAAAAA\n";
	let error = workspace
