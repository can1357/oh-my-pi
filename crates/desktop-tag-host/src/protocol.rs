...
impl CaptureManifest {
    pub fn validate(&self, capture_root: &Path) -> Result<()> {
        ...
        if self.capture_id.len() < 16
            ...
            || !self.capture_id.bytes().all(|byte| byte.is_ascii_alphanumeric() || byte == b'-')
        {
...
impl StageResponse {
    pub fn claim_url(&self, gateway_url: &str) -> Result<String> {
        ...
        let gateway = gateway_url.strip_prefix("http://").context("gateway must use HTTP loopback")?;
...
mod tests {
    ...
    fn manifest(root: &Path) -> CaptureManifest {
        CaptureManifest {
            ...
            foreground_app: ForegroundApp { process_id: 7, process_name: None, window_title: Some("Editor".into()), executable_path: None },
        }
    }
    ...
    fn claim_secret_is_fragment_only() {
        let response = StageResponse { version: 1, invocation_id: "abc def".into(), claim_token: "0123456789abcdef".into(), overlay_path: "/".into() };
    ...
    fn protocol_round_trips_exactly() {
        ...
        assert!(serde_json::from_str::<ControlRequest>(r#"{"version":1,"command":"capture","token":"bad"}"#).is_err());
    }
}
