use std::{ffi::c_void, io::{BufRead, BufReader, Read, Write}, mem::size_of, ptr, sync::{Arc, atomic::{AtomicBool, Ordering}}, thread};

use anyhow::{bail, Context, Result};
use windows_sys::Win32::{
    Foundation::{CloseHandle, ERROR_ALREADY_EXISTS, GENERIC_READ, GENERIC_WRITE, HANDLE, INVALID_HANDLE_VALUE, LocalFree},
    ...
    System::{Pipes::{ConnectNamedPipe, CreateNamedPipeW, DisconnectNamedPipe, PIPE_READMODE_MESSAGE, PIPE_TYPE_MESSAGE, PIPE_WAIT}, Threading::{CreateMutexW, GetCurrentProcess, OpenProcessToken}},
};
...
pub fn forward(sid: &str, request: &ControlRequest) -> Result<ControlResponse> {
    ...
    let handle = unsafe { CreateFileW(name.as_ptr(), GENERIC_READ | GENERIC_WRITE, 0, ptr::null(), OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, ptr::null_mut()) };
    ...
    BufReader::new(file).take(MAX_MESSAGE as u64).read_line(&mut response)?;
...
impl PipeServer {
    pub fn start(sid: &str, handler: Arc<dyn Fn(ControlRequest) -> ControlResponse + Send + Sync>) -> Result<Self> {
        let name = pipe_name(sid);
        let thread_name = name.clone();
        let sid = sid.to_owned();
        let stopped = Arc::new(AtomicBool::new(false));
        let thread_stopped = Arc::clone(&stopped);
        let thread = thread::Builder::new().name("desktop-tag-control".into()).spawn(move || serve(&thread_name, &sid, &handler, &thread_stopped))?;
...
impl Drop for PipeServer {
    fn drop(&mut self) {
        ...
        if let Ok(mut wake) = std::fs::OpenOptions::new().read(true).write(true).open(&self.name) {
...
fn serve(name: &str, sid: &str, handler: &Arc<dyn Fn(ControlRequest) -> ControlResponse + Send + Sync>, stopped: &AtomicBool) {
    while !stopped.load(Ordering::Acquire) {
        let Ok(pipe) = create_pipe(name, sid) else { return };
        ...
        let result = BufReader::new(&mut file).take(MAX_MESSAGE as u64 + 1).read_line(&mut data);
        if result.is_ok() && data.len() <= MAX_MESSAGE && !data.is_empty() {
            if let Ok(request) = serde_json::from_str::<ControlRequest>(&data) {
                let response = handler(request);
                if serde_json::to_writer(&mut file, &response).is_ok() { let _ = file.write_all(b"\n"); }
            }
        }
...
fn create_pipe(name: &str, sid: &str) -> Result<HANDLE> {
    ...
    let converted = unsafe { ConvertStringSecurityDescriptorToSecurityDescriptorW(sddl.as_ptr(), SDDL_REVISION_1, &mut descriptor, ptr::null_mut()) };
    if converted == 0 { return Err(std::io::Error::last_os_error()).context("create pipe security descriptor"); }
    let mut attributes = SECURITY_ATTRIBUTES { nLength: size_of::<SECURITY_ATTRIBUTES>() as u32, lpSecurityDescriptor: descriptor, bInheritHandle: 0 };
    let pipe = unsafe { CreateNamedPipeW(wide(name).as_ptr(), PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1, MAX_MESSAGE as u32, MAX_MESSAGE as u32, 0, &mut attributes) };
    unsafe { LocalFree(descriptor) };
    if pipe == INVALID_HANDLE_VALUE { return Err(std::io::Error::last_os_error()).context("create control pipe"); }
    Ok(pipe)
}
...
pub fn current_user_sid() -> Result<String> {
    ...
    let result = (|| {
        ...
        if length == 0 { bail!("current user SID is unavailable"); }
