use std::{fs, io::{BufRead, BufReader, Read, Write}, net::TcpStream, path::{Path, PathBuf}, process::{Child, Command, Stdio}, sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}}, thread, time::{Duration, Instant}};

use anyhow::{bail, Context, Result};
use windows_sys::Win32::{Security::Cryptography::{BCryptGenRandom, BCRYPT_USE_SYSTEM_PREFERRED_RNG}, UI::Shell::ShellExecuteW, UI::WindowsAndMessaging::SW_SHOWNORMAL};

use crate::{backoff::RestartBackoff, protocol::{CaptureManifest, GatewayReady, StageResponse}};
use super::wide;
...
impl GatewaySupervisor {
    pub fn start(program: PathBuf, root: &Path) -> Result<Self> {
        ...
        if !token_file.exists() { write_control_token(&token_file)?; }
        let capture_root = root.join("captures");
        fs::create_dir_all(&capture_root)?;
        let state = Arc::new(Mutex::new(GatewayState { ready: None, status: "gateway starting".into() }));
        let stopped = Arc::new(AtomicBool::new(false));
        let thread_state = Arc::clone(&state);
        let thread_stopped = Arc::clone(&stopped);
        let thread_token = token_file.clone();
        let thread = thread::Builder::new().name("desktop-tag-gateway".into()).spawn(move || supervise(program, thread_token, capture_root, thread_state, thread_stopped))?;
        Ok(Self { state, stopped, thread: Some(thread), token_file })
    }

    pub fn state(&self) -> GatewayState { self.state.lock().unwrap_or_else(|value| value.into_inner()).clone() }
...
fn supervise(program: PathBuf, token_file: PathBuf, capture_root: PathBuf, state: Arc<Mutex<GatewayState>>, stopped: Arc<AtomicBool>) {
            ...
            Ok((mut child, ready)) => {
                ...
                while !stopped.load(Ordering::Acquire) {
                    match child.try_wait() {
                        Ok(Some(status)) => { set_state(&state, None, &format!("gateway exited: {status}")); break; }
                        Ok(None) => thread::sleep(Duration::from_millis(250)),
                        Err(error) => { set_state(&state, None, &format!("gateway status failed: {error}")); break; }
                    }
                }
                if stopped.load(Ordering::Acquire) { let _ = child.kill(); let _ = child.wait(); return; }
                if started.elapsed() >= HEALTHY_RESET { backoff.reset(); }
            }
            Err(error) => set_state(&state, None, &format!("gateway launch failed: {error:#}")),
        }
        let delay = backoff.next_delay();
        let until = Instant::now() + delay;
        while !stopped.load(Ordering::Acquire) && Instant::now() < until { thread::sleep(Duration::from_millis(100)); }
    }
}

fn spawn_gateway(program: &Path, token_file: &Path, capture_root: &Path) -> Result<(Child, GatewayReady)> {
    ...
    let line = match receiver.recv_timeout(READINESS_TIMEOUT) {
        Ok(result) => result?,
        Err(_) => { let _ = child.kill(); let _ = child.wait(); bail!("gateway readiness timed out"); }
    };
    let ready: GatewayReady = serde_json::from_str(line.trim()).context("invalid gateway readiness JSON")?;
    validate_ready(&ready, child.id())?;
    Ok((child, ready))
}

fn validate_ready(ready: &GatewayReady, child_pid: u32) -> Result<()> {
    if ready.version != 1 || ready.pid != child_pid || ready.instance_id.is_empty() { bail!("gateway readiness identity mismatch"); }
    let (_, authority, _) = split_http_url(&ready.url)?;
    let host = authority.split(':').next().unwrap_or_default();
    if host != "127.0.0.1" && host != "localhost" && host != "[::1]" { bail!("gateway readiness URL is not loopback"); }
    Ok(())
}

fn set_state(state: &Mutex<GatewayState>, ready: Option<GatewayReady>, status: &str) {
    *state.lock().unwrap_or_else(|value| value.into_inner()) = GatewayState { ready, status: status.into() };
}
...
fn write_control_token(path: &Path) -> Result<()> {
    ...
    if status < 0 { bail!("secure token generation failed: {status}"); }
...
fn stage_request(url: &str, token: &str, manifest: &CaptureManifest) -> Result<StageResponse> {
    if token.len() != 64 || !token.bytes().all(|byte| byte.is_ascii_hexdigit()) { bail!("invalid private control token"); }
    ...
    write!(stream, "POST /api/native/invocations HTTP/1.1\r\nHost: {authority}\r\nAuthorization: Bearer {token}\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n", body.len())?;
    ...
    if !status.contains(" 200 ") && !status.contains(" 201 ") { bail!("gateway staging rejected request: {status}"); }
...
fn split_http_url(url: &str) -> Result<(&str, &str, &str)> {
    let rest = url.strip_prefix("http://").context("gateway URL must use HTTP")?;
    let (authority, path) = rest.split_once('/').unwrap_or((rest, ""));
    if authority.is_empty() || authority.contains('@') { bail!("invalid gateway authority"); }
...
pub fn launch_url(url: &str) -> Result<()> {
    ...
    if result as isize <= 32 { bail!("default browser launch failed: {}", result as isize); }
    Ok(())
}
