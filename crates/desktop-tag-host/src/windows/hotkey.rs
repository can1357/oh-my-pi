...
use windows_sys::Win32::{Foundation::HWND, UI::Input::KeyboardAndMouse::{RegisterHotKey, UnregisterHotKey}};
...
impl RegisteredHotkey {
    pub fn register(hwnd: HWND, binding: Hotkey) -> Result<Self> {
        ...
        if registered == 0 {
            return Err(std::io::Error::last_os_error()).context("global hotkey is already in use or unavailable");
        }
        Ok(Self { hwnd })
    }
}
