...
use windows_sys::Win32::{
    ...
    System::Registry::{RegCloseKey, RegCreateKeyExW, RegDeleteValueW, RegQueryValueExW, RegSetValueExW, HKEY, HKEY_CURRENT_USER, KEY_QUERY_VALUE, KEY_SET_VALUE, REG_OPTION_NON_VOLATILE, REG_SZ},
    UI::{Shell::{Shell_NotifyIconW, NOTIFYICONDATAW, NIF_ICON, NIF_MESSAGE, NIF_TIP, NIM_ADD, NIM_DELETE, NIM_MODIFY, NIM_SETVERSION, NOTIFYICON_VERSION_4}, WindowsAndMessaging::{AppendMenuW, CreatePopupMenu, DestroyMenu, GetCursorPos, LoadIconW, SetForegroundWindow, TrackPopupMenu, IDI_APPLICATION, MF_CHECKED, MF_SEPARATOR, MF_STRING, TPM_BOTTOMALIGN, TPM_LEFTALIGN, TPM_RETURNCMD, WM_APP}},
};
...
pub fn show_menu(hwnd: HWND) -> Option<usize> {
    ...
    unsafe {
        ...
        AppendMenuW(menu, MF_STRING | if startup { MF_CHECKED } else { 0 }, CMD_STARTUP, wide("Start at sign-in").as_ptr());
        AppendMenuW(menu, MF_SEPARATOR, 0, ptr::null());
        AppendMenuW(menu, MF_STRING, CMD_EXIT, wide("Exit").as_ptr());
        let mut point = POINT::default();
        GetCursorPos(&mut point);
        SetForegroundWindow(hwnd);
        let command = TrackPopupMenu(menu, TPM_LEFTALIGN | TPM_BOTTOMALIGN | TPM_RETURNCMD, point.x, point.y, 0, hwnd, ptr::null());
        DestroyMenu(menu);
...
pub fn toggle_startup(executable: &str) -> Result<bool> {
    ...
    let result = if enabled {
        ...
        unsafe { RegSetValueExW(key, name.as_ptr(), 0, REG_SZ, command.as_ptr().cast(), (command.len() * 2) as u32) }
    };
    unsafe { RegCloseKey(key) };
    if result != 0 { return Err(std::io::Error::from_raw_os_error(result as i32)).context("update Desktop Tag sign-in startup"); }
    Ok(!enabled)
}

fn startup_enabled() -> Result<bool> {
    let key = open_run_key(KEY_QUERY_VALUE)?;
    let result = unsafe { RegQueryValueExW(key, wide("OhMyPi-DesktopTag").as_ptr(), ptr::null_mut(), ptr::null_mut(), ptr::null_mut(), ptr::null_mut()) };
    unsafe { RegCloseKey(key) };
    Ok(result == 0)
}
...
fn open_run_key(access: u32) -> Result<HKEY> {
    ...
    let result = unsafe { RegCreateKeyExW(HKEY_CURRENT_USER, wide(r"Software\Microsoft\Windows\CurrentVersion\Run").as_ptr(), 0, ptr::null_mut(), REG_OPTION_NON_VOLATILE, access, ptr::null(), &mut key, ptr::null_mut()) };
    if result != 0 { return Err(std::io::Error::from_raw_os_error(result as i32)).context("open current-user Run key"); }
    Ok(key)
}
