...
Commands use autogenerated transport IDs by default. Hosts that need to correlate
a command with their own logs or callbacks can provide a keyword-only
`request_id`:
...
with RpcClient(model="anthropic/claude-sonnet-4-5") as client:
    turn = client.prompt_and_wait(
        ...
        request_id="host-command-42",
    )
```

`prompt()`, `abort_and_prompt()`, `prompt_and_wait()`, and `request_raw()` all
accept `request_id`. It must be a nonempty string and must not start with
`req_`, which is reserved for IDs generated by the client. The same ID is used
in the outbound envelope and by the response waiter. Passing `id` inside a raw
payload is rejected; use `request_id` instead.

A custom ID must be unique for every command on a connection, including after
the command is accepted or times out. The client rejects a collision while the
earlier request is still waiting for its initial response, but callers remain
responsible for longer-lived per-connection uniqueness. This avoids retaining
an unbounded history of completed IDs.

Register `on_protocol_error()` callbacks and record your ID-to-command mapping
before sending a command. A scheduling failure reported immediately after initial
acceptance may reach the callback before `prompt()` or `abort_and_prompt()`
returns. The callback's `RpcProtocolError.request_id` contains the supplied ID.

These IDs correlate RPC transport requests only. They are distinct from
operation IDs that identify durable execution: a `request_id` does not add
execution lifecycle tracking, retry behavior, or continuity across reconnects.
