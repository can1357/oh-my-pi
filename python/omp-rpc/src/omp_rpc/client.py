...
class RpcClient:
    ...
    def request_raw(
        ...
        **payload: JsonValue,
        ...
        return self._request(command_type, request_id=request_id, **payload)
    ...
    def _request(
        ...
        **payload: JsonValue,
        ...
        if "id" in payload:
            raise ValueError(
                "Raw RPC payloads cannot set 'id'; pass request_id= instead"
            )
        if request_id is not None:
            ...
            if request_id.startswith("req_"):
                raise ValueError(
                    "request_id values starting with 'req_' are reserved "
                    "for autogenerated request IDs"
                )

        process = self._require_process()
        envelope: JsonObject = {"type": command_type}
        ...
        with self._state_lock:
            if request_id is None:
                ...
                raise ValueError(f"request_id {request_id!r} is already pending")
