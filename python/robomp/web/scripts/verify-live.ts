...
async function main(): Promise<void> {
	...
	const runningCardCorrect =
		runningCard !== undefined &&
		runningCard.bucket === "running" &&
		runningCard.metaText.includes("claude-3-5-sonnet") &&
		runningCard.metaText.includes("last action edit") &&
		runningCard.metaText.includes("elapsed");
	...
	await page.evaluate(() => {
		const card = Array.from(document.querySelectorAll(".rmp-card")).find(c =>
			c.querySelector(".rmp-card-id")?.textContent?.includes("octo/widget#1"),
		);
	...
	await Bun.sleep(500);
	const retryStatusText = await page.evaluate(
		() => document.querySelector(".rmp-trigger-bar-status")?.textContent ?? "",
	);
