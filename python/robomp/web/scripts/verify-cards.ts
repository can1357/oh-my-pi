...
async function main(): Promise<void> {
	...
	page.on("request", async (req: HTTPRequest) => {
		...
		if (url.pathname === "/api/status") {
			...
			const parsedBody: Record<string, unknown> | null = req.postData()
				? (JSON.parse(req.postData()!) as Record<string, unknown>)
				: null;
	...
	results.push({
		...
		ok:
			!!fullFailedCard &&
			fullFailedCard.bucket === "failed" &&
			fullFailedCard.errorText === "triage failed error" &&
			fullFailedCard.actions.includes("retry"),
	...
	results.push({
		...
		ok:
			!!runningCard &&
			runningCard.bucket === "running" &&
			runningCard.metaText.includes("claude-3-5") &&
			runningCard.metaText.includes("last action edit") &&
			runningCard.actions.includes("cancel"),
	...
	results.push({
		...
		ok:
			!!inflightOnlyCard &&
			inflightOnlyCard.bucket === "running" &&
			inflightOnlyCard.metaText.includes("held by pool") &&
			inflightOnlyCard.actions.length === 0 &&
			inflightOnlyCard.stepper &&
			inflightOnlyCard.liveStep,
	...
	results.push({
		...
		ok:
			!!compactOrphanRunning &&
			compactOrphanRunning.bucket === "running" &&
			!compactOrphanRunning.stepper &&
			compactOrphanRunning.actions.includes("cancel"),
	...
	results.push({
		...
		ok:
			!!compactOrphanFailed &&
			compactOrphanFailed.bucket === "failed" &&
			!compactOrphanFailed.stepper &&
			compactOrphanFailed.actions.includes("retry"),
	...
	results.push({
		...
		ok:
			!!failedWithoutErr &&
			failedWithoutErr.bucket === "failed" &&
			failedWithoutErr.errorText === "" &&
			failedWithoutErr.actions.includes("retry"),
	...
	results.push({
		...
		ok:
			!!queuedCard &&
			queuedCard.bucket === "queued" &&
			queuedCard.stepper &&
			queuedCard.currentStep &&
			queuedCard.actions.length === 0,
	...
	results.push({
		...
		ok:
			!!activeCard &&
			activeCard.bucket === "active" &&
			activeCard.stepper &&
			activeCard.currentStep &&
			activeCard.actions.length === 0,
	...
	const structuralCheck = await page.evaluate(() => {
		...
		const borderShadowOk = cards.every(el => {
			...
			return (
				radius === "8px" &&
				(shadow === "none" || shadow.includes("0px 0px 0px") || shadow === "") &&
				bl <= 1.05 &&
				br <= 1.05
			);
		});
	...
	const responsiveOk = await page.evaluate(() => {
		...
		const noPageOverflow =
			docEl.scrollWidth <= docEl.clientWidth + 1 && document.body.scrollWidth <= document.body.clientWidth + 1;
	...
	results.push({
		...
		ok:
			retryReqs.length === 1 &&
			retryReqs[0].headers["x-robomp-replay-token"] === REPLAY_TOKEN &&
			statusTextA.includes("queued retry: fail-del-1"),
	...
	await page.evaluate(() => {
		window.confirm = () => true;
		const card = Array.from(document.querySelectorAll(".rmp-card")).find(c =>
			c.querySelector(".rmp-card-id")?.textContent?.includes("octo/widget#2"),
		);
	...
	results.push({
		...
		ok:
			cancelReqs.length === 1 &&
			cancelReqs[0].headers["x-robomp-replay-token"] === REPLAY_TOKEN &&
			statusTextB.includes("cancel signaled: run-del- (fired=false)"),
