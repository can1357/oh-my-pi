import { createMemo, createResource, createSignal, For, type JSX, type ResourceReturn, Show } from "solid-js";
...
export function Browse(): JSX.Element {
	...
	const meta = (): string => {
		...
		const cacheInfo =
			d.cache.fetched_at > 0 ? ` · ${d.cache.hit ? "cached" : "loaded"} ${ageSeconds.toFixed(0)}s ago` : "";
		...
		<GlassCard heading="browse" accessory={<span class="text-[11px] text-ink-400">{meta()}</span>}>
			...
			>
				...
				<Show when={errorMessage()}>
					<div class="px-5 py-2.5 text-[12px] text-err border-b border-stroke-soft tint-err">{errorMessage()}</div>
				</Show>
				...
				<div class="max-h-[44vh] overflow-y-auto scrollable">
					<Show
						...
						fallback={
							<div class="empty">
								{hideProcessed() && processedCount() > 0 && processedCount() === data().issues.length
