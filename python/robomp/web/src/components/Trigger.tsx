...
function PanelBody(p: PanelBodyProps): JSX.Element {
	return (
		<GlassCard heading="trigger">
			<Show
				when={CONFIG.replayEnabled}
				fallback={
					<div class="px-5 py-7 text-ink-300 text-[13px] leading-relaxed">
						trigger disabled. set <code>ROBOMP_REPLAY_TOKEN</code> in the server env to enable manual triage and
						retry actions.
					</div>
				}
			>
				<div class="px-5 pb-5 pt-1 flex flex-col gap-4">
					...
					<Show
						...
						fallback={<span class={`text-[12px] ${STATUS_TONE.idle}`}>{p.validate() ?? "ready"}</span>}
					>
						<span class={`text-[12px] ${STATUS_TONE[triggerStatus().kind]}`}>{triggerStatus().text}</span>
					</Show>
				</div>
			</Show>
		</GlassCard>
