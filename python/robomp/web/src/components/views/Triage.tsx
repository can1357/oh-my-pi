...
export function Triage(): JSX.Element {
	return (
		<Show when={CONFIG.replayEnabled} fallback={<FirstRunPanel />}>
		...
		<GlassCard heading="triage" accessory={<span class="rmp-readonly-chip">locked</span>}>
			<div class="rmp-firstrun">
				...
				<p class="rmp-firstrun-body">
					Triage and retry let you drive the bot by hand — fetch a fresh issue, re-run a failed delivery, or browse
					the open backlog. These actions are gated behind a server-side replay token so the dashboard is safe to
					expose read-only.
				</p>
