...
## Security Model & Server Configuration

- **Token scope (reads + writes)**: `ROBOMP_REPLAY_TOKEN` gates **everything** — `POST /replay`, `POST /api/trigger`, `POST /api/cancel`, `GET /api/github/issues` (private-repo titles), and all dashboard reads (`GET /events`, `/issues`, `/releases`, `/api/status`, `/api/logs`) once a token is set. The token is **never embedded in the page HTML**: `GET /` ships only `{replayEnabled}`; the SPA round-trips its credential through the token-gated `GET /api/config` (the sole route that returns the token, and only to callers who already present it) and keeps it in sessionStorage. Token-less deployments (loopback dev) leave all reads open — there is no credential to protect; the bind address is the mitigation.
- **`ROBOMP_TRUSTED_HOSTS`** (optional, comma-separated Host-header allowlist): enables `TrustedHostMiddleware` as DNS-rebinding defense-in-depth. It is **not set in the bundled docker-compose.yml** — enabling it with only `localhost`/`127.0.0.1` would 400 GitHub webhook deliveries arriving through a reverse proxy/tunnel that preserves the public hostname. Operators behind a reverse proxy should set it explicitly to their public hostname(s).
- **Default bind**: `ROBOMP_BIND_HOST` defaults to `0.0.0.0` in `config.py` (accepts off-host traffic); host-side exposure is controlled by the compose `127.0.0.1:6543:8080` port pin. Only change the default bind deliberately.
## Testing & QA
