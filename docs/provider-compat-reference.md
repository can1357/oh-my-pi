...
- **Results**: `function_call_output` and `custom_tool_call_output` items pair with calls by `call_id` (the `callId` half of the composite). Their `output` is either a string or an array of canonical `input_text` and `input_image` blocks. Vision-capable models keep tool-result images inside that array instead of creating synthetic user messages; models without image input receive a text placeholder. Auth-gateway parsing also accepts legacy `output_text`, `text`, and `refusal` blocks, decodes inline data-image URLs into image content, and retains remote image URLs or OpenAI image file IDs as references. Remote URLs are forwarded only when the resolved serializer can replay the exact reference: effective vision guards apply, and Google targets require a known image MIME type. The gateway rejects incompatible references before dispatch. OpenAI file-ID images are provider- and endpoint-owned: they can be replayed only by the official OpenAI provider at its exact Responses endpoint, not by Codex, Azure, or custom Responses-compatible targets. Incompatible targets are rejected before dispatch. `input_file` remains supported for request messages but is rejected in tool outputs until canonical tool results can replay its bytes and references losslessly. Stateful `previous_response_id` chaining works across turns.
