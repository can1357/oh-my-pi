...
"Shared" means the field exists on both resolved views with the same wire contract — **not** that both builders detect the same default. `buildOpenAICompat` and `buildOpenAIResponsesCompat` each compute their own defaults; rows below use _Chat:_ / _Responses:_ where they diverge (single detection = identical on both surfaces).
