...
| `PresentationDeliveryLedger` (`test/helpers/acp-delivery-ledger.ts`) | for the typed `reduceAcpToolView` path: a declared fact whose reducer receipt disagrees with the audience/capability-derived policy; a call's `sourceEcho` that reaches settlement without appearing on any rendered channel (the generalized form of the old, since-removed `EvalSourceDeliveryAuditor`) | `driveAcpToolView` — `acp-view-reducer.test.ts`, `bash-presentation-protocol.test.ts`, `eval-e2e-wire.test.ts` |
...
| `presentation-model-goldens.test.ts` / `presentation-model-parity.test.ts` | model-facing bytes from `renderModelContent`/`renderTuiPresentation` (`presentation/projections.ts`) drifting from the golden corpus, or a golden that no longer matches what a real producer (bash) actually sends the model — this is the CI-mandatory enforcement contract for the "model sees only audience-eligible facts" projection guarantee. `renderModelContent` has **no production caller yet** (a later phase wires one in via the atomic golden-locked cutover); until then, these two suites are the entire enforcement — observational, not by construction | `bun test test/presentation-model-goldens.test.ts test/presentation-model-parity.test.ts` |
