...
| `retry.hardErrorSameModelRetries`          | number  | `0`               | Retry the same model with backoff before the fallback chain is consulted on hard provider errors. `0` preserves the current behavior and consults the chain immediately. |
