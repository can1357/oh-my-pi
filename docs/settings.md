...
| `tier.openai`       | enum   | `provider` | `provider`, `none`, `auto`, `default`, `flex`, `scale`, `priority`. Sent as `service_tier` for OpenAI / OpenAI-Codex and OpenAI-family OpenRouter models. `provider` defers to the model's own default tier when its rule declares one (Doubleword's `flex`); `none` omits `service_tier` outright. Launch with `--service-tier <value>` for a one-session OpenAI override; the flag is not persisted. |
| `tier.anthropic`    | enum   | `provider` | `provider`, `none`, `priority`. `priority` realizes fast mode on supported direct Claude models (ignored on Bedrock/Vertex and via OpenRouter).                                                                                                                                            |
| `tier.google`       | enum   | `provider` | `provider`, `none`, `flex`, `priority`. Gemini API sends it in the body; Vertex sends `priority` via header (`flex` is a no-op on Vertex).                                                                                                                                                 |
| `tier.subagent`     | enum   | `inherit` | `inherit`, `provider`, `none`, `auto`, `default`, `flex`, `scale`, `priority`. Applied to the spawned model's family; `inherit` tracks the main agent.                                                                                                                                     |
...
| `tier.advisor`      | enum   | `none`    | `inherit`, `provider`, `none`, `auto`, `default`, `flex`, `scale`, `priority`. Applied to the advisor model's family.                                                                                                                                                                      |
