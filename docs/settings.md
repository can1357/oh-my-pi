...
| `contextProfile`       | enum    | `full`                      | `full`, `balanced`, or `aggressive`. `full` preserves the complete prompt, skill list, and default native tool set. `balanced` uses the compact prompt, omits the eager skill list, keeps `read`, `write`, `bash`, `edit`, `grep`, and `glob` native, and mounts other tools under `xd://`. `aggressive` keeps `read`, `write`, and `bash` native. Both reduced profiles also keep enabled `todo` and `ask` native because their integrations depend on native identity. Reduced profiles use bounded `skill://` and `xd://` discovery. See [system prompt customization](./system-prompt-customization.md). |
