...
| `tools.xdevPromote`            | array   | `[]`    | Tool names kept top-level instead of mounted under `xd://` while `tools.xdev` is on (e.g. `lsp`, `mcp__context_resolve_library_id`). A single name or comma-separated string (e.g. `lsp` or `lsp, ast_grep`) is accepted and split into the tool-name list. MCP names must be written in the exact minted form `mcp__<server>_<tool>`: server and tool parts are lowercased, runs of characters outside `[a-z_]` (digits included) collapse to a single underscore, leading/trailing underscores are trimmed, a tool part redundantly starting with its server name is un-prefixed (`puppeteer/puppeteer_screenshot` mints `mcp__puppeteer_screenshot`), and names over 64 characters are truncated with a deterministic hash suffix (`mcp__<tool_…_a1b2c3d4`). Minted names are listed with their tools under `xd://` — copy from there rather than deriving by hand. Promotion only changes presentation — tools still need their normal enablement — and is a no-op when `tools.xdev` is off. |
