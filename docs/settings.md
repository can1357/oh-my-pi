...
| `providers.openai-codex.historyNotes` | enum | `off` | `off`, `on`, `auto`. Enable private, server-side Codex history and checkpoint notes; `auto` follows the model catalog. Requires official ChatGPT OAuth. Independent of the `window` compaction method; activation is fixed when the session starts. See [Codex context windows and notes](provider-quirks.md#context-windows-and-notes). |
...
| `compaction.methodOrder` | array | `remote, snapcompact, handoff, shake, soft` | Ordered fallbacks. `remote` uses provider-native OpenAI-compatible server compaction. Add `window` first to opt into [Codex checkpoint-based context windows](provider-quirks.md#context-windows-and-notes); with compaction enabled, this requires `providers.openai-codex.historyNotes: "on"`. Invalid combinations are rejected at startup and on runtime setting changes. Notes with `remote` remain supported. Unavailable or failed methods advance. |
