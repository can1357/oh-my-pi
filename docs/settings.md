...
| `providers.cacheWarming`            | enum    | `idle`    | `off`, `streaming`, `idle`. Prompt-cache warming re-sends the last request with a one-token output budget shortly before its cache entry expires, but only when the expected avoided-miss cost clears the refresh cost by at least $0.05. `off` disables warming; `streaming` protects prefixes during long tool executions and stops when the agent settles; `idle` also refreshes between runs (15% continuation probability) until 30 minutes after the last real request. Models without a declared prompt-cache lifetime (see [Models](./models.md)) are never warmed. |
