...
| `providers.webSearchXaiModel`      | string  | _(unset)_ | Grok model ID for xAI Responses web search; defaults to `grok-4.6`, overridden by `XAI_SEARCH_MODEL`. Reasoning effort follows the selected model's registry/catalog compatibility. Configure relay endpoints and API keys together in `models.yml` (`providers.xai.baseUrl`/`apiKey`) or via `XAI_BASE_URL` + `XAI_API_KEY`. A fallback from a custom `xai-oauth` endpoint to an `xai` key uses the `xai` provider's endpoint and headers. |
