...
To select AnySearch, set `modelRoles.web: web/anysearch` or run `omp web-search --model web/anysearch "query"`. With no custom web role or fallback chain, AnySearch runs first when a key is configured. Without a key, it runs after the first available alternative has failed, then falls back to the remaining engines if needed. If no alternative is available, anonymous AnySearch runs directly. Explicit primary and fallback selections retain their configured order. Credentials come from `/login anysearch` or `ANYSEARCH_API_KEY`. Without a key, only an explicit primary selection can provision and save a generated key; an incidental fallback never provisions credentials. Legacy `providers.webSearchOrder` and `providers.webSearchExclude` entries for `anysearch` migrate to the web role and its fallback chain.
