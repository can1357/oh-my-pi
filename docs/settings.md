...
retry:
  ...
  refusalFallbackRevertPolicy: default # after-success opts into request-scoped refusal fallback
  fallbackChains:
...
| `retry.refusalFallbackRevertPolicy` | enum | `default` | Main/task refusal and provider-classified content-block restoration. `default` preserves existing error-specific policies; `after-success` restores the previous model after one successful fallback response and its tools, before the next model call. |
...
Classifier refusals (`stopDetails.type: refusal` or `sensitive`) keep the fallback pinned by default, even with `fallbackRevertPolicy: cooldown-expiry`. Set `retry.refusalFallbackRevertPolicy: after-success` to return after the first successful replacement response, including before a same-turn tool-result continuation. This also covers normalized content-blocked errors such as Codex `cyber_policy`, without changing the precedence of sibling-credential rotation. Failed or aborted fallback responses do not trigger restoration. Existing availability cooldowns and explicit model selections remain authoritative; a refusal fallback layered over an outage fallback returns to that outage fallback. Ordinary assistant refusal prose does not trigger this policy, and rate-limit/outage fallback behavior is unchanged.
