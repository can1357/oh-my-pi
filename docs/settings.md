...
| `task.agentAdvisor`   | record  | `{}`    | Per-agent subagent advisor: agent name → `"on"` / `"off"` / advisor model pattern. Overrides agent frontmatter `advisor` and implicit enrollment from `watchdogs`; configured from the `/agents` hub. |
