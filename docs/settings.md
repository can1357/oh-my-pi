...
| `memory.backend`              | enum    | `off`                                    | `off`, `local`, `hindsight`, `mnemopi`, `sharpshooter`. Each backend has its own `hindsight.*` / `mnemopi.*` / `memories.*` / `sharpshooter.*` tuning keys.                                                                               |
| `sharpshooter.enabled`        | boolean | `false`                                  | Run Sharpshooter's project decision files beside the selected backend rather than in place of it. Ignored when `memory.backend` is already `sharpshooter`.                                                                                |
