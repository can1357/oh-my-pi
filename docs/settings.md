...
| `retry.fallbackOnContentFilter`           | boolean | `false`           | Opt in to treating provider-declared content-filter terminations as fallback-eligible. Requires `retry.modelFallback` and a matching configured fallback chain; otherwise the filtered turn remains terminal.                                                                                                                                                                                                                                                           |
...
Content-filter fallback is disabled by default. Set both `retry.modelFallback: true` and `retry.fallbackOnContentFilter: true` to let a provider-declared content-filter termination walk the same configured chain used by classifier refusals.
