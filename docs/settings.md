...
| `tools.xdevPromote`            | array   | `[]`    | Tool names kept top-level instead of mounted under `xd://` while `tools.xdev` is on (e.g. `lsp`, `mcp__context_resolve_library_id`). A single name or comma-separated string (e.g. `lsp` or `lsp, ast_grep`) is accepted and split into the tool-name list. MCP names use the sanitized minted form (server/tool parts lowercased, non-alphanumerics → `_`). Promotion only changes presentation — tools still need their normal enablement — and is a no-op when `tools.xdev` is off. |
