...
| `promptProfile`        | enum    | `full`                      | `full` or `compact`. `compact` renders a shorter bundled instruction template, dropping the examples, the internal-URL catalog, and the long workflow and delegation prose while keeping every generated surface: skills, rules, context files, tool inventory, and the `xd://` protocol. For local models where the prompt competes with the conversation for a small context window. A custom `SYSTEM.md` replaces the template outright and ignores this. See [system-prompt-customization](./system-prompt-customization.md). |
...
| `tools.xdevForceMount`         | array   | `[]`    | Glob patterns of tool names to mount under `xd://` even when the tool declares itself essential or is pinned top-level, keeping their schemas out of every request. `read` and `write` always stay top-level: they carry the `xd://` transport, and demoting either makes every mounted device unreachable. Forcing `grep`, `todo`, `ask`, or `web_search` gives up the harness integration that pinned them. Off by default; intended for small-context local models. |
