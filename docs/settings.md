...
| `retry.classifierRefusalCooldownMs`      | number  | `0`               | How long (ms) to sideline a model after it declines a turn via its content classifier before it is eligible again. `0` (default) pins the fallback for the rest of the session; a positive value treats the refusal like any transient failure (suppress the declining model for that window, leave the fallback unpinned) so `retry.fallbackRevertPolicy` restores the primary once the cooldown ends. |
