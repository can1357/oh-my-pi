...
| `providers.maxImagesPerRequest`     | record  | `{}`      | Positive per-provider caps on images per request, overriding the built-in budget for that id. Omitted providers fall back to the built-in budget for their id, then their wire API family (`anthropic-messages` 90, `google-generative-ai` 200), then a floor of 5. `omp config set` rejects non-positive or non-numeric values.                                                                                                       |
| `providers.maxImageBytesPerRequest` | record  | `{}`      | Budget in base64 image bytes for one request, per provider id, for gateways that reject large bodies. The oldest droppable images are removed until the request fits or none are left; images retained in assistant turns are never removed and count against the budget. Snapcompact inline imaging plans against this budget, so a tool result or system prompt whose frames would not fit stays text. Omitted providers are unbounded. `omp config set` rejects non-positive or non-numeric values.                                                                |
