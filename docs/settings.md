...
| `omp config set <key> <value>` | Parse `<value>` against the key's schema type and write it to the global main YAML file. When a setting env var still supplies the effective value, it says so (`--json`: `overriddenBy`) instead of reporting the env value as the one set.                                                     |
| `omp config reset <key>`       | Delete the key from the global config, so the schema default (or a lower layer such as project config) applies again and later default changes still reach you. Prints the resulting effective value.                                                                                            |
...
| `providers.fetch`                   | enum    | `auto`    | `auto`, `native`, `trafilatura`, `lynx`, `parallel`, `firecrawl`, `jina`, `tinyfish`.                                                                                                                                                                                                                                                                                                                                                              |
