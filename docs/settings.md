...
| `tier.autoFastMode` | boolean | `false` | Use priority processing for primary-session requests after a user prompt. Explicit family tiers and manual `/fast` take precedence; tasks, advisors, panels, side-channel turns, and auto-learn capture retain their configured tiers. |
| `tier.autoFastModeDurationMinutes` | number | `20` | Minutes automatic priority remains active after a user prompt. |
