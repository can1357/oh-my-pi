...
| `compaction.v2TimeoutMs` | number | `180000` | Absolute deadline in milliseconds for each V2 streaming remote-compaction request; `0` disables it. Progress does not reset it, and expiry does not retry V2. Does not bound the entire fallback chain. |
