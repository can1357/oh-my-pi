...
| `retry.classifierRefusalCooldownMs`      | number  | `0`               | How long (ms) to keep a session on its fallback after the primary declines a turn via its content classifier. `0` (default) pins the fallback for the rest of the session; a positive value leaves the fallback unpinned so `retry.fallbackRevertPolicy` restores the primary once the cooldown ends. Tracked per session, so a refusal never sidelines the model for other sessions. |
