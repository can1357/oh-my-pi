...
| `providers.webSearchFanout`         | number  | `1`       | Number of eligible providers queried concurrently from the effective `providers.webSearchOrder`. Values are truncated to an integer with a floor of `1`; the effective provider list is the only ceiling. `1` preserves sequential first-success fallback and existing output. Implicit candidates that report unavailable are skipped, availability checks that throw become error sections, and explicitly configured candidates retain explicit-selection behavior. Fewer than two eligible candidates use normal single-provider output. Higher values return separate, unmerged provider sections and may multiply search API usage and cost. Opt in with `omp config set providers.webSearchFanout 2`. |
