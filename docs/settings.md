...
| `providers.autoThinkingMaxEffort` | enum    | `xhigh` | Highest effort `defaultThinkingLevel: auto` may resolve. `high` limits auto to high, `xhigh` keeps the classifier one tier below the top, and `max` lets the classifier bill the top tier on models that expose it. The local on-device classifier honors the `high` ceiling but remains capped at `xhigh` for the `max` opt-in. This governs what `auto` _resolves_: a model whose ladder offers nothing under the ceiling gets no auto level at all, and one that also sets `thinking.requiresEffort` still receives its lowest supported effort from the transport — on a `["max"]` ladder that is `max`, because the model accepts nothing else. |
