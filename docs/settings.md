...
| `task.agentAdvisor`   | record  | `{}`    | Agent name → `"on"` / `"off"` / advisor model pattern. Overrides frontmatter `advisor` and implicit enrollment from `watchdogs`. Configure it in `/agents`. |
