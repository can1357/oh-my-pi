...
| `providers.imageOpenRouterModel`    | string  | _(unset)_ | OpenRouter image model ID, independent of the chat model. Applies when image provider ordering reaches OpenRouter, without changing that order. Per-call `model` overrides it and selects OpenRouter. Explicit selections fail without replacement. Blank clears the setting. |
