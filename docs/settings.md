...
Settings are stored as plain YAML mappings. Every key, its type, default, and enum values come from the settings schema, and you can inspect or change any of them with `oh-my-pk config` or the interactive `/settings` panel.
...
- For instruction files discovered into the agent context (`AGENTS.md`, `.ompk/`, etc.), see [Context files](./context-files.md).
...
|---|---|---|---|
| Global | `~/.ompk/agent/config.yml` | The main persistent settings file. Always loaded. | `/settings`, `oh-my-pk config set`, and `oh-my-pk config reset` write here. |
| Global legacy | `~/.ompk/agent/settings.json` | Migrated into `config.yml` once, only when `config.yml` does not yet exist. | Not written after migration; the original is renamed to `settings.json.bak`. |
| Project | `<cwd>/.ompk/config.yml` (plus `.ompk/settings.json`) | Loaded when the process working directory has a non-empty `.ompk/`. | Read-only from settings commands; edit the file by hand. |
| Project legacy | `<cwd>/.ompk/settings.json` | Still read; project `config.yml` is merged on top of it. | Not written by settings commands. |
...
`PI_CODING_AGENT_DIR` relocates the `~/.ompk/agent` base directory. When it is set, the global `config.yml`, the auth store (`agent.db`), and everything else under the agent directory move with it. Use `oh-my-pk config path` to print the active agent directory.

Native project settings are intentionally scoped to the process working directory's `.ompk/` folder — settings discovery does **not** walk ancestor directories looking for the nearest `.ompk/`. Other discovery providers (Claude, Codex, Gemini, Cursor, OpenCode) can also contribute project-level settings from their own files; those are read-only from `omp` settings commands and can be turned off by provider id (see [Provider and source disabling](#provider-and-source-disabling)).
...
Use the interactive `/settings` panel inside a session, or the `oh-my-pk config` command from a shell. Both operate on the merged effective settings, but every persistent write lands in the **global** file only.

```bash
oh-my-pk config list                 # all settings with current effective values
oh-my-pk config list --json          # same, machine-readable
oh-my-pk config get theme.dark       # one value
oh-my-pk config get theme.dark --json
oh-my-pk config set compaction.enabled false
oh-my-pk config set defaultThinkingLevel medium
oh-my-pk config reset steeringMode   # restore a key to its schema default
oh-my-pk config path                 # print the active agent directory
```

For users who want the full first-run animation on normal launches, set `startup.showSplash`:

```bash
oh-my-pk config set startup.showSplash true
```
...
### Interactive spawned-agent tier

The interactive TUI exposes the default execution tier for newly spawned agents in two places:

- `/tier [status|light|mid|frontier|auto]`
- `/settings` → **Model** → **Agent Model Profiles** → **Default Spawn Tier**

This tier is independent of model selection. For example, `/tier light` can run a strong model inside the light read/search/report-only capability envelope, while `/tier mid` permits the basic execution and delegation tools with replace-only editing. The selection persists as `agent.tier`.

`auto` removes the interactive override: `task.agentPolicies` and agent/workflow policy still apply, and the normal fallback is the frontier execution profile. Policy composition is restrictive, so a more-specific `frontier` policy cannot widen a global `light` selection. Changes affect agents spawned after the setting changes; existing child sessions retain their immutable execution profile.

### Subcommands

| Command | Effect |
|---|---|
| `oh-my-pk config list` | Print every setting grouped by tab, with its current value and type. `--json` emits an object keyed by setting path with `{ value, type, description }`. |
| `oh-my-pk config get <key>` | Print the effective value of one key. Unknown keys exit non-zero. `--json` emits `{ key, value, type, description }`. |
| `oh-my-pk config set <key> <value>` | Parse `<value>` against the key's schema type and write it to the global `config.yml`. |
| `oh-my-pk config reset <key>` | Write the key's schema **default** back to the global config (this persists the default, it does not delete the key). |
| `oh-my-pk config path` | Print the active agent directory (honors `PI_CODING_AGENT_DIR`). |

`oh-my-pk config` with no subcommand, or `--help`, prints the help and lists settings. The `--json` flag is accepted by `list`, `get`, `set`, and `reset`.

### Value parsing

`oh-my-pk config set` parses the value string according to the target key's schema type. The string is trimmed first.
...
`oh-my-pk config set`, `oh-my-pk config reset`, `/settings`, and any runtime settings change all write to the global `config.yml` under the active agent directory. They never write to `<cwd>/.ompk/config.yml`. To create a project-local override, edit that file directly (see [Project-local config](#project-local-config)). Saves are debounced and re-read the file under a lock, so external edits made while a session is open are preserved.
...
3. **Project settings** — `<cwd>/.ompk/settings.json` then `<cwd>/.ompk/config.yml` (and contributions from other discovery providers at project level).
4. **Global settings** — `~/.ompk/agent/config.yml`.
...
Create `<repo>/.ompk/config.yml` when a repository needs its own settings:
...
```bash
oh-my-pk --config ./local/ci-settings.yml "check this failure"
oh-my-pk --config ./base.yml --config ./experiment.yml "try this model"
```
...
# <repo>/.ompk/config.yml — inside this repo ONLY groq is disabled
disabledProviders:
  - groq
```
...
Every key below is defined in the settings schema; `oh-my-pk config list` shows the full set with current values. Defaults and enum values are taken from the schema. Settings that accept an env or flag override are noted; those overrides are process-local and not persisted.
...
| `autolearn.enabled` | boolean | `false` | Experimental: after the agent stops, nudge it to capture lessons to memory and create/enhance isolated managed skills under `~/.ompk/agent/managed-skills`. Enables the `manage_skill` tool (and `learn` when a memory backend is active). |
...
`compaction` has additional tuning keys (idle compaction, supersede/drop heuristics) visible in `oh-my-pk config list`. See [Compaction](./compaction.md) for the full strategy reference.
...
When enabled, the entire top-level agent session runs from the Ethereal Workspace. This includes interactive TUI sessions, not only `oh-my-pk -p` one-shot prompts. See [Ethereal Workspaces](./ethereal-workspaces.md) for lifecycle and safety details.
...
When `~/.ompk/agent/config.yml` does not exist, startup builds it once from legacy sources, then writes the result:

1. `~/.ompk/agent/settings.json` (renamed to `settings.json.bak` after a successful migration).
...
- Start `omp` from the directory that contains `.ompk/config.yml`. Settings discovery only checks the current working directory's `.ompk/`, not ancestor directories.
- Ensure `.ompk/` is non-empty; empty config directories are ignored.
- Confirm the file is valid YAML and its top level is a mapping.
- Run `oh-my-pk config get <key>` from that directory to see the effective value.
...
### `oh-my-pk config set` changed the wrong file

`oh-my-pk config set` and `oh-my-pk config reset` always write the global `config.yml` under the active agent directory. Run `oh-my-pk config path` to print it. For project-local settings, edit `<repo>/.ompk/config.yml` directly.

### `oh-my-pk config reset` did not remove my key

`reset` writes the schema **default** value into the global config — it persists the default rather than deleting the key. To stop overriding a project value from global config, delete the key from `~/.ompk/agent/config.yml` by hand.
...
### `oh-my-pk config set <key>` says "Unknown setting"

Keys must match a schema path exactly, with no shorthand. Use `theme.dark`, not `theme`. Run `oh-my-pk config list` to see every valid key.
