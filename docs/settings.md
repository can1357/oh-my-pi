...
| `providers.maxImagesPerRequest`      | record  | `{}`      | Positive per-provider caps on how many images a single request may carry, overriding the built-in budget for that provider id. Omitted providers resolve to the built-in budget for their id, then their wire API family (a gateway speaking `anthropic-messages` inherits 90, `google-generative-ai` 200), then a floor of 5. Raise this for a custom gateway that proxies a vendor API but is not recognized by id. `omp config set` rejects non-positive or non-numeric values.                                                                                                                                                                                                          |
| `providers.maxImageBytesPerRequest`  | record  | `{}`      | Positive per-provider caps on the total base64 image bytes a single request may carry. Set this for a gateway that answers 413 on large bodies; the oldest droppable images are removed until the request fits, and images already retained in assistant turns are counted against the cap. Omitted providers are unbounded. `omp config set` rejects non-positive or non-numeric values.                                                                                                                                                                                                          |
