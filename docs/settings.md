...
| `providers.antigravitySensitiveWords` | array   | `["RFC 2119"]` | Literal phrases split with a zero-width space (U+200B) in the system prompt sent to `google-antigravity`. Cloud Code Assist inspects `systemInstruction` on agent-mode requests and answers matched payloads with a bare `429 RESOURCE_EXHAUSTED` that no retry clears; the split keeps the text readable for the model while clearing the match. Matching is literal and case-sensitive. Set to `[]` to send the prompt verbatim. |
