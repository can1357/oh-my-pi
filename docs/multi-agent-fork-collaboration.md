...
Current operation is manual admission: one dispatcher enforces the queue labels, claims, and WIP limits before delegation. The deployed Linear Agent Worker enforces atomic admission, fenced leases with heartbeats, reconcile parking, dead-letter surfacing, transient-failure retries with the documented backoff schedule, per-grant logical attempt keys (`linear:<org>:<issueId>:<attempt>`), in-flight prompt refresh on issue revisions, and a pre-push fence guard on branch mutations in relay-spawned runners. Queue-state label mirroring and the transition audit log remain unimplemented (comments only today). Automated delegation remains disabled by policy until the remaining contract is implemented and verified.
