...
**Pull requests test against source-matched native addons.** `native_addons` compares the checkout's native build inputs with the first parent of GitHub's synthetic merge commit. If unchanged, it reuses the retained `natives-linux-x64` artifact from that exact base commit's push CI run in the base repository. Changed inputs or a missing/expired artifact trigger a GitHub-hosted Bazel build of the Linux x64 pair, one target at a time through the existing native build step. Both variants are smoke-loaded before uploading the downstream artifact. Latest-release npm addons are not a safe substitute: merged native behavior can change before the next release. Full Rust tests, clippy, and rustfmt remain post-merge (`rust_validate` is skipped on PRs).
