...
The active stats database is recognized through symlink aliases and platform path equivalence. Only direct children of the blob store count as content-addressed blobs; nested files are auxiliary. Files under a session's `local` artifact directory remain artifacts regardless of their suffix, including after archival. GC and reporting share the archived-sessions directory resolver.
