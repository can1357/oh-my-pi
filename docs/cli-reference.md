...
Text output removes terminal control characters; JSON retains the original path and error strings for machine consumers. Blob file classification is shared with GC. Stats database selection also matches GC: the active agent directory uses the normal profile/XDG-aware root database; a different explicit agent directory checks its local `stats.db` without opening or creating it.

Active agent directory aliases use the same profile/XDG storage roots for sessions, blobs, and databases. Only direct children of the blob store count as content-addressed blobs; nested files are auxiliary. Files under a session's `local` artifact directory remain artifacts regardless of their suffix, including after archival. GC and reporting share the archived-sessions directory resolver and journal/backup filename grammar.
