...
On macOS and Linux, run `omp config init-xdg` to opt an existing default store into the XDG data, state, and cache roots. A lone uninitialized `$XDG_*_HOME/omp` directory does not relocate an existing `~/.omp` store; fresh installs may adopt existing roots. For a named profile, OMP uses an XDG category only when that category already contains `omp/profiles/<name>`; otherwise that category remains under `~/.omp/profiles/<name>`.
