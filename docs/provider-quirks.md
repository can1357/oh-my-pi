...
- **Provider Header Forwarding**: `streamSimple` asks the model's `ProviderDefinition.preparePiNativeHeaders` hook for headers to carry on a forwarded turn and merges them *under* `options.headers` (`packages/ai/src/stream.ts`). The gateway then merges those under its own captured passthrough headers. This is the only path by which client-side request policy the gateway cannot recompute — an environment-gated retention opt-in, for example — reaches the upstream provider; `commandcode`'s `x-cmd-zdr` uses it. The hook must not touch `model` or `options.apiKey` (the gateway bearer).
- **Forwarded Policy Is Reconciled, Not Applied Blindly**: A forwarded value is a *decision by the originating process*, not yet a request header, so a provider forwards it under an omp-internal name (e.g. `x-omp-cmd-zdr`) that the receiving transport consumes. On arrival the performing process — the one holding the resolved model — weighs it against that model before emitting the real request header, then deletes the internal entry so it never reaches a provider. The distinction matters because `options.headers` are merged *over* model headers case-sensitively: a forwarded value spelled as the real header would either overwrite the resolved model's explicit choice or, under different casing, be joined with it into one comma-separated field.
