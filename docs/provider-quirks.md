...
- **Provider Protocol & Request Envelope**:
  ...
  - **System Instructions & Synthetic 429 Fallback**: Antigravity tags top-level `systemInstruction` with `role: "user"`. However, Google's Cloud Code Assist backend fingerprints `systemInstruction` content in agent mode (`requestType: "agent"`), rejecting certain prompts with a synthetic HTTP 429 `RESOURCE_EXHAUSTED` lacking an `error.details` array. The transport automatically detects this synthetic 429, logs a warning once per session, folds the system prompt into the first user turn (`contents[0].parts`), removes `systemInstruction`, and immediately retries the request. Upon success, this preference is memoized in `AntigravityProviderSessionState.foldedModels` so subsequent turns bypass top-level `systemInstruction`. If a folded request later fails, the memo is reset. Synthetic 429 errors without details are classified as standard `RATE_LIMIT_EXCEEDED` (short backoff) rather than 30-minute `QUOTA_EXHAUSTED`, preventing premature OAuth credential locking.
...
- **System Instruction & Request Envelope**: Antigravity tags `systemInstruction` with `role: "user"`. If Cloud Code Assist rejects `systemInstruction` with a details-less synthetic 429 `RESOURCE_EXHAUSTED` in agent mode, the transport automatically folds system instructions into the first user message turn (`contents[0].parts`) and retries once. `buildAntigravityRequestEnvelope` injects structured `requestId` (`agent/<id>/<ts>/<trajectoryId>/<step>`), `userAgent: "antigravity"`, `requestType: "agent"`, `sessionId`, and `labels` (`model_enum`, `trajectory_id`, `last_step_index`, `last_execution_id`, `used_claude*`) using `getAntigravityModelWireProfile`.
