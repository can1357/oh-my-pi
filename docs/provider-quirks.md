...
- **Pricing & Antigravity backfill**: Base prices are calculated via `calculateCost`. `google-antigravity` models report $0 list price upstream and are backfilled at generation time by `applyPricingPeerFallbacks` (`packages/catalog/scripts/generated-policies.ts`) using the `pricing-peer` rule in `packages/catalog/src/compat/rules/runtime/behavior.kdl` (peers `google`, `google-vertex`, `anthropic`; e.g. `gemini-3-flash` -> `gemini-3-flash-preview`). Runtime-discovered $0 rows resolve the same peer mirror in `buildModel`.
...
- **Pricing Fallback**: `applyPricingPeerFallbacks` (`packages/catalog/scripts/generated-policies.ts`) backfills 0-cost discovery models from the `pricing-peer` rule (`packages/catalog/src/compat/rules/runtime/behavior.kdl`: peers `google`, `google-vertex`, `anthropic`; e.g. `gemini-3-flash` -> `gemini-3-flash-preview`, `claude-opus-4-5` -> `claude-opus-4-5@20251101`), mapping Gemini models to Google API prices and Claude models to Google Vertex list prices.
