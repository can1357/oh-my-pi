...
Codex history/notes is opt-in and works alongside your existing compaction method. It requires the `openai-codex` provider, an official ChatGPT Codex endpoint, and ChatGPT OAuth. API keys and proxy endpoints do not enable it. Enable notes without changing your compaction order:
...
To use checkpoint-and-reset instead of summarization, also put `window` first in `compaction.methodOrder`. Window resets require the model catalog's context-window policy. Requested checkpoint features make at most one optional discovery attempt per shared model registry when that metadata is missing. Main-session startup can await the attempt; subagent creation does not wait for the network and adopts discovered metadata once idle, unless the model has changed or the session was disposed. `auto` notes activation resolves when catalog metadata first becomes available. Refresh failures do not prevent startup, and omp never substitutes its own guidance text.

Remaining tokens are the **effective configured compaction threshold minus prompt input tokens minus the catalog fallback buffer**, clamped to zero. The threshold uses the existing `thresholdTokens`, `thresholdPercent`, and reserve rules, not an independent raw model limit. When `window` is the first available method, normal threshold compaction waits for the model's checkpoint sequence: one response writing or appending notes, then one response calling `new_context`. If the model does not follow that sequence, omp logs the failure at debug level and advances to the next configured method.

A successful `new_context` call resets history at the next safe, paired tool boundary. It generates no summary and keeps only initial context. The next request is a cold full frame with a fresh window ID; later requests can chain normally. The transcript shows **Started context window N**. Window identities survive resume and same-agent forks. Omp retains its own main/sub-agent identity and registry IDs; the provider adapter translates them into valid backend namespaces. While history/notes or window mode is live, subagents reuse the root session's backend session id so their private history and notes share one server-side store, and each keeps its own agent path and window lineage; subagents do not inherit the parent's window identity.

History and notes are OpenAI-hosted, model-only state, not portable memory. Omp forwards encrypted arguments and results without decrypting them, and persists opaque results verbatim for Codex replay: session persistence never truncates an encrypted result block or a model-only call's encrypted arguments, so notes files up to the protocol's 1,000,000-byte limit survive resume. Public tool events, tool hooks, RPC messages, collab replication, and exported or shared transcripts use `[private model-only result]` instead of private payloads. These tools are not exposed through the Code Mode tool bridge. Switching to another provider unregisters them and stops context-window injections; each private call is dropped together with its paired result on non-Codex requests (with a warning), because the namespaced name is invalid for providers that allow only alphanumerics, underscores, and hyphens. Public content in the same assistant turn is kept. Returning to Codex restores availability and retains the window identity, but a dropped exchange is gone from that request's history.

History recording is explicitly enabled on opted-in model requests through Codex's native `history_ingest_requested` metadata and matching agent/window identities. It is not a local journal search. Indexing can lag briefly, and windows that were never ingested are not guaranteed to be recoverable.

The backend's alpha routes can change. HTTP and malformed-response failures become tool errors; an encrypted result can itself report an operation failure that omp cannot inspect. Verify a write by reading it back and checking its contents, not by checking transport success. A failed or oversized thread hint is omitted. Notes are not a replacement for user-readable memory backends.
