...
- **Usage & Quota**: `ollamaCloudUsageProvider` (`packages/ai/src/usage/ollama.ts`) queries `GET https://ollama.com/api/usage` with the Bearer API key. Migrated monthly-credit plans surface `limits.monthly.usage` (a 0..1 fraction, mapped to a percent-unit "Monthly allowance" limit); unmigrated accounts keep the legacy `limits.session`/`limits.weekly` fractions as "Ollama 5 Hour"/"Ollama 7 Day" limits; `activity.cost` renders as a trailing ~4-week USD spend. The endpoint is undocumented, so failures degrade to `null` via `ctx.logger?.warn`. The local `ollamaUsageProvider` keeps a stub report (no quota endpoint; per-response `prompt_eval_count`/`eval_count` only). Both providers set `validatesCredentials: false` so usage probing never marks credentials healthy or unhealthy.
...
- **Usage Accounting**: `ollamaCloudUsageProvider` (`packages/ai/src/usage/ollama.ts`) fetches `GET https://ollama.com/api/usage` (Bearer API key): `limits.monthly.usage` → "Monthly allowance" percent limit for migrated plans, legacy `limits.session`/`limits.weekly` → "Ollama 5 Hour"/"Ollama 7 Day" percent limits for unmigrated accounts, `activity.cost` → trailing ~4-week USD spend. `validatesCredentials: false` keeps the usage probe out of credential health checks.
