...
## ExLlamaV3 via TabbyAPI (Local OpenAI-compatible) (`exllamav3`)
ExLlamaV3 is a local inference engine for EXL3-quantized models, served through [TabbyAPI](https://github.com/theroyallab/tabbyAPI)'s OpenAI-compatible API. It uses the OpenAI Chat Completions transport over HTTP/SSE. Entry modules include `packages/catalog/src/compat/rules/auth/exllamav3.kdl` for authentication and credential handling, and `packages/catalog/src/provider-models/openai-compat.ts` (`exllamav3ModelManagerOptions`) for catalog options and dynamic model discovery.

### Special casings
- **Reasoning Content Replay (`replayReasoningContent`)**: Registered in `LOCAL_OPENAI_COMPAT_PROVIDERS` (`packages/catalog/src/compat/resolve.ts`), so `isLocalOpenAICompatBackend` auto-enables `replayReasoningContent: true`. TabbyAPI splits `<think>` blocks server-side into `reasoning_content` (streamed as `delta.reasoning_content`) and accepts `reasoning_content` on history assistant messages, letting the chat template rebuild `<think>` blocks for prompt-stable continuations.
- **Qwen Thinking (`thinking-format "qwen"`)**: Qwen 3.8+ on `exllamav3` rides the llama.cpp dialect (classes/qwen.kdl): TabbyAPI accepts the flat top-level `enable_thinking` and `reasoning_effort` request fields directly and forwards them into the chat template as template vars (unlike vLLM, which ignores top-level `enable_thinking` and needs `chat_template_kwargs`).
- **Named Tool Choice**: Unlike llama.cpp/LM Studio (string-only `tool_choice`), TabbyAPI natively accepts named tool-choice objects alongside `"none"`/`"auto"`/`"required"`, so `supportsNamedToolChoice` stays `true`.
- **Stream Idle Timeout Floor**: As a local serving backend (`isLocalServingBackend`), the expanded 5-minute stream idle timeout floor applies, accommodating long EXL3 prefills of 100k+ token contexts.
- **Dynamic-Only Catalog Exclusion**: Included in `DISCOVERY_ONLY_PROVIDERS` (`scripts/generate-models.ts`) and `LOCAL_ONLY_PROVIDERS` (`packages/ai/test/models-json-no-local-endpoints.test.ts`). Local models are excluded from static catalog generation so machine-specific endpoints are never committed to `models.json`.

### Auth & usage
- **Credential Resolution & Defaults**: Declared in `packages/catalog/src/compat/rules/auth/exllamav3.kdl` as a `login "api-key"` rule. Reads the TabbyAPI API key from the `EXLLAMAV3_API_KEY` environment variable or credentials stored via `omp auth-broker login exllamav3`. TabbyAPI generates an `api_tokens.yml` with random keys on first start unless `disable_auth: true` is set in its `config.yml`; the admin key can be used to enumerate the whole model directory.
- **Unauthenticated Local Mode**: Defaults to base URL `http://127.0.0.1:5000/v1` (`EXLLAMAV3_BASE_URL` overrides it) and placeholder token `"exllamav3-local"` (`empty-fallback`) when no key is supplied.

### Catalog model handling
- **Descriptor Configuration**: Registered in `packages/catalog/src/provider-models/descriptors.ts` with `id: "exllamav3"`, `defaultModel: "qwen3.8-27b-exl3"`, `envVars: ["EXLLAMAV3_API_KEY"]`, and manager options generated by `exllamav3ModelManagerOptions`.
- **Dynamic Model Discovery**: `exllamav3ModelManagerOptions` first fetches `GET /v1/model` (the loaded model's card) and then `fetchOpenAICompatibleModels` against `/v1/models`. TabbyAPI's list entries carry no parameters — the card's `parameters.max_seq_len` becomes the discovered context window and `parameters.use_vision` adds image input. When the card is reachable, only the loaded model is kept: with an admin key `/v1/models` also enumerates every directory under `model_dir` plus dummy OpenAI compatibility ids, none of which are separately servable.
- **Reasoning Heuristic**: `/v1/models` reports no capability metadata; the mapper lights up the effort dial for Qwen 3.8+ open weights (which always think) via `classifyModel("exllamav3", id)`, matching the vLLM rule.
- **Cache Provider ID**: Resolved by `resolveModelCacheProviderId("exllamav3", { baseUrl })`, generating base-URL-hashed cache keys formatted as `exllamav3:models-v1:${Bun.hash(baseUrl).toString(36)}`.
