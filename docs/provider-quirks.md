...
- **Authoritative Single-Model Catalog (`dynamicModelsAuthoritative`)**: A successful discovery IS the whole catalog. When TabbyAPI reloads from model A to model B, the refresh replaces the prior snapshot instead of merging, so stale A disappears from selection — otherwise requests labeled A would silently execute on B.
