...
- **Budget usage provider (`packages/ai/src/usage/litellm.ts`)**: `litellmUsageProvider` reads the key's own `GET /key/info` and `GET /user/info` (self-service management routes at the proxy root, i.e. the base URL with its trailing `/v1` stripped) and emits one USD `UsageLimit` per budget window that has both `spend` and `max_budget`: the key's primary window, each `budget_limits[]` window whose spend the proxy reports under `budget_limits_usage` (older proxies omit it and those windows are skipped), and the owning user's window. `budget_duration` maps to `windowId` `daily` / `7d` / `monthly`; `max_budget: 0` renders as exhausted. Either route failing leaves the other's limits intact.
