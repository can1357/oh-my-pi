...
A successful `new_context` call resets history at the next safe, paired tool boundary. It generates no summary and keeps only initial context. The next request is a cold full frame with a fresh window ID; later requests can chain normally. The transcript shows **Started context window N**. Window identities survive resume and same-agent forks. Omp retains its own main/sub-agent identity and registry IDs; the provider adapter translates them into valid backend namespaces. Subagents do not inherit the parent's window identity.
