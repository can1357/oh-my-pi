...
- **Usage quota**: `minimaxCodeUsageProvider` / `minimaxCodeCnUsageProvider` in `packages/ai/src/usage/minimax-code.ts` poll `GET /v1/token_plan/remains` at `https://api.minimax.io` for `minimax-code` and `https://api.minimaxi.com` for `minimax-code-cn`, parsing rolling interval and weekly usage windows per plan bucket into remaining percentages for `omp usage`.
...
- **Usage Telemetry**: `minimaxCodeCnUsageProvider` (`packages/ai/src/usage/minimax-code.ts`, registered in `packages/ai/src/auth-storage.ts`) fetches quota remaining percentages from `https://api.minimaxi.com/v1/token_plan/remains` — the same wire contract as international `minimax-code`, served by the China host (#12448).
...
- **Usage quota**: `zhipuCodingPlanUsageProvider` (`packages/ai/src/usage/zai.ts`, registered in `packages/ai/src/auth-storage.ts`) reuses the Z.AI quota fetcher against `https://open.bigmodel.cn/api/monitor/usage/quota/limit` (raw-key Authorization, same `zai:` limit-id prefixes ranked by `zaiRankingStrategy`), surfacing monthly request and 5-hour token windows for `omp usage` (#12448).
