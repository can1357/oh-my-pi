...
- **Provider Protocol & Request Envelope**:
  ...
  - **System Instructions**: Antigravity tags system instructions with `role: "user"`. No identity prompt is injected. The backend does inspect the field under `requestType: "agent"`: matched payloads come back as a bare `429 RESOURCE_EXHAUSTED` (no `ErrorInfo`/`RetryInfo`) that is indistinguishable from quota exhaustion and identical on every retry, while the same prompt succeeds with `requestType` omitted or set to `chat`. `providers.antigravitySensitiveWords` (default `["RFC 2119"]`) splits the configured literal phrases with U+200B via `obfuscateSensitiveWords` (`packages/ai/src/utils/antigravity-sensitive-words.ts`) before the prompt is serialized; only `systemInstruction` is rewritten, never `contents` or tools. Matching is literal and case-sensitive, so the phrase list tracks whatever the gateway currently rejects (can1357/oh-my-pi#11689).
