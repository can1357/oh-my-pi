...
- **Provider Protocol & Request Envelope**:
  ...
  - **`requestType` Fallback On Unexplained 429**: CCA refuses some `requestType: "agent"` payloads with `429 RESOURCE_EXHAUSTED` while every quota bucket still reports headroom, and serves the byte-identical payload tagged `"chat"`. Such a refusal names no quota metric and carries neither `Retry-After` nor `RetryInfo`, so it is indistinguishable from a real exhaustion by status alone. `streamGoogleGeminiCli` (`packages/ai/src/providers/google-gemini-cli.ts`) therefore retries the same endpoint once with `requestType: "chat"` when the 429 body matches `ANTIGRAVITY_RESOURCE_EXHAUSTED_RE` but not `ANTIGRAVITY_NAMED_QUOTA_RE` (`quota metric`, `quota exceeded`, `rateLimitExceeded`); one downgrade per request, tracked by `downgradedAgentRequestType`. 429s that do name a spent metric keep failing fast, and the outgoing default stays `"agent"` so client alignment is unchanged. Covered by `packages/ai/test/google-antigravity-request-type-downgrade.test.ts`.
