...
- **Native Compaction**: `openai-responses` models on this route use OpenAI's native compaction (V2 streamed `compaction_trigger`, then V1 `/responses/compact`) by default, detected by `isBedrockOpenAIUrl` (`packages/catalog/src/hosts.ts`). The compaction requests run `prepareProviderRequest` (`packages/ai/src/registry/registry.ts`) first, so `{region}` and bearer/SigV4 auth match normal turns. `enabled: false` disables both methods; `v2StreamingEnabled: false` disables only V2. See [compaction](./compaction.md).
