...
History and notes are OpenAI-hosted, model-only state, not portable memory. Omp forwards encrypted arguments and results without decrypting them, and persists opaque results for Codex replay. Public tool events, tool hooks, RPC messages, and exported or shared transcripts use `[private model-only result]` instead of private payloads. These tools are not exposed through the Code Mode tool bridge. Switching to another provider unregisters them and stops context-window injections; private content is stripped from non-Codex requests with a warning. Returning to Codex restores availability and retains the window identity.
