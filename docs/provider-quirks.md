...
- **Service Tier / Priority Control**: `excludesInferredOpenAIServiceTier` and `shouldSendServiceTier` (`packages/ai/src/types.ts`) allow `fireworks` requests to send `service_tier: "priority"` when the independent `providers.fireworksTier: priority` setting is enabled, suppressing unneeded tier defaults. Native `/fast` scopes do not change this provider-specific control.
