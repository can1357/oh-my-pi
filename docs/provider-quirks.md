...
- **Keyless Env Opt-In**: a nonempty `EXLLAMAV3_BASE_URL` marks the built-in provider keyless (when no credential is stored), so discovered models are available and requests resolve `kNoAuth` — mirroring how `LLAMA_CPP_BASE_URL` opts the implicit llama.cpp engine in. A stored key always wins for requests.
...
- **No-Model-Loaded Contract**: TabbyAPI answers `GET /v1/model` with HTTP 503 "No models are currently loaded" while an admin key still enumerates directories and dummy ids on `/v1/models`; discovery publishes an authoritative empty catalog for that state instead of ids inference would immediately fail on. Card-unreachable failures (404/401/timeout) keep the raw-list fallback.
