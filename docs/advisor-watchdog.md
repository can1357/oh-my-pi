...
| `concern`       | Interrupting steering message when `advisor.steerSeverity` includes concerns and the delivery constraints below permit it. A late terminal-answer `concern` is preserved as a visible card instead. | Material risk, likely wrong direction, missing constraint, hallucinated API. |
| `blocker`       | Interrupting steering message when `advisor.steerSeverity` includes blockers and the delivery constraints below permit it. Unlike a `concern`, a terminal answer alone does not prevent it from triggering a turn. | Continuing would clearly waste work or produce broken output. |
...
- **While the loop is still streaming**, blockers can steer into the live turn when `advisor.steerSeverity` includes blockers. Nits and concerns from an in-progress review remain deferred until a final boundary.
- **Once the loop has yielded and gone idle**, delivery keys on how the turn ended:
  - If the primary's tail is a **terminal text answer with no queued work**, a late `concern` is preserved as a visible card rather than waking the agent to restate a completed turn (#4840) — it re-enters context on the next resume (a new message, `.`/`c`, or a steer/follow-up), exactly like the interrupt case. A `blocker` is the exception: when the threshold includes blockers, it normally steers a triggered turn, because it means the agent handed off broken or unexercised work that must be acknowledged before the turn is considered done (#5628).
  - Otherwise (the agent yielded mid-work, no terminal answer), an idle severity the threshold lets steer normally triggers a fresh turn so the advice is acted on immediately.
...
So the advisor can steer and resume a run the agent ended on its own **while it is running or yielded mid-work and the current mode/client permits steering** — for severities `advisor.steerSeverity` allows to steer. When steering is blocked instead, the note is either preserved as a card (the terminal-answer, plan-mode, and deferred-ACP cases above) or downgraded to a non-interrupting aside (the `advisor.immuneTurns` cooldown below); either way it waits for the next step boundary or resume rather than waking the agent. Severities outside the threshold skip steering entirely and ride the aside queue instead.

`advisor.immuneTurns` limits interruption frequency. After the advisor successfully delivers a `concern` or `blocker` through the steering channel, later concerns/blockers are routed as non-interrupting asides until the configured number of primary turns has completed. The default is `3`. `nit` notes are unchanged, and advice raised while user-interrupt auto-resume suppression is active is still preserved instead of restarting a stopped run. Severities the threshold excludes never reach the steering channel, so the window only ever downgrades severities the threshold lets steer.

`advisor.steerSeverity` selects which severities may interrupt through the steering channel: `blocker` (only blockers steer; concerns ride the aside queue), `concern` (only concerns steer; blockers ride the aside queue instead of steering), or `both` (the default — both steer, exactly today's behaviour). A plain `nit` never steers under any threshold.
