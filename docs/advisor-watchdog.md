...
- **Once the loop has yielded and gone idle**, delivery keys on how the turn ended:
  - If the primary's tail is a **terminal text answer with no queued work**, a late `concern` is preserved as a visible card rather than waking the agent to restate a completed turn (#4840) — it re-enters context on the next resume (a new message, `.`/`c`, or a steer/follow-up), exactly like the interrupt case, unless `advisor.lateConcern` is `steer` (see below). A `blocker` is the exception: it normally steers a triggered turn, because it means the agent handed off broken or unexercised work that must be acknowledged before the turn is considered done (#5628).
...
`advisor.lateConcern` controls the late terminal-answer case above. The default `preserve` records a `concern` that lands after the agent's final answer as a visible card (the #4840 behavior) rather than waking the agent to restate a completed turn. Set it to `steer` to instead wake the agent to act on the concern immediately, the same triggered turn a `blocker` gets. It is aimed at slow, higher-quality advisors whose review reliably arrives after the turn completes, so their concerns would otherwise always be demoted to cards. `steer` still respects the guards above: a deliberate user interrupt, plan mode, and ACP deferred agent-initiated turns still preserve the note instead of resuming.
