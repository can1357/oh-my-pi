...
- `enabledTools?: string[]` — per-server tool allowlist. Entries are raw advertised tool names or glob patterns (`*`, `?`, `[...]`, `{a,b}`); only matching tools are contributed to the session. An empty array is fail-open (same as absent — filters nothing); to contribute no tool, use a never-matching entry such as `"__none__"`. This OMP-specific field follows the same scoping rule as `requestIdFormat` (camelCase keys in translated files are ignored; translated files may still map their own native spellings, e.g. Codex `enabled_tools`).
- `disabledTools?: string[]` — per-server tool denylist; matching tools are excluded. When both fields are set, `disabledTools` wins (deny subtracts from allow). An empty array is fail-open (same as absent — filters nothing); to exclude every tool, use `"*"` in `disabledTools`.

Glob semantics: entries are matched against the tool's advertised name and its sanitized spelling — a run of characters outside `[A-Za-z0-9_-]` collapses to a single `_`, with edge underscores trimmed (`admin//delete` → `admin_delete`, `.hidden` → `hidden`), so a name like `admin/delete` is addressed by `admin*` or `admin_delete`, and distinct raw names that sanitize to the same spelling are addressed together. Patterns are picomatch globs over that sanitized name (any entry holding `*?[]{}()|\\` takes the glob path; anything else is literal exact equality), with the raw name matched too, so an entry written for the name's literal spelling still reaches it. A pattern may therefore select more tools than the spellings it names. An entry carrying four or more consecutive backslashes cannot be compiled and is ignored (reported as unmatched); the same never-matching degradation applies to an entry carrying more than six unescaped wildcard tokens (`*?[{`), where the compiled regex could otherwise backtrack for seconds against a server-controlled name. A `"` preceded by an even number of backslashes (including none) is treated as bare and matched literally. Escapes mean what they mean in a glob (`\d` is the digit class, `\x41` is `A`), `{a,b}` alternates top-level comma-separated branches, and any brace, grouping, or extglob syntax picomatch cannot read as such is either literal or unmatched rather than disabling the server. The glob path runs only against names up to 64 characters (the registry-name ceiling); longer names stay addressable through the exact literal path.
