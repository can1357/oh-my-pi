...
Glob semantics: tool names are opaque strings, not paths — `/` is an ordinary character, so `*`, `?`, and `**` cross it (`admin*` matches `admin/delete`), and character classes treat it like any other member (`admin[/]delete` and `admin[.-0]delete` both match `admin/delete`, while `admin[^/]delete` does not). `?` matches exactly one character of the raw name, an astral character counting as one. Names that look like path segments are ordinary too: `*` matches a tool literally named `.` or `..`, and it still spans zero characters after a literal dot (`.*` addresses `.`). A brace pair that does not enclose a top-level comma is literal, so a tool named `{` is addressed by `{`. A leading `!`, an extglob prefix (`+(a|b)`), and any grouping or alternation characters are literal, even beside wildcards: `+(a|b)*` addresses the literal name `+(a|b)foo`, not `+afoo`. Only `*`, `?`, `[...]` and `{a,b}` carry meaning. POSIX bracket classes (`[[:alpha:]]`, `[[:punct:]]`, …) are supported and expand as picomatch expands them.
