...
Glob semantics: tool names are opaque strings, not paths — `*`, `?`, and `**` cross `/`, so `admin*` matches `admin/delete`, and a class member `/` matches a slash (`admin[/]delete` matches `admin/delete`). A leading `!` or extglob prefix (`+(a|b)`) is treated as a literal. Caveats (rare patterns; documented here so they are not surprises):
- `*` spans everything, but `?` and character classes resolve against an internal matching domain where the slash is one transliterated character — `?` spans a single slash position, a *negated* class without a `/` member (`[^a]`) treats the slash as an ordinary member (so `[^a]` matches a slash), while a class with an explicit `/` member only routes slash-containing names when that member is *positive* (`[/]` matches, `[^a/]` does not), and implicit class *ranges* (`[.-0]`, where `/` lies between `.` and `0`) do not span it — spell the slash member explicitly (`[/]`) instead. The common patterns (`*`, `**`, explicit `/` members, braces, literal names) are unaffected.
