...
- a *negated* class cannot exclude the slash character — `[^/]` matches slash-containing names too;
- a *negated* class with an explicit `/` member (`[^a/]`) matches slash-containing names as if the `/` member were absent;
- `?` and class *ranges* (`[.-0]`) resolve against the internal matching domain, so they cannot span a slash position — use `*` or an explicit `/` class member instead. The common patterns (`*`, `**`, explicit `/` members, braces, literal names) are unaffected.
