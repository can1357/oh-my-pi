...
| `gemini-3-flash` / `gemini-3-flash[]` + tools | Catalog `sand-tools-wire=keep-model`: product Shell/Read/Write + jsonSchema on the original Gemini id (bare, no effort params). Native field-2 completed empty. Also promotes ```tool_code / `default_api.bash(...)` from thinking/text. Empty/incomplete tool turns retry with thinking off. |
