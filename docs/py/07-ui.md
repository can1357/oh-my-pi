...
| Registry identity digests | `crates/tool/src/registry.rs:2623` `slot_hash()` (policy-resolved model-visible slots), `:2654` `device_hash()` (mounted availability + claimant-qualified reachability), `:2690` `projection_hash()` (every registered `(name, rev)` + projection code); each blake3-256 | Yes. The questions are already split at the source — prompt-cache identity keys on `slot_hash`, device lifecycle on `device_hash`, projection identity on `projection_hash`; never collapse them back into one digest. |
