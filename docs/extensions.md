...
- `agentIdentity` — identity of the agent this handler runs in: `kind` (`"main"` | `"sub"`), `depth` (`0` = top-level), `agentId` (registry id, `"Main"` for the top-level session), `displayName`, optional `parentId`, and `parentChain` (nearest-first ancestor registry ids, excluding `"Main"`; empty for the top-level session and for runner-less SDK sessions, which cannot resolve the chain). Read lazily; frozen — copy rather than mutate.
