...
- `tool_approval_requested` / `tool_approval_resolved` (observability everywhere; for interactive `edit`/`write` approvals the requested event additionally carries `files` — the content-only proposals `{ path, before, after }`, paths relative to the session cwd, structural operations (delete/rename) excluded — and a `respond({ approved, files? })` promise: the first valid responder between the TUI prompt and `respond()` wins, an invalid response throws and leaves the dialog open, a late response resolves `false`, and revisions are re-validated against the tool schema and per-file approval policy before the edited contents replace the proposed ones; `tool_approval_resolved` gains `source: "user" | "extension" | "abort"` so an IDE can close its tabs whichever way the approval settled)
