...
- `tool_approval_requested` / `tool_approval_resolved` (observability everywhere; for interactive `edit`/`write` approvals the requested event additionally carries `files` — the content-only proposals `{ path, before, after }`, paths relative to the session cwd for files inside it (absolute for targets resolved outside it, e.g. `local://` artifacts), structural operations (delete/rename) excluded — and a `respond({ approved, files? })` promise: the first valid responder between the TUI prompt and `respond()` wins, an invalid response throws and leaves the dialog open, a late response resolves `false`; settlement notifications are serialized after the requested delivery so no extension observes `tool_approval_resolved` before its `tool_approval_requested`. `respond` `files` revisions are re-validated against the tool schema and per-file approval policies before settling.
