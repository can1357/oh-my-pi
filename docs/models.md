...
- `replayReasoningContent` — replay prior assistant thinking as `reasoning_content` on every assistant turn that carried reasoning. Default: auto (on for local llama.cpp-style backends). Set `true` for remote endpoints that require historical `reasoning_content` to be sent back (e.g. DashScope Qwen 3.8 with `preserve_thinking` on).
- `qwenPreserveThinking` — send `preserve_thinking: true` so the Qwen3.6+ chat template keeps rendering `<think>...</think>` on older assistant turns instead of stripping them. Default: auto (Qwen thinking format on a local backend).
