...
- **Provider adapters**
  ...
  - **xAI** — `packages/coding-agent/src/web/search/providers/xai.ts`
    ...
    - Querying: POSTs the Responses API with `tools: [{ type: "web_search", ... }]`; sends reasoning effort `low` only when the selected model's registry/catalog compatibility permits it, otherwise omits the field. Custom endpoints retain their configured `xai-oauth` API keys. Only official OAuth/env credentials fall back to an available `xai` key; without one, the request is refused before sending credentials.
    - Model: `providers.webSearchXaiModel` selects the Grok search model; `XAI_SEARCH_MODEL` overrides it. Defaults to the provider's catalog default model (currently `grok-4.6`).
    - Relay routing: `XAI_BASE_URL` + `XAI_API_KEY` applies after the selected provider's registry overrides, and also works without a registry (SDK custom-tool embedding). `models.yml` `providers.xai.baseUrl`/`apiKey` applies only when `xai` is selected. A preferred `xai-oauth` credential does not consult `providers.xai` overrides: configure `providers.xai-oauth` with its own relay endpoint/key, or use the environment override when no registry endpoint override takes precedence.
