...
- **Provider adapters**
  ...
  - **Parallel** — `packages/coding-agent/src/web/search/providers/parallel.ts`, `packages/coding-agent/src/web/parallel.ts`
    - Availability: available without credentials and first in the default automatic order. Configured provider orders and exclusions still apply.
