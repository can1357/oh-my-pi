...
- **Provider adapters**
  ...
  - **xAI** — `packages/coding-agent/src/web/search/providers/xai.ts`
    ...
    - Querying: POSTs the Responses API with `tools: [{ type: "web_search", ... }]` and reasoning effort `low`. Custom endpoints retain their configured `xai-oauth` API keys. Only official OAuth/env credentials fall back to an available `xai` key; without one, the request is refused before sending credentials.
