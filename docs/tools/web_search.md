...
- **Provider adapters**
  ...
  - **Gemini** — `packages/coding-agent/src/web/search/providers/gemini.ts`
    ...
    - Model: `providers.webSearchGeminiModel` selects the Gemini grounding model; `GEMINI_SEARCH_MODEL` overrides it. Defaults to the rolling `gemini-flash-latest` alias (pin an explicit version for reproducible grounding).
