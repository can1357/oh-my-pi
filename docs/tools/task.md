...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, task, effort?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.enabled` is true, plan mode is disabled, and the calling agent is not itself isolated (unless `task.isolation.allowNested` is enabled, default false). Inside an isolated agent without the setting, the tool description omits `isolated` and the wire schema constrains it to `false` on providers that preserve const/enum (an explicit `isolated: true` — on the task tool or the eval `agent()` — is rejected with a clear error instead of silently ignored; the Google `parametersJsonSchema` normalizer degrades the constraint to a bare boolean, so Gemini/Vertex advertise an undescribed optional field whose explicit values still reject); `effort` exists only when `task.enableEffort=true` (default off).

- **Batch shape** (`task.batch` on): `{ context, tasks: item[] }` — one subagent per item, all run under the same fan-out rules; there is no top-level agent field. `context` is **required** shared background rendered into every spawned subagent's system prompt (`CONTEXT` section); `agent`, `outputSchema`, and `schemaMode` are per item. `effort` is added only when its setting enables it; `isolated` additionally requires plan mode to be disabled and the calling agent to not be isolated (unless `task.isolation.allowNested` is enabled, default false).
...
| `isolated` | `boolean` | No | Run in an isolated workspace and return patches. Exists only when `task.isolation.enabled` is true, plan mode is disabled, and the calling agent is not itself isolated (unless `task.isolation.allowNested` is enabled, default false). Inside an isolated agent without the setting, the tool description omits `isolated` and the wire schema constrains it to `false` on providers that preserve const/enum (an explicit request — on the task tool or the eval `agent()` — is rejected with a clear error instead of silently ignored); per item in batch shape, top-level in flat shape. Isolated agents are torn down at completion — not revivable. |
