...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, task, effort?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.mode` is not `none`, plan mode is disabled, and the calling agent is not itself isolated (unless `task.isolation.allowNested` is enabled, default false). Inside an isolated agent without the setting, the tool description omits `isolated` and the wire schema constrains it to `false` (an explicit `isolated: true` — on the task tool or the eval `agent()` — is rejected with a clear error instead of silently ignored); `effort` exists only when `task.enableEffort=true` (default off).
...
| `isolated` | `boolean` | No | Run in an isolated workspace and return patches. Exists only when `task.isolation.mode` is not `none`, plan mode is disabled, and the calling agent is not itself isolated (unless `task.isolation.allowNested` is enabled, default false). Inside an isolated agent without the setting, the tool description omits `isolated` and the wire schema constrains it to `false` (an explicit request — on the task tool or the eval `agent()` — is rejected with a clear error instead of silently ignored); per item in batch shape, top-level in flat shape. Isolated agents are torn down at completion — not revivable. |
