...
For example, warm up `{ "task": "Study the repository structure.", "saveSnapshotAs": "shared-base" }`. After it yields, start several items of the same agent type with `fromSnapshot: "shared-base"` and different assignments. Each new child receives the frozen warm-up branch and its new call context. Later tasks can fork the same snapshot without inheriting earlier forks' work.
