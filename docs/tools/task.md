...
- Advisor: agent frontmatter `advisor` or `task.agentAdvisor[agentName]` (`"on"` / `"off"` / model pattern) controls enrollment. A nonempty `watchdogs` list also opts in unless explicitly disabled. Watchdogs can be bundled definitions or references to shared IDs; see [agent watchdogs](../advisor-watchdog.md#bundle-watchdogs-with-an-agent). An explicit advisor pattern sets the child's `modelRoles.advisor`; individual watchdog models take precedence.
