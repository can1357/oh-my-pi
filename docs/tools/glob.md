...
| `limit` | `number` | No | Max returned paths. Defaults to `200`; finite positive inputs are floored with a minimum of `1`. |
...
7. `limit` defaults to `DEFAULT_LIMIT` (`200`), must be positive and finite, and is floored with a minimum of `1`; larger requested limits are honored. `hidden` and `gitignore` both default to `true`. An internal timeout of `5` seconds (`5000` ms) is built via `AbortSignal.timeout(...)`.
...
- Explicit positive result limits are not capped at `200`; the native glob binding accepts up to `u32::MAX`, and scans may still hit the 5-second timeout or output byte cap.
