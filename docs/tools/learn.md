...
- Backend-specific persistence: queued Hindsight, scoped Mnemopi SQLite, synchronously stored Dakera row, or project-scoped local `learned.md`.
...
- Network: Hindsight queue flushes to the configured server later; Dakera stores with `POST /v1/memories/store/batch` before the call returns. Mnemopi can schedule configured embedding/fact-extraction provider work after the synchronous row write; local file-backed storage itself is offline.
