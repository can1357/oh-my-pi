...
Native Responses replay plaintext is obfuscated too, including message text and tool arguments/results. Advisors re-scrub retained replay and its next-compaction source when later regex values make an earlier friendly prefix unsafe, including snapshots committed while a new value arrives. Encrypted replay, protocol identifiers, and image/file bytes remain untouched; opaque encrypted history cannot be retrospectively inspected or rewritten.
