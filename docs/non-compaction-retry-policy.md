...
8. If the final delay exceeds `retry.maxDelayMs` and no credential/model switch happened, emit final failure without sleeping. The message names the winning wait's provenance: `Provider requested …` when it traces to timing the provider stated — a parsed retry hint, a complete usage-report reset, a provider-parked credential block, or a provider-parked sibling's unblock deadline — otherwise `OMP-estimated …` for the capped backoff, the hintless `QUOTA_EXHAUSTED` heuristic, and any deadline only the persisted block store holds (persisted blocks carry no provenance).
