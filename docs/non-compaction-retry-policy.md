...
- `retry.fallbackRevertPolicy` (`"cooldown-expiry"` by default; `"never"` disables automatic restoration). Under `"cooldown-expiry"`, restores are bounded per fallback episode: after 3 consecutive restores of the same primary fail without it serving a turn, the session stops restoring and stays on the fallback. A served turn on the restored primary, a prompt already running on the primary, or an explicit model change resets the count.
...
- Model fallback changes append temporary `model_change` entries and may later restore the primary model when its cooldown expires, depending on `retry.fallbackRevertPolicy`. A restore never targets the model already in use, and at most 3 consecutive failed restores are attempted per fallback episode before the session pins to the fallback.
