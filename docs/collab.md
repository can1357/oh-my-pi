...
1. `welcome` + `snapshot-chunk` frames — initial state and transcript. The transcript is byte-bounded into chunks so each arrival resets the guest's progress timeout; oversized replicated entries are shrunk before transmission, and anything trimmed carries a `collabElided` record so the guest can fetch it later. A guest that sends `hello.snapshot = { mode: "tail", maxBytes }` receives only the latest whole turns within that budget, and `welcome.history` tells it where that tail starts and whether earlier entries exist.
...
Guest→host: `hello`, `prompt`, `abort`, `agent-cmd` (hub chat/kill/revive), `fetch-transcript` (incremental subagent-transcript reads answered by targeted `transcript` frames), `fetch-history` (the whole turns before an entry id, answered by a targeted `history` frame, or `stale` once that entry has left the active branch), `fetch-value` (the full value behind a `collabElided` record, answered in slices by targeted `value` frames), and `ui-response`. The replica loads through the regular `/resume` machinery, so theming, ctrl+o, and transcript behavior are native by construction; the guest process never chdirs to host paths.
