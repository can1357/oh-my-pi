...
V2 uses an absolute per-request deadline of 180,000 ms, configurable with `compaction.v2TimeoutMs`. For a slow provider, `omp config set compaction.v2TimeoutMs 300000` allows five minutes; `0` disables this deadline. Progress events do not refresh it. Expiry preserves the timeout reason and does not retry the same V2 request with another full deadline. Transient transport failures still use the existing retry policy. This setting does not change V1/custom-endpoint timeouts or bound the full method/provider fallback chain.
