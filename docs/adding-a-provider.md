...
| `preparePiNativeHeaders` | Provider-owned headers to forward with a `pi-native` request (`{ [name]: value }`, merged under the caller's). Use for request policy this process owns but the gateway cannot recompute — e.g. an environment-gated retention opt-in. Forward it under a private name and consume it in `prepareRequest`, since the receiving process must reconcile it against its own resolved model before it becomes a request header.                                                                 |
