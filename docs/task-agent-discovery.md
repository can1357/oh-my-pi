...
- `model` accepts one selector, CSV, or an array. Entries are tried in order after role aliases are expanded. The literal `inherit` is a model-neutral sentinel: it skips the agent-level pin and uses the parent session's active model, then the configured/default fallback.
