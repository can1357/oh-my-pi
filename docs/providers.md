...
`grokbot` is not Cursor OAuth and not xAI. `/login grokbot` only surfaces the host-install prompt for `~/.omp/agent/secrets/grokbot.env`. One-shot probes: [Grok Bot](./grokbot.md).
