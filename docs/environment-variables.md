...
| `AKI_API_KEY`                   | AKI.IO auth                                      | Using `aki-io` provider                                        | OpenAI-compatible endpoint at `https://aki.io/openai/v1`                                             |
| `CORTECS_API_KEY`               | Cortecs auth                                     | Using `cortecs` provider                                       | OpenAI-compatible European LLM router at `https://api.cortecs.ai/v1`                                 |
| `EUROUTER_API_KEY`              | EUrouter auth                                    | Using `eurouter` provider                                      | OpenAI-compatible European routing gateway at `https://api.eurouter.ai/api/v1`                       |
...
| `MELIOUS_API_KEY`               | Melious auth                                     | Using `melious` provider                                       | OpenAI-compatible European inference endpoint at `https://api.melious.ai/v1`                         |
| `NEBIUS_API_KEY`                | Nebius Token Factory auth                        | Using `nebius` provider                                        | OpenAI-compatible Nebius Token Factory endpoint at `https://api.tokenfactory.nebius.com/v1`          |
| `OVH_AI_ENDPOINTS_ACCESS_TOKEN` | OVHcloud AI Endpoints auth                       | Using `ovhcloud` provider                                      | OpenAI-compatible endpoint at `https://oai.endpoints.kepler.ai.cloud.ovh.net/v1`                      |
| `OPPER_API_KEY`                 | Opper auth                                       | Using `opper` provider                                         | OpenAI-compatible endpoint at `https://api.opper.ai/v3/compat`                                       |
...
| `SCW_SECRET_KEY`                | Scaleway Generative APIs auth                    | Using `scaleway` provider                                      | OpenAI-compatible endpoint at `https://api.scaleway.ai/v1`                                           |
