...
| `MERGE_GATEWAY_API_KEY`         | Merge Gateway auth                               | Using `merge-gateway` provider                                 | Can also be stored with `/login merge-gateway`; model discovery uses the authenticated Gateway catalog |
