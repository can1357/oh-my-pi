...
| `CMD_ZDR`                       | Command Code zero-data-retention opt-in          | Using `commandcode` provider                                   | Set to `1` to send the documented `x-cmd-zdr: 1` header (forwarded through a `pi-native` gateway under an internal name and applied there only if the gateway's own model sets none); explicit `x-cmd-zdr` headers win |
