...
| `XAI_API_OAUTH_TOKEN`           | xAI API-only OAuth bearer                       | Using `xai-api-oauth` provider                                 | Requests use `api:access` without `grok-cli:access`; normally populated through `/login xai-api-oauth`                |
