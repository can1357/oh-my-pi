...
| `FACTORY_DROID_ACCESS_TOKEN`    | Factory Droid data-plane auth                    | Headless `factory-droid` use without a local Droid sign-in     | WorkOS access token; normally resolved from the local `droid auth login` session instead            |
| `FACTORY_DROID_ORG_ID`          | Factory Droid org override                       | Headless `factory-droid` use with `FACTORY_DROID_ACCESS_TOKEN` | Optional; defaults to the org stored with the local session                                          |
| `FACTORY_DROID_SESSION_SYNC`    | Factory Droid dashboard session registration     | Opt-in `factory-droid` dashboard parity                        | `1`/`true` registers each session via `/api/sessions/create` so it appears on the Factory site       |
