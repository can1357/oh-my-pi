...
| `NOUS_PROXY_API_KEY`            | Nous Portal proxy client bearer                  | Using `nous` through `hermes proxy start`                      | The local proxy accepts any non-empty value and attaches the real refreshed Portal credential       |
