...
| `STEPFUN_CN_API_KEY`            | StepFun Step Plan (China) auth                   | Using `stepfun-cn` provider                                    | `STEP_API_KEY` is ignored so a pay-as-you-go key does not enable this provider                      |
