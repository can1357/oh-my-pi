...
Most runtime lookups use `$env` from `@pk-nerdsaver-ai/pi-utils` (`packages/utils/src/env.ts`).
...
| `ALIBABA_TOKEN_PLAN_API_KEY`    | QwenCloud Token Plan auth                        | Using `alibaba-token-plan` provider                            |                                                                                                     |
| `BAILIAN_TOKEN_PLAN_API_KEY`    | QwenCloud Token Plan auth (alias)                | Using `alibaba-token-plan` provider                            | Checked after `ALIBABA_TOKEN_PLAN_API_KEY`                                                          |
| `DEEPSEEK_API_KEY`              | DeepSeek auth                                    | Using DeepSeek models                                          |                                                                                                     |
| `KILO_API_KEY`                  | Kilo auth                                        | Using Kilo models                                              |                                                                                                     |
| `CLINE_API_KEY`                 | Cline account gateway auth                       | Using Cline models (or `/login cline`)                         | OpenAI-compatible gateway at `https://api.cline.bot/api/v1`                                         |
...
| `PI_TOKENIZER`               | `estimate` forces the `bytes/4` token heuristic instead of the native cl100k tokenizer (perf escape hatch). Default: native cl100k when the addon loads (pathological single-char runs are clipped and scaled so counting stays fast), byte estimate otherwise. Replaces the removed `PI_TOKENIZER_ACCURATE` opt-in — accurate counting is now the default, including under tests.                                                                                                                          |
...
These are consumed via `@pk-nerdsaver-ai/pi-utils/dirs` and affect where coding-agent stores data.
