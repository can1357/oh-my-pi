...
Launch a split-view multi-agent workflow with one main agent (coordinator) and N sub-agents (workers) coordinating through a file-based queue.
...
1. Create the `.side-agent/` coordination directory, including the `claims/`, `workers/`, and `results/` subdirectories.
...
5. Poll for results, recover stale claims, and aggregate.
...
2. Enter the poll loop: **atomically** claim a task by creating its claim directory, execute it, write the result, and repeat.
...
The agent reads the existing queue, atomically claims a task via its claim directory, executes it, writes the result, and loops.
